...
char clockonoff[4]={0};
static struct class *clockprintonoff_class =NULL;
int clock_show_yl(int force);
#endif
	...
	do {
...
#ifdef CONFIG_YL_PM_DEBUG
		{
			...
			clock_debug_output(m, 1, "%s%s:%u:%u [%ld]", start,
				...
				c->rate);
#ifdef CONFIG_YL_PM_DEBUG
                        pr_err("%s%s:%u:%u [%ld]\n",start,c->dbg_name,c->prepare_count, c->count,c->rate);
                }
#endif
		start = " -> ";
...
#ifdef CONFIG_YL_PM_DEBUG
static ssize_t clock_onoff_show(struct class *cls , struct class_attribute *attr, char *buf)
{
	printk(KERN_ERR"%s,clock_onoff status=%s\n",__func__,buf);
	return sprintf(buf, "%s\n", clockonoff);
}
static ssize_t clock_onoff_store(struct class *cls,struct class_attribute *attr, const char *buf, size_t count)
{
	int err = 0;
	printk(KERN_ERR"Input is:%s:%zd\n",buf,count);
	if( count > 4 )
		err =-1;
	else{
		sscanf(buf,"%s",clockonoff);
		//err = strcmp(value,achr);
	}
	return count;
}

static CLASS_ATTR(clock_onoff, 0664, clock_onoff_show,clock_onoff_store );

static ssize_t clock_print_yl_show(struct class *cls , struct class_attribute *attr, char *buf)
{
	return clock_show_yl(1);
}
static CLASS_ATTR(clock_print_yl, 0664, clock_print_yl_show, NULL);
#endif
...
static int clock_debug_add(struct clk *clock)
{
	...
	if(NULL == clockprintonoff_class)
	{	
		clockprintonoff_class = class_create(THIS_MODULE, "clockprintonoff");
		if(IS_ERR(clockprintonoff_class))
		{
			printk(KERN_ERR " %s: failed to create clockprintonoff_class\n", __func__);
		}
		err = class_create_file(clockprintonoff_class, (const struct class_attribute *)&class_attr_clock_onoff.attr);
		
		if(err < 0)
		{
			printk(KERN_ERR"failed to create class_attr_clock_onoff\n");
		}
		err = class_create_file(clockprintonoff_class, (const struct class_attribute *)&class_attr_clock_print_yl.attr);
		
		if(err < 0)
		{
			printk(KERN_ERR"failed to create class_attr_clock_print_yl\n");
		}
	}
#endif
