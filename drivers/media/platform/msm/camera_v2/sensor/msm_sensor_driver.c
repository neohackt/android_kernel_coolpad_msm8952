	...
	struct msm_sensor_power_setting *pu)
{
	...
	if (is_compat_task()) {
		...
		for (i = 0; i < size_down; i++) {
			...
			pd[i].use_system_power = power_setting_iter[i].use_system_power; 
		}
	} else
#endif
	{
	...
	*  sensor control structure */
	...
	if (rc < 0) {
		...
		goto free_camera_info;
            ...
            if(!strncmp(slave_info->sensor_name, "ov8856", strlen("ov8856"))) {
	    rc = ov8856_get_interface(s_ctrl);
	    if(rc < 0) {
	        pr_err("%s:ov8856_get_interface enter failed \n", __func__);
	    }else {
	         pr_err("%s: ov8856 s_ctrl->sensor_prepare_otp is %d \n",__func__, s_ctrl->sensor_prepare_otp);
	    }
	} 
...
//zq s
       ...
       {
         s_ctrl->camerainfo_devnode.name = "camera2";
       }
