...
bool front_camera_probe_ok = false;
bool rear_camera_probe_ok = false;
bool front_prepare_otp_data = false;
bool rear_prepare_otp_data = false;
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (rc < 0) {
		//added by congpeichao start
		if(s_ctrl->sensordata->sensor_info->position){
			if(front_camera_probe_ok == true )
				front_camera_probe_ok = false;
		}else{
			if(rear_camera_probe_ok == true)
				rear_camera_probe_ok = false;
		}
		//added by congpeichao end
	...
	//added by congpeichao start	
	printk("sensor info: name: %s, prepare_otp_data: %d, prepare_otp_data:front %d, rear %d.\n", 
	        s_ctrl->sensordata->sensor_name, s_ctrl->sensor_prepare_otp, 
	        front_prepare_otp_data, rear_prepare_otp_data);
	if(s_ctrl->sensor_prepare_otp == 1 && s_ctrl->func_tbl->sensor_prepare_otp)	{
		if (rear_prepare_otp_data==false && (0 == s_ctrl->sensordata->sensor_info->position)) {
	    	//printk("start prepare_otp_data for rear camera\n");
	  		if (s_ctrl->func_tbl->sensor_prepare_otp(s_ctrl))
	        	printk("rear camera sensor_prepare_otp failed\n");
	    	else 
	        	rear_prepare_otp_data = true;
	      	//printk("finish prepare_otp_data for rear camera\n");
 		} else if (front_prepare_otp_data==false && (1 == s_ctrl->sensordata->sensor_info->position)) {
        	//printk("start prepare_otp_data for front camera\n");
        	if (s_ctrl->func_tbl->sensor_prepare_otp(s_ctrl))
            	printk("front camera sensor_prepare_otp failed\n");
        	else 
	        	front_prepare_otp_data = true;
    		//printk("finish prepare_otp_data for front camera\n");
		} else
        	printk(KERN_INFO "already prepare OTP data for sensor(%s).\n", s_ctrl->sensordata->sensor_name);
	} else {
    	printk("No need to prepare OTP data for sensor(%s).\n", s_ctrl->sensordata->sensor_name);
	}

	if(!(s_ctrl->module_id & (0xFFFF << 16)))
    	s_ctrl->module_id |= (chipid << 16);
	
	printk("upto vendor module_id(factory_id):0x%x\n",s_ctrl->module_id);
    if (s_ctrl->sensordata->sensor_info->position) {
	    printk("front camera probe ok\n");
	    front_camera_probe_ok = true;
    } else {
		printk("rear camera probe ok\n");
		rear_camera_probe_ok = true;
    }
	//added by congpeichao end 
	...
	void __user *argp)
{
	...
	case CFG_UPDATE_OTP:
		if(s_ctrl->func_tbl->sensor_update_otp){
        	...
        	if(s_ctrl->module_id)
            	...
            	cdata->cfg.sensor_init_params.module_id = 0;
		}
		break;
	...
	case CFG_POWER_UP:
		...
		if (s_ctrl->func_tbl->sensor_power_up) {
			...
			//added by congpeichao start
			//for vendor get module id from camera module using eeprom not otp like sony imx214
			if(s_ctrl->module_id)
				cdata->cfg.sensor_init_params.module_id = s_ctrl->module_id;
			else
				cdata->cfg.sensor_init_params.module_id = 0;
...
ssize_t camerainfo_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
        ...
        uint32_t module_temp= 0;
        printk(KERN_ERR "%s :%x %d count=%zd f_ops=%d", __func__, s_ctrl->module_id, (int)(*offp), count, (int)file->f_pos);

        /*if(vendor_id_in_eeprom && !(s_ctrl->module_id & 0xFF))
                s_ctrl->module_id |= vendor_id_in_eeprom;*/
		module_temp = s_ctrl->module_id;
		module_temp = module_temp & 0xFF;
        switch(s_ctrl->module_id) {
		//add by wbl for daren product for do not display module name     	
#ifdef CONFIG_NODISPLAY_PRODUCT
			case MODULE_SUNNY_OV13850:
			case MODULE_TECH_OV13850:
			case MODULE_SUNNY_IMX135:	
			case MODULE_QTECH_IMX135:
			case MODULE_FOXCONN_IMX135:
			case MODULE_SUNNY_IMX214:
			case MODULE_OFILM_IMX214:
			case MODULE_SUNNY_AR1335:
			case MODULE_OFILM_AR1335:
				return sprintf(buff, "%s%s %X", "1 ", "13M-Camera",module_temp);
			    break;

			case MODULE_SUNNY_8865:
			case MODULE_OFILM_8865:
			case MODULE_FOXCONN_8865:
			case MODULE_TECH_8865:
				return sprintf(buff, "%s%s %X", "1 ", "8M-Camera",module_temp);
			    break;
			case MODULE_SUNNY_OV5648:
			case MODULE_OFILM_OV5648:
			case MODULE_TECH_OV5648:
			case MODULE_TECH_OV5693:
				return sprintf(buff, "%s%s %X", "1 ", "5M-Camera",module_temp);
			    break;
#else
			case MODULE_SUNNY_8865:
			case MODULE_SUNNY_OV13850:
			...
			case MODULE_SUNNY_IMX214:
			case MODULE_SUNNY_OV5648:
			case MODULE_SUNNY_AR1335://wbl add
				return sprintf(buff, "%s%s %s", "1 ", s_ctrl->sensordata->sensor_name, "Sunny");
				break;
			case MODULE_OFILM_AR1335://wbl add				
			case MODULE_OFILM_8865:
			case MODULE_OFILM_OV5648:
			case MODULE_OFILM_IMX214:
				return sprintf(buff, "%s%s %s", "1 ", s_ctrl->sensordata->sensor_name, "OFilm");
				break;
			case MODULE_FOXCONN_8865:
			case MODULE_FOXCONN_OV13850:
			case MODULE_FOXCONN_IMX135:
				return sprintf(buff, "%s%s %s", "1 ", s_ctrl->sensordata->sensor_name, "FoxConn");
				break;
			case MODULE_TECH_8865:
			case MODULE_TECH_OV5693:
			case MODULE_TECH_OV13850:				
			case MODULE_QTECH_IMX135:
			case MODULE_TECH_OV5648:
				return sprintf(buff, "%s%s %s", "1 ",s_ctrl->sensordata->sensor_name, "Tech");
				break;
#endif
			default:
#ifdef CONFIG_NODISPLAY_PRODUCT
				if(strcmp(s_ctrl->sensordata->sensor_name, "ov5648") == 0){
					return sprintf(buff, "%s%s %X", "1 ","5M-Camera",module_temp);
				}else{
					return sprintf(buff, "%s%s", "1 ", s_ctrl->sensordata->sensor_name);
				}
#else 
				return sprintf(buff, "%s%s", "1 ", s_ctrl->sensordata->sensor_name);
#endif
...
int camerainfo_open(struct inode *inode, struct file *file)
{
    struct miscdevice *cdn = (struct miscdevice*)file->private_data;
    struct msm_sensor_ctrl_t *s_ctrl = container_of(cdn, struct msm_sensor_ctrl_t, camerainfo_devnode);
    printk(KERN_ERR "%s %p %p", __func__, cdn, s_ctrl);
...
//zq s
    ...
    s_ctrl->camerainfo_devnode.name = s_ctrl->sensordata->sensor_info->position ? "camera1" : "camera0";
...
//added by congpeichao begin
bool rearCameraProbeInfo(void){
	printk("%s rear_camera_probe_ok = %d\n",__func__,rear_camera_probe_ok);
	return rear_camera_probe_ok;
}

bool frontCameraProbeInfo(void){
	printk("%s front_camera_probe_ok = %d\n",__func__,front_camera_probe_ok);
	return front_camera_probe_ok;
}
//added by congpeichao end
