...
struct msm_sensor_fn_t {
	...
	int (*sensor_prepare_otp)(struct msm_sensor_ctrl_t *s_ctrl);//otp@cn
	int (*sensor_update_otp) (struct msm_sensor_ctrl_t *);//use for otp
};
...
struct msm_sensor_ctrl_t {
	...
	uint8_t is_yuv;
	bool sensor_prepare_otp; //get the updated OTP data in probe to make camera open quickly@cn
	uint32_t module_id;//muyuezhong add,zq modify
...
bool rearCameraProbeInfo(void);//added by congpeichao
bool frontCameraProbeInfo(void);//added by congpeichao
