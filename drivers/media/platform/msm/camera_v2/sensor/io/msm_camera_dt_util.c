...
//add by congpeichao for fix kernel die start				
			else if (!strcmp(seq_name, "sensor_gpio_af-pwdm"))
				ps[i].seq_val = SENSOR_GPIO_AF_PWDM;
			else if (!strcmp(seq_name, "sensor_gpio_reset"))
				ps[i].seq_val = SENSOR_GPIO_RESET;
//add by congpeichao for fix kernel die end									
...
int msm_camera_power_up_share_power(struct msm_camera_power_ctrl_t *ctrl,
	enum msm_camera_device_type_t device_type,
	struct msm_camera_i2c_client *sensor_i2c_client);
int msm_camera_power_down_share_power(struct msm_camera_power_ctrl_t *ctrl,
	enum msm_camera_device_type_t device_type,
	struct msm_camera_i2c_client *sensor_i2c_client);
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	pr_err("%s:%d dev name = %s\n", __func__, __LINE__, dev_name(ctrl->dev));
	if (strstr(dev_name(ctrl->dev), "camera") != NULL)
		return msm_camera_power_up_share_power(ctrl, device_type, sensor_i2c_client);
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	pr_err("%s:%d dev name = %s\n", __func__, __LINE__, dev_name(ctrl->dev));
	if (strstr(dev_name(ctrl->dev), "camera") != NULL)
		return msm_camera_power_down_share_power(ctrl, device_type, sensor_i2c_client);
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	int rc = 0, index = 0, no_gpio = 0, ret = 0;
	struct msm_sensor_power_setting *power_setting = NULL;
	struct msm_camera_sensor_board_info * sensor_board_info = NULL; //added by congpeichao

	sensor_board_info = container_of(ctrl, struct msm_camera_sensor_board_info ,power_info);
	CDBG("%s:%d dev name = %s\n", __func__, __LINE__, dev_name(ctrl->dev));
	CDBG("%s:%d postition=%d\n", __func__, __LINE__, sensor_board_info->sensor_info->position);
	if (!ctrl || !sensor_i2c_client) {
		pr_err("failed ctrl %p sensor_i2c_client %p\n", ctrl,
			sensor_i2c_client);
		return -EINVAL;
	...
	power_setting = &ctrl->power_setting[0];

	if (power_setting->use_system_power) { 
		if(((0 == sensor_board_info->sensor_info->position) && rearCameraProbeInfo()) || 
		    ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo())) {
			
			CDBG("current sensor is probed,do not request reset,pwd,ldo,af_pwd gpios,just request mclk gpio\n");
			...
			for (index = 0; index < ctrl->gpio_conf->cam_gpio_req_tbl_size; index++) {
		        ...
		        if (strncmp(ctrl->gpio_conf->cam_gpio_req_tbl[index].label,"CAMIF_MCLK", strlen("CAMIF_MCLK")) == 0) {
	                CDBG("request mclk gpio\n");
	                gpio_request(ctrl->gpio_conf->cam_gpio_req_tbl[index].gpio,ctrl->gpio_conf->cam_gpio_req_tbl[index].label);
				}
			}
	    } else {
            rc = msm_camera_request_gpio_table(
		...
		case SENSOR_VREG:
			if (power_setting->seq_val >= CAM_VREG_MAX) {
				...
				goto power_up_failed;
                        ...
                        if ((power_setting->use_system_power) &&
                                (((0 == sensor_board_info->sensor_info->position) && rearCameraProbeInfo()) || 
                                ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo()))) {
                                CDBG("current sensor use system power,and has probed ok,do not request verg now\n");
                        } else {
			        if (power_setting->seq_val < ctrl->num_vreg)
                                        msm_camera_config_single_vreg(ctrl->dev,
...
power_up_failed:
	...
	if (power_setting->use_system_power) 
	{
        ...
        printk("camera sensor release gpios\n");        
        msm_camera_request_gpio_table(
            ctrl->gpio_conf->cam_gpio_req_tbl,
            ctrl->gpio_conf->cam_gpio_req_tbl_size, 0);
    }
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	struct msm_sensor_power_setting *ps;
    struct msm_camera_sensor_board_info * sensor_board_info = NULL; //added by congpeichao
	sensor_board_info = container_of(ctrl, struct msm_camera_sensor_board_info ,power_info);//added by congpeichao
