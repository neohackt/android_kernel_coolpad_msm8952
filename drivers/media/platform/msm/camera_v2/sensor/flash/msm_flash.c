...
static struct led_trigger *torch_trigger1;
static struct led_trigger *switch_trigger = NULL;
static int torch_register_num = 0;
				...
				enum led_brightness value)
{
	...
	if (!torch_trigger1) {
		pr_err("No torch trigger1 found, can't set brightness\n");
		return;
	}

	led_trigger_event(torch_trigger, value);
    //	led_trigger_event(torch_trigger1, value);
...
void msm_torch_brightness_set1(struct led_classdev *led_cdev,
                                enum led_brightness value)
{
        if (!torch_trigger) {
                pr_err("No torch trigger found, can't set brightness\n");
        ...
        pr_err("msm_torch_brightness_set value:%d\n",value);
      //  led_trigger_event(torch_trigger, value);
        led_trigger_event(torch_trigger1, value);
...
static struct led_classdev msm_torch_led[MAX_LED_TRIGGERS] = {
	...
	{
		.name		= "torch-light1",
		.brightness_set	= msm_torch_brightness_set1,
		.brightness	= LED_OFF,
	},
	{
		.name		= "torch-light2",
...
/*
			if(0 == i)
				...
				torch_trigger1 = fctrl->torch_trigger[i];
*/
                        if(0==torch_register_num)
                             torch_trigger = fctrl->torch_trigger[i];
                        else
                             torch_trigger1 = fctrl->torch_trigger[i];
			...
			rc = led_classdev_register(&pdev->dev,
				&msm_torch_led[torch_register_num]);
			if (rc) {
	...
	struct msm_sensor_power_setting32 *ps32, uint32_t size)
{
	...
	for (i = 0; i < size; i++) {
		...
		ps[i].use_system_power = ps32[i].use_system_power; 
	}
}
#endif
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	for (i = 0; i < flash_ctrl->flash_num_sources; i++)
		if (flash_ctrl->flash_trigger[i]) {
			CDBG("flash_trigger = %p", flash_ctrl->flash_trigger[i]);
			led_trigger_event(flash_ctrl->flash_trigger[i], 0);
		}
