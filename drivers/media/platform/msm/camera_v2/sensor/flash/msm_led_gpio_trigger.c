...
extern int32_t msm_led_gpio_torch_create_classdev(
...
int msm_flash_led_gpio_init(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	if (power_info->gpio_conf->cam_gpio_req_tbl_size >= 2 && 
		(power_info->gpio_conf->cam_gpio_req_tbl[0].gpio == power_info->gpio_conf->cam_gpio_req_tbl[1].gpio)) {
	...
	const void *data)
{
	...
	if (!rc)
		msm_led_gpio_torch_create_classdev(pdev, fctrl);
