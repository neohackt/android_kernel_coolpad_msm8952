...
//yulong add
#ifdef CONFIG_YULONG_DEBUG
int yldebug_subsystem_restart_dev(struct subsys_device *dev)
{
	const char *name;
...
EXPORT_SYMBOL(yldebug_subsystem_restart_dev);

int yldebug_subsystem_restart(const char *name)
{
	int ret;
	struct subsys_device *dev = find_subsys(name);
	...
	ret = yldebug_subsystem_restart_dev(dev);
	put_device(&dev->dev);
	return ret;
}
EXPORT_SYMBOL(yldebug_subsystem_restart);
#endif
