...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
#define PANEL_NAME_SIZE	128
char lcd_name[PANEL_NAME_SIZE];
#endif
...
#ifdef CONFIG_YL_KICKOFF_REBOOT_BACKLIGHT_SET
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl);
typedef enum reboot_mode {
	NORMAL_REBOOT_MODE = 0,
	SILENCE_REBOOT_MODE = 1,
	OTABYFORCE_BOOT_MODE = 2,
	LOWMEM_BOOT_MODE = 3,
} REBOOT_MODE;

static ssize_t bl_enable = 0 ;
static REBOOT_MODE  reboot_mode_flag = NORMAL_REBOOT_MODE ;

static ssize_t powerkey_counter = 0;
static int light_on_flag = 0;

#define SAVED_COMMAND_LINE_DATA 1016

static REBOOT_MODE get_reboot_event(void)
{
        char          buff[SAVED_COMMAND_LINE_DATA];
        char*         p;
        char*         q;
        int           saved_command_line_len = strlen(saved_command_line);
        int           len = SAVED_COMMAND_LINE_DATA-1 > saved_command_line_len ? saved_command_line_len:SAVED_COMMAND_LINE_DATA-1;
        REBOOT_MODE      returnval  = NORMAL_REBOOT_MODE;

        strncpy(buff,saved_command_line,len);
        buff[len] = 0;

        p = strstr( buff, " yulong.reboot_event=" );
        if (p == NULL) {
	   returnval = NORMAL_REBOOT_MODE;
            return returnval;
        }
        p += strlen(" yulong.reboot_event=" );
        q  = p;
        while ( *q && *q != ' ' && *q != '\t' )
            q += 1;
	    *q = '\0';
		q=p;
	if(strcmp("silence",p)==0){
		returnval = SILENCE_REBOOT_MODE;
		return returnval;
	}
		p=q;
	if(strcmp("otabyforce",p)==0){
		returnval = OTABYFORCE_BOOT_MODE;
		return returnval;
	}
		p=q;
	if(strcmp("lowmem",p)==0){
		returnval = LOWMEM_BOOT_MODE;
		return returnval;
	}
       return returnval;
 }
...
static void yulong_reboot_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		{
			printk("..func:%s...third backlight..pc trace..\n",__func__);
			mfd->allow_bl_update=0;
		       mfd->unset_bl_level = bkl_lvl;
		}
		else if( mfd->panel_power_state )
		{
			printk("..func:%s...second backlight...pc trace..\n",__func__);
			mfd->bl_level = 120;
...
extern int power_key_status(void);
DEFINE_MUTEX(pwr_state_mutex);

static void msm_powerkey_detect_func(unsigned long data)
{
    ssize_t pwr_key_status = 0;
    struct msm_fb_data_type *mfd = (struct msm_fb_data_type*)data;

	//mutex_lock(&pwr_state_mutex);
	pwr_key_status = power_key_status();
	//mutex_unlock(&pwr_state_mutex);
	 ...
	 if( powerkey_counter >= 6 )
	 {
	       yulong_reboot_set_backlight(mfd,60);
	       light_on_flag = 1 ;
	 }

	 if(!bl_enable)
		mod_timer(&mfd->pw_key_timer, jiffies + HZ/4);
}



static ssize_t  yulong_backlight_enable_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
       bl_enable = 1;
	printk("windy added log :backlight was enabled\n");
    return count;
}

static ssize_t yulong_backlight_enable_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	printk("windy added log :read backlight state bl_enable=%d\n",(int)bl_enable );
       if(reboot_mode_flag == NORMAL_REBOOT_MODE)
		return sprintf(buf, "%d\n",2);
	else
	       return sprintf(buf, "%d\n",light_on_flag);
}

static ssize_t yulong_pwrkey_status_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	mutex_lock(&pwr_state_mutex);
	if(1 == power_key_status()){
		mutex_unlock(&pwr_state_mutex);
		return sprintf(buf, "%d\n",1);
	}
	else{
...
static ssize_t  yulong_pwrkey_status_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
    return count;
}
...
static DEVICE_ATTR(backlight_enable, 0666, yulong_backlight_enable_show, yulong_backlight_enable_store);
static DEVICE_ATTR(pwk_state, 0666, yulong_pwrkey_status_show, yulong_pwrkey_status_store);

static struct attribute *reboot_mode_attributes[] = {
	&dev_attr_backlight_enable.attr,
	&dev_attr_pwk_state.attr,
	NULL
};

static const struct attribute_group reboot_mode_attr_group = {
    .attrs = reboot_mode_attributes,
};
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
/*-----adb read lcd driver ic reg---start------*/
extern u32 mdss_dsi_panel_cmd_read(struct mdss_dsi_ctrl_pdata *ctrl, char cmd0,
		char cmd1, void (*fxn)(int), char *rbuf, int len);
extern struct mdss_dsi_ctrl_pdata *ctrl_this_pdata;
static ssize_t mdss_fb_get_lcdtype(struct device *dev,struct device_attribute *attr,char *buf)
{
	ssize_t ret=0;
	buf[0]=0;
	ret+=snprintf(buf+ret,255-ret,lcd_name);
...
static ssize_t windy_set_reg(struct device *dev,struct device_attribute *attr,const char *buf,size_t count){
	ssize_t ret=0;
	int reg,num;
	int rc=0;
	
	rc=sscanf(buf,"%x %d",&reg,&num);

	if(rc==2){
		reg_to_read=reg;
		num_read=num;
		printk("%s: windy added log reg=%x num=%d\n",__func__,reg_to_read,num_read);
		ret=rc;
	}
	else
		ret=-EINVAL;
...
char reg_content[255];

static ssize_t windy_get_reg(struct device *dev,struct device_attribute *attr,char *buf){

	ssize_t ret=0;
	int i=0;
	char *regdata=&reg_content[0];	/* buffer */
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;

	if((ctrl_this_pdata->panel_data.panel_info.esd_check_enabled) && (mfd->panel_power_state)){
		mdss_dsi_panel_cmd_read(ctrl_this_pdata,reg_to_read,0x00,NULL,regdata,num_read);

		ret+=snprintf(buf+ret,255-ret,"reg %x value=",reg_to_read);
		regdata=ctrl_this_pdata->rx_buf.data;
		for(i=0;i<num_read;i++)
			ret+=snprintf(buf+ret,255-ret,"%x ",*regdata++);
	}else
		ret+=snprintf(buf+ret,255-ret,"Reading reg when lcd off error  or  ESD checking is not enabled.");
...
static ssize_t read_reg_val(struct device *dev,struct device_attribute *attr,char *buf){
	...
	cmd_desc=ctrl_this_pdata->on_cmds.cmds;
			
	for(i=0;i<reg_index;i++)cmd_desc++;

	ret+=snprintf(buf+ret,255-ret,"reg index[%d]=",reg_index);

	for(j=0;j<cmd_desc->dchdr.dlen;j++)
		ret+=snprintf(buf+ret,255-ret," 0x%x",cmd_desc->payload[j]);

	ret+=snprintf(buf+ret,255-ret,"\n");

	printk("windy added log reg index=%d,rc=%d,cmd_desc->dchdr->dlen=%d\n",reg_index,reg_val_count,cmd_desc->dchdr.dlen);

	return ret;
}


static ssize_t write_reg_index(struct device *dev,struct device_attribute *attr,const char *buf,size_t count){
	ssize_t ret=0;
	int index,cnt;
	struct dsi_cmd_desc *cmd_desc=NULL;
	int i,rc=0;
	...
	if(rc==2){
		...
		for(i=0;i<reg_index;i++)
			cmd_desc++;
		reg_val_count=cmd_desc->dchdr.dlen;
		printk("%s: windy added log reg=%x count=%d\n",__func__,reg_index,reg_val_count);
	}
...
static ssize_t read_reg_index(struct device *dev,struct device_attribute *attr,char *buf){
	ssize_t ret=0;
	
	buf[0]=0;
	ret+=snprintf(buf+ret,255-ret,"reg index = %d\n",reg_index);

	return ret;
}

static ssize_t write_reg_val(struct device *dev,struct device_attribute *attr,const char *buf,size_t count){
	...
	rc=sscanf(buf,"%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x",\
		...
		&val[68],&val[69],&val[70],&val[71],\
		&val[72],&val[73],&val[74],&val[75],\
		&val[76],&val[77],&val[78],&val[79],\
		&val[80],&val[81],&val[82],&val[83],\
		&val[84],&val[85],&val[86],&val[87],\
		&val[88],&val[89],&val[90],&val[91],\
		&val[92],&val[93],&val[94],&val[95],\
		&val[96],&val[97],&val[98],&val[99],\
		&val[100],&val[101],&val[102],&val[103],\
		&val[104],&val[105],&val[106],&val[107],\
		&val[108],&val[109],&val[110],&val[111],\
		&val[112],&val[113],&val[114],&val[115],\
		&val[116],&val[117],&val[118],&val[119],\
		&val[120],&val[121],&val[122],&val[123],\
		&val[124],&val[125],&val[126],&val[127],\
		&val[128],&val[129]);

	if(rc==reg_val_count){
		ret=rc;
		printk("%s: windy added log reg=0x%x count=%d rc=%d\n",__func__,reg_index,reg_val_count,rc);

			cmd_desc=ctrl_this_pdata->on_cmds.cmds;
			cmd_count=ctrl_this_pdata->on_cmds.cmd_cnt;
			pdata=ctrl_this_pdata->panel_data;
		for(i=0;i<reg_index;i++)cmd_desc++;

		printk("windy added log reg index=%d,rc=%d,cmd_desc->dchdr->dlen=%d\n",reg_index,reg_val_count,cmd_desc->dchdr.dlen);

		if(reg_val_count>cmd_desc->dchdr.dlen){
			ret=-EINVAL;
			return ret;
		}

		for(j=0;j<reg_val_count;j++)
...
static ssize_t windy_write_timing(struct device *dev,struct device_attribute *attr,const char *buf,size_t count){
	...
	pinfo =&(ctrl_this_pdata->panel_data.panel_info);
...
static ssize_t windy_read_timing(struct device *dev,struct device_attribute *attr,char *buf){
	...
	regulator = pinfo->mipi.dsi_phy_db.timing;
	buf[0]=0;
	ret+=snprintf(buf+ret,255-ret,"timing=");
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
static DEVICE_ATTR(lcd_type, 0664, mdss_fb_get_lcdtype, NULL);
static DEVICE_ATTR(morison_timing,0664,windy_read_timing,windy_write_timing);
static DEVICE_ATTR(windy_reg_index,0600, read_reg_index, write_reg_index);
static DEVICE_ATTR(windy_reg_val,0600,read_reg_val,write_reg_val);
static DEVICE_ATTR(lcd_esd_reg, 0664, windy_get_reg, windy_set_reg);
#endif
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_windy_reg_index.attr,
	&dev_attr_windy_reg_val.attr,
	&dev_attr_lcd_esd_reg.attr,
#endif
	NULL,
};
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	if (!lcd_backlight_registered) {
		backlight_led.brightness = 102;//mfd->panel_info->brightness_max;
	...
	if ((mfd->panel.type == MIPI_CMD_PANEL)||(mfd->panel.type == MIPI_VIDEO_PANEL)){
	  mfd->sdev.name = "power_state";
	   rc = switch_dev_register(&mfd->sdev);
	   if(!rc )
	        switch_set_state(&mfd->sdev, 0);

		reboot_mode_flag=get_reboot_event();

	  printk("reboot_mode_flag=%d\n",reboot_mode_flag);
	  if(reboot_mode_flag!=NORMAL_REBOOT_MODE)
	  {
	        bl_enable = 0;
	  }
	  ...
	  {
		 bl_enable = 1;
		 switch_set_state(&mfd->sdev, 1);
	  }

	  if(sysfs_create_group(&(mfd->sdev.dev->kobj), &reboot_mode_attr_group))
	  {
		 printk("failed to create for reboot silence mode\n");
	   }
	   init_timer(&mfd->pw_key_timer);
	   mfd->pw_key_timer.function = &msm_powerkey_detect_func;
	   mfd->pw_key_timer.data = (unsigned long )mfd;
	   mfd->pw_key_timer.expires = jiffies + HZ/10;

	   if(reboot_mode_flag !=NORMAL_REBOOT_MODE)
	   {
	       printk("timer...silence..reboot..pc trace..\n");
	       add_timer(&mfd->pw_key_timer);
	   }
	}
	#endif
...
#ifdef CONFIG_YL_KICKOFF_REBOOT_BACKLIGHT_SET
            pr_err("backlight set[0]:%d,bl_enable=%d\n", temp, (int)bl_enable);
            if(bl_enable)
#endif
		...
		if ((pdata) && (pdata->set_backlight)) {
			...
			if (bl_notify)
				sysfs_notify(&mfd->fbi->dev->kobj, NULL,
								"pp_bl_event");
#ifdef CONFIG_YL_KICKOFF_REBOOT_BACKLIGHT_SET
            pr_err("backlight set[1]:%d,bl_enable=%d\n", temp, (int)bl_enable);
            if(bl_enable)
#endif
			pdata->set_backlight(pdata, temp);
...
static int mdss_fb_blank_unblank(struct msm_fb_data_type *mfd)
{
	...
	int cur_power_state;
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
#if SHOW_LCD_WAKEUPTIME
	long int time_dif;
	long int yl_poweron_time;
	struct timeval yl_tv2;
	struct timeval yl_tv1;
#endif
#endif
	bool cur_panel_dead;
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
#if SHOW_LCD_WAKEUPTIME
		do_gettimeofday(&yl_tv1);
		yl_poweron_time = yl_tv1.tv_sec*1000000+yl_tv1.tv_usec;
#endif
#endif
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
#if SHOW_LCD_WAKEUPTIME
			do_gettimeofday(&yl_tv2);
			time_dif = yl_tv2.tv_sec*1000000+yl_tv2.tv_usec;
			time_dif -=yl_poweron_time;
			pr_err("*****LCD panel wakeup time = [%ld ms]*****\n",time_dif/1000);
#endif
#endif
