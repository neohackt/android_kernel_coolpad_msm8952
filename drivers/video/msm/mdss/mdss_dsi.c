...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	if(ctrl_pdata->resetdown_after_avdd_avee_enabled){
		mdelay(5);		//added for meeting the time must be more than 4ms. The time is the period between  Reset down and AVDD(+5V) power off
		if (gpio_is_valid(ctrl_pdata->rst_gpio)) {
			gpio_set_value((ctrl_pdata->rst_gpio), 0);
			gpio_free(ctrl_pdata->rst_gpio);
			pr_err("%s: resetdown_after_avdd_avee_enabled is done\n",__func__);
		}
	}

	if((gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio))||(gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio))){
		mdelay(5);		//added for meeting the time must be more than 2ms. The time is the period between AVDD(+5V) power off and VDDA(3.3V) power off
	}

	if(gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio)){
		ret = mdss_dsi_panel_vci_enable(pdata, 0);
		if (ret) {
	...
	if(gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio)){
		...
		mdelay(30);		//added for meeting the time must be more than 120ms. The time is the period interval of low level of 1.8V of lcd from sleepin to sleepout
	}
	//added end
#endif
end:
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if(gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio)){
		ret = mdss_dsi_panel_vci_enable(pdata, 1);
		mdelay(3);
		if (ret)
...
//gongrui@yulong.com set reset pin to low for nt35596 wakeup issue 9-06
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
 ...
 */
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
struct mdss_dsi_ctrl_pdata *ctrl_this_pdata;//ce   read panel
#endif
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//gongrui@yulong.com reset avdd/avee here start 8-12
        if(ctrl_pdata->avdd_avee_enabled){
	        ctrl_pdata->lab = regulator_get(&pdev->dev, "lab_reg");
	        ctrl_pdata->ibb = regulator_get(&pdev->dev, "ibb_reg");
	        regulator_set_voltage(ctrl_pdata->lab , ctrl_pdata->avdd , ctrl_pdata->avdd);
	        regulator_set_voltage(ctrl_pdata->ibb , ctrl_pdata->avee , ctrl_pdata->avee);
        }
//gongrui@yulong.com reset avdd/avee here end 8-12
#endif
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
	//added by lvle for lcd 1.8v enable
	ctrl_pdata->disp_vddi_enable_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-vddi-enable-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio))
		pr_err("%s:%d, vddi enable gpio not specified\n",
						__func__, __LINE__);

	ctrl_pdata->disp_vci_enable_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
	ctrl_this_pdata = ctrl_pdata;//ce  read panel
#endif
