...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//ce-begin
void mdss_dsi_panel_color_cmds_send(struct mdss_dsi_ctrl_pdata *ctrl,
			struct dsi_panel_cmds *pcmds)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo = &(ctrl->panel_data.panel_info);

	pr_debug("william %s: panel_power_state=%d\n", __func__,pinfo->panel_power_state);
	if(pinfo->panel_power_state==0)
		return;
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//gongrui@yulong.com add backlight curve 7-30 start
...
//gongrui@yulong.com add backlight curve 7-30 end
#endif

#ifdef CONFIG_BOARD_CPA9S_9
static char sh1387_pwm[3] = {0xC1, 0x00, 0x00}; /* DTYPE_DCS_LWRITE */
static struct dsi_cmd_desc sh1387_backlight_cmd = {
    {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_pwm)},
    sh1387_pwm
};
static char sh1387_lcm_ib_cc[2] = {0xCC,0x05};	/* DTYPE_DCS_LWRITE */
static char sh1387_lcm_page[6] = {0xF0,0x55,0xAA,0x52,0x08,0x02};	/* DTYPE_DCS_LWRITE */
static struct dsi_cmd_desc sh1387_lcm_ib_cc_cmd = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_lcm_ib_cc)},
	sh1387_lcm_ib_cc
};
static struct dsi_cmd_desc sh1387_lcm_ib_cc_cmds[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_lcm_page)},sh1387_lcm_page},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_lcm_ib_cc)},sh1387_lcm_ib_cc}
};
static struct dsi_cmd_desc sh1387_backlight_cmds[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_lcm_page)},sh1387_lcm_page},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(sh1387_pwm)},sh1387_pwm},
};
static unsigned int stat1=0;
static unsigned int stat1_1=0;
static unsigned int stat2=0;
static unsigned int sh1387_level_backup=0;
static unsigned char sh1387_gain_backup=0;
void mdss_dsi_panel_cmd_send(struct mdss_dsi_ctrl_pdata *ctrl,
			struct dsi_cmd_desc *pcmds,int cmdscnt)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo = &(ctrl->panel_data.panel_info);
...
static void sh1387_lcm_setbacklight(struct mdss_dsi_ctrl_pdata *ctrl, unsigned int level)
{
	unsigned int gain;
	unsigned char gain_hex;
	unsigned int level_back=level;
	unsigned int boundary=64;
	unsigned int factor=16;
	unsigned int rate1=7;
	unsigned int rate2=13;
	unsigned int inigain=10;

	if(level>255) level=255;

	if((level<=255)  && (level>boundary))
	{
		gain=(255-((level*level)/255))*rate1/rate2+inigain;
		gain_hex = gain & 0xFF;
		...
		if(stat1==1)
		{
			if(stat1_1==1)
			{
				if(sh1387_gain_backup != gain_hex){
					sh1387_gain_backup = gain_hex;
					sh1387_lcm_ib_cc[1]=gain_hex;
					//pr_err("%s: stat1=1  level=%d,level_back=%d,gain_hex=%d\n",__func__,level,level_back,gain_hex);
					mdss_dsi_panel_cmd_send(ctrl, &sh1387_lcm_ib_cc_cmd,1);
				}
				stat1=1;
				stat1_1=1;
				stat2=0;
			...
			{
				sh1387_gain_backup = gain_hex;
				sh1387_lcm_page[5]=0x00;
				sh1387_lcm_ib_cc[1]=gain_hex;
				//pr_err("%s: stat1=2  level=%d,level_back=%d,gain_hex=%d\n",__func__,level,level_back,gain_hex);
				mdss_dsi_panel_cmd_send(ctrl, sh1387_lcm_ib_cc_cmds,2);
		...
		else if(stat1==0)
		{
			...
			sh1387_lcm_page[5]=0x02;
			level = 255*4;
			sh1387_pwm[1] = (unsigned char)(level>>8);
			sh1387_pwm[2] = (unsigned char)level;
			//pr_err("%s: stat1=0  sh1387_pwm[1]=0x%x,sh1387_pwm[2]=0x%x,level=%d,level_back=%d,gain_hex=%d\n",__func__, sh1387_pwm[1],sh1387_pwm[2],level,level_back,gain_hex);
			mdss_dsi_panel_cmd_send(ctrl, sh1387_backlight_cmds,2);
	...
	else if((level>0)&&(level<=boundary))
	{
		if(stat2==1)
		{
			if(sh1387_level_backup != level){
				sh1387_level_backup = level;
				level = (level-boundary)*factor+255*4;
				sh1387_pwm[1] = (unsigned char)(level>>8);
				sh1387_pwm[2] = (unsigned char)level;
				//pr_err("%s: stat2=1  sh1387_pwm[1]=0x%x,sh1387_pwm[2]=0x%x,level=%d,level_back=%d\n",__func__, sh1387_pwm[1],sh1387_pwm[2],level,level_back);
				mdss_dsi_panel_cmd_send(ctrl, &sh1387_backlight_cmd,1);
			}
			stat1=0;
			stat1_1=0;
			stat2=1;
		...
		{
			sh1387_level_backup = level;
			gain=(255-((boundary*boundary)/255))*rate1/rate2+inigain;
			gain_hex = gain & 0xFF;
			...
			sh1387_pwm[1] = (unsigned char)(level>>8);
			sh1387_pwm[2] = (unsigned char)level;
			//pr_err("%s: stat2=0  sh1387_pwm[1]=0x%x,sh1387_pwm[2]=0x%x,level=%d,level_back=%d,gain_hex=%d\n",__func__, sh1387_pwm[1],sh1387_pwm[2],level,level_back,gain_hex);
			mdss_dsi_panel_cmd_send(ctrl, sh1387_backlight_cmds,2);
		...
		sh1387_pwm[1] = (unsigned char)(level>>8);
		sh1387_pwm[2] = (unsigned char)level;
		//pr_err("%s: stat3=0  sh1387_pwm[1]=0x%x,sh1387_pwm[2]=0x%x,level=%d,level_back=%d\n",__func__, sh1387_pwm[1],sh1387_pwm[2],level,level_back);
		mdss_dsi_panel_cmd_send(ctrl, sh1387_backlight_cmds,2);
	}
	...
	if(level_back == 0){
		...
		stat1_1=0;
		stat2=0;
		sh1387_level_backup=0;
		sh1387_gain_backup=0;
	}
}
#endif
...
#ifdef CONFIG_BOARD_CPA9S_9
	sh1387_lcm_setbacklight(ctrl,level);
	return;
#endif
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//added by lvle for lcd 1.8v enable
static int mdss_dsi_request_vddi_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio)) {
		rc = gpio_request(ctrl_pdata->disp_vddi_enable_gpio, "panel_vddi");
		if (rc) {
...
int mdss_dsi_panel_vddi_enable(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
			if (gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio)){
				pr_err("%s: disp_vddi_enable_gpio is set to 1.\n",__func__);
				gpio_direction_output((ctrl_pdata->disp_vddi_enable_gpio), 1);
			}
		}

	} else {
		if (gpio_is_valid(ctrl_pdata->disp_vddi_enable_gpio)) {
			pr_err("%s: disp_vddi_enable_gpio is set to 0.\n",__func__);
			gpio_direction_output((ctrl_pdata->disp_vddi_enable_gpio), 0);
...
static int mdss_dsi_request_vci_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int rc = 0;
	if (gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio)) {
		rc = gpio_request(ctrl_pdata->disp_vci_enable_gpio, "panel_vci");
		if (rc) {
			pr_err("request panel vci enable gpio failed,rc=%d\n",rc);
		}
	}
	return rc;
}
int mdss_dsi_panel_vci_enable(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
	int rc = 0;
	...
	if (enable) {
		rc = mdss_dsi_request_vci_gpios(ctrl_pdata);
		if (rc) {
			pr_err("%s: gpio request failed\n",__func__);
			return rc;
		}
		if (!pinfo->cont_splash_enabled) {
			if (gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio)){
				pr_err("%s: disp_vci_enable_gpio is set to 1.\n",__func__);
				gpio_direction_output((ctrl_pdata->disp_vci_enable_gpio), 1);
			}
		}

	} else {
		if (gpio_is_valid(ctrl_pdata->disp_vci_enable_gpio)) {
			pr_err("%s: disp_vci_enable_gpio is set to 0.\n",__func__);
			gpio_direction_output((ctrl_pdata->disp_vci_enable_gpio), 0);
			gpio_free(ctrl_pdata->disp_vci_enable_gpio);
		}
	}
	return rc;
}
...
#ifndef CONFIG_YL_LCD_DISPLAY_COMMON
			if (gpio_is_valid(ctrl_pdata->bklt_en_gpio)) {
				rc = gpio_direction_output(
					ctrl_pdata->bklt_en_gpio, 1);
				if (rc) {
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
		if(!(ctrl_pdata->resetdown_after_avdd_avee_enabled)){
#if defined(CONFIG_BOARD_CPA8_931)
			pr_err("%s:--A8 set Sharp LCD RST High when deep sleep in--\n",__func__);
			gpio_set_value((ctrl_pdata->rst_gpio), 1);
#else
			gpio_set_value((ctrl_pdata->rst_gpio), 0);
#endif
			gpio_free(ctrl_pdata->rst_gpio);
			mdelay(5);		//added for meeting the time must be more than 4ms. The time is the period between  Reset off and AVDD(+5V) power off
		}
#else
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
		gpio_free(ctrl_pdata->rst_gpio);
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//gongrui@yulong.com add backlight mapped value here
int bklt_new_level = 0;
int temp = 0;
#endif
							...
							u32 bl_level)
{
	...
	case BL_WLED:
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
                temp = (bl_level >> 4) & 0xff ;  //add for new level gongrui@yulong.com 7-30
                bklt_new_level = backlight_response_curve[temp];
                bklt_new_level = bklt_new_level << 4;
		led_trigger_event(bl_led_trigger, bklt_new_level);
		if(!backlight_on){
			backlight_on = 1;
			pr_err("%s:--backlight level after optimize is [**%d**%d**]\n",__func__,bklt_new_level,temp);
		}
#else
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
static int mdss_dsi_parse_alive_regs(struct device_node *np,
		struct dsi_panel_alive_reg *alive_regs,  char *cmd_reg)
{
	int i, len,cnt;
	int blen = 0;
	const char *data;
	char *buf, *bp;
	struct panel_reg_check *ppanel_check_regs;
	...
	printk("windy added debug log blen= %d \n", blen);

	bp = buf;
	len = blen;
	cnt = 0;
	while (len > sizeof(char)*2) {
		ppanel_check_regs = (struct panel_reg_check *)bp;

		printk("windy added debug log reg[%d].reg= %0x \n", cnt,ppanel_check_regs->reg);
		printk("windy added debug log reg[%d].num= %0x \n", cnt,ppanel_check_regs->ret_num);

		if (ppanel_check_regs->ret_num > len) {
			pr_err("%s: dtsi reg check reg=%x error, len=%d",
				__func__, ppanel_check_regs->reg, ppanel_check_regs->ret_num);
			return -ENOMEM;
		}

		bp += sizeof(ppanel_check_regs->reg);
		len -= sizeof(ppanel_check_regs->reg);
		bp += sizeof(ppanel_check_regs->ret_num);
		len -= sizeof(ppanel_check_regs->ret_num);
		printk("windy added debug log reg[%d].reg_content= %0x \n", cnt,*bp);
		bp += ppanel_check_regs->ret_num;
		len -= ppanel_check_regs->ret_num;
		cnt++;
	}
	...
	alive_regs->check_regs= kzalloc(cnt * sizeof(struct panel_reg_check),
						GFP_KERNEL);
	if (!alive_regs->check_regs)
		return -ENOMEM;

	alive_regs->reg_num= cnt;

	bp = buf;
	len = blen;
	for (i = 0; i < cnt; i++) {
		ppanel_check_regs = (struct panel_reg_check *)bp;
		alive_regs->check_regs[i].reg= ppanel_check_regs->reg;

		printk("windy added debug log reg alive_regs->check_regs[%d].reg = %0x \n", i,alive_regs->check_regs[i].reg);

		alive_regs->check_regs[i].ret_num= ppanel_check_regs->ret_num;

		printk("windy added debug log reg alive_regs->check_regs[%d].ret_num = %0x \n", i,alive_regs->check_regs[i].ret_num);

		bp += sizeof(ppanel_check_regs->reg);
		len -= sizeof(ppanel_check_regs->reg);
		bp += sizeof(ppanel_check_regs->ret_num);
		len -= sizeof(ppanel_check_regs->ret_num);
		alive_regs->check_regs[i].reg_content= bp;

		printk("windy added debug log reg alive_regs->check_regs[%d].reg_content = %0x \n", i,*(alive_regs->check_regs[i].reg_content));
		bp += ppanel_check_regs->ret_num;
		len -= ppanel_check_regs->ret_num;
	}
...
//gongrui@yulong.com add backlight curve 7-30 start
static int mdss_dsi_parse_backlight_response_curve(struct device_node *np, char *cmd_reg)
{
        int i,k;
        int blen = 0;
        const char *data;
        ...
        data = of_get_property(np, cmd_reg, &blen);
        if (!data) {
                pr_err("--backlight--%s: failed, key=%s\n", __func__, cmd_reg);
                return -ENOMEM;
        }
        buf = kzalloc(sizeof(char) * blen, GFP_KERNEL);
        if (!buf)
        ...
        memcpy(buf, data, blen);
        pr_err("--backlight--%s: blen=%d \n",__func__,blen);
        if (256 != blen) {
                pr_err("--backlight--%s: failed---blen=%d---length is error---\n", __func__, blen);
                return -EINVAL;
        }
        ...
        for(k=0; k<(blen-1); k++)
        {
                pr_debug("--mdss-backlight--*bp=%d--*(bp+1)=%d--k=%d--\n", *bp, *(bp+1), k);
                if(*bp > *(bp+1) )
                {
                        pr_err("--mdss-backlight--%s----backlight-initializtion-error--k=%d-- \n",__func__, k);
                        return -EINVAL;
                }
                bp++;
        }
        ...
        for (i = 0; i < blen; i++) {
                ...
                pr_debug("backlight_response_curve[%d] = %d \n", i, backlight_response_curve[i]);
        }
        pr_debug("--mdss-backlight--%s: exit------ \n",__func__);
        return 0;
}
//gongrui@yulong.com add backlight curve 7-30 end
#endif
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (!pinfo->esd_check_enabled)
		return;
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
	mdss_dsi_parse_alive_regs(np, &ctrl->panel_alive_reg,
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
//gongrui@yulong.com add backlight curve 7-30 start
        mdss_dsi_parse_backlight_response_curve(np, "qcom,panel-backlight-response-curve");
        pr_err("%s: gr mdss_dsi_parse_backlight_response_curve successful!\n", __func__);
//gongrui@yulong.com add backlight curve 7-30 end
//gongrui@yulong.com obtain lab/ibb start 8-12
	ctrl_pdata->avdd_avee_enabled = of_property_read_bool(np,
		"qcom,mdss-dsi-avdd-avee-enabled");
	ctrl_pdata->resetdown_after_avdd_avee_enabled = 0;
	if(ctrl_pdata->avdd_avee_enabled){
		//resetdown_after_avdd_avee_enabled for avoiding the problem that there is a pluse 
		//on the avdd/avee line while the reset power down before avdd/avee.
		ctrl_pdata->resetdown_after_avdd_avee_enabled = of_property_read_bool(np,
		"qcom,mdss-dsi-resetdown-after-avdd-avee-enabled");

		rc = of_property_read_u32(np, "qcom,mdss-dsi-avdd-init-lcd-voltage", &tmp);
		ctrl_pdata->avdd  = (!rc ? tmp : 5000000);

		rc = of_property_read_u32(np, "qcom,mdss-dsi-avee-init-lcd-voltage", &tmp);
		ctrl_pdata->avee = (!rc ? tmp : 5000000);
		pr_err("%s: gr------avdd =%d avee = %d !resetdown_after_avdd_avee_enabled is %d\n",
			__func__,ctrl_pdata->avdd , ctrl_pdata->avee,ctrl_pdata->resetdown_after_avdd_avee_enabled);
	}
//gongrui@yulong.com obtain lab/ibb end 8-12
#endif
...
#ifdef CONFIG_YL_LCD_DISPLAY_COMMON
extern char lcd_name[];		//add read panel name
#endif
