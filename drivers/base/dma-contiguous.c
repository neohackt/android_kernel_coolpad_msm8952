				...
				int depth, void *data)
{
	...
	name = of_get_flat_dt_prop(node, "label", NULL);
         ...
         if(name != NULL && strcmp("pstore_reserve_mem", name) == 0) {
           phys_addr_t end_of_DRAM_addr = memblock_end_of_DRAM();   
 
           base = end_of_DRAM_addr - size;   
 
          pr_info("found pstore mem, end of DRAM addr = %llx, base = %llx\n", end_of_DRAM_addr, base); 
