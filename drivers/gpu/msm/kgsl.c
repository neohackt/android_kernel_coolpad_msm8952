...
static struct kgsl_device *device_t = NULL;
...
static int get_nearest_pwrlevel(struct kgsl_pwrctrl *pwr, unsigned int clock)
{
        ...
        if(abs(pwr->pwrlevels[i].gpu_freq - clock) < abs(pwr->pwrlevels[i+1].gpu_freq - clock)) {
                return i;
        } else {
                return i + 1;
        }
                           ...
                           unsigned long l, void *v)
{
        ...
        if (max_level < 0) {
                printk("error gpu max freq(%d) is set!\n",qmax);
                goto done;
        }
	...
	min_level = get_nearest_pwrlevel(pwr, qmin);
        if (min_level < 0) {
                printk("error gpu min freq(%d) is set!\n",qmin);
                goto done;
        }
