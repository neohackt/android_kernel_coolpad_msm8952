...
#ifdef CONFIG_APP_PROFILE
#ifndef CONFIG_CPU_FREQ_MUTEX_DEL_YL
...
#ifdef CONFIG_APP_PROFILE
extern unsigned int get_nearest_cpufreq(struct cpufreq_policy *policy,unsigned int set_freq);
#endif
				...
				struct cpufreq_policy *new_policy)
{
	...
	qapppri = (unsigned int)pm_qos_request(PM_QOS_APP_START_PRI); //added for start app priority

//cuizhouhua added for start app priority,set to force /dev/cpu_freq_min can bigger than /dev/cpu_freq_max
...
extern int app_profile_disable_flag;
static int cpu_freq_notify(struct notifier_block *b,
			   unsigned long l, void *v)
{
	int cpu;
	if(0 != app_profile_disable_flag) {
		printk("appprofile is disable\n");
		return NOTIFY_OK;
	}
...
static int __init cpufreq_core_init(void)
{
	...
	//BUG_ON(rc);
	if(rc)
                pr_err("failed add CPU_FREQ_MIN notifier,err:%d\n",rc);
	rc = pm_qos_add_notifier(PM_QOS_CPU_FREQ_MAX,
				 &max_freq_notifier);
	//BUG_ON(rc);
	if(rc)
                pr_err("failed add CPU_FREQ_MAX notifier,err:%d\n",rc);
#endif
