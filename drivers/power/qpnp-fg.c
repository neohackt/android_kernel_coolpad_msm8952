...
static struct fg_mem_setting settings[FG_MEM_SETTING_MAX] = {
 ...
 /*100/400/50/450 old*/
	...
	SETTING(HARD_COLD,       0x454,   2,      -30),
	SETTING(HARD_HOT,        0x454,   3,      400),
...
static char *fg_batt_type = "CPLD400lishen_4000mAh";
#else
...
static int batt_capacity;
static int batt_current_now_ma;
static int batt_voltage_now_mv;
static int batt_ocv_mv;
static int batt_temp_c;
static int batt_resistance_mr;
static int batt_id_mr;
static int batt_full;
static int batt_voltage_max_mv;
static int batt_cool_c;
static int batt_warm_c;
...
static int delta_soc_count = 2; //used to print fg_heart_beat
...
static irqreturn_t fg_soc_irq_handler(int irq, void *_chip)
{
	...
	/*if(soc_rt_sts & 0x20){     //if delta soc irq happend, mod 3 to print fg heart beat log
                delta_soc_count += 1;
                delta_soc_count %= 3;
        }*/
        if(delta_soc_count == 2){
                cancel_delayed_work(&chip->heart_beat);
                schedule_delayed_work(&chip->heart_beat, 0);
        }
...
#define HVDCP_MAX_CHARGE_CURRENT 3800
#include <linux/wakelock.h>
static struct wake_lock  fg_heart_beat_wake_lock_timeout;
static struct wake_lock  fg_current_limit_wake_lock_timeout;
static void fg_heart_beat(struct work_struct *work)
{
        struct fg_chip *chip = container_of(work,struct fg_chip,heart_beat.work);
        int unused;

        struct timespec ts;
        struct rtc_time tm;
        struct power_supply * smbcharger_psy;
        union power_supply_propval pval = {0, };

        wake_lock_timeout(&fg_heart_beat_wake_lock_timeout, 2*HZ);
        power_supply_changed(&chip->bms_psy);
        getnstimeofday(&ts);
        ...
        batt_current_now_ma = get_sram_prop_now(chip, FG_DATA_CURRENT);
        batt_voltage_now_mv = get_sram_prop_now(chip, FG_DATA_VOLTAGE);
        batt_ocv_mv = get_sram_prop_now(chip, FG_DATA_OCV);
        batt_voltage_max_mv = chip->batt_max_voltage_uv;
        batt_temp_c = get_sram_prop_now(chip, FG_DATA_BATT_TEMP);
        batt_cool_c = get_prop_jeita_temp(chip, FG_MEM_SOFT_COLD);
        batt_warm_c = get_prop_jeita_temp(chip, FG_MEM_SOFT_HOT);
        batt_resistance_mr = get_sram_prop_now(chip, FG_DATA_BATT_ESR);
        batt_id_mr = get_sram_prop_now(chip, FG_DATA_BATT_ID);
        batt_full = chip->learning_data.learned_cc_uah;
	pr_err("SOC(%3d%%),Ibat(%5dmA),Vbat(%4dmV),OCV(%4dmV),TMP(%2d.%dC),R(%dmR),ID(%dmR),STS(%d),FULL(%d),TIME( %02d-%02d %02d:%02d:%02d.%09lu)\n",
                batt_capacity,batt_current_now_ma/1000,batt_voltage_now_mv/1000,batt_ocv_mv/1000,
                        batt_temp_c/10,batt_temp_c%10,batt_resistance_mr/1000,batt_id_mr/1000,chip->status,batt_full,
                                tm.tm_mon + 1,tm.tm_mday+(tm.tm_hour + 8)/24,(tm.tm_hour + 8)%24,tm.tm_min,tm.tm_sec,ts.tv_nsec);
        if(batt_capacity <= 15)
                schedule_delayed_work(&chip->heart_beat,msecs_to_jiffies(29000));
        else
                schedule_delayed_work(&chip->heart_beat,msecs_to_jiffies(61000));

        batt_current_now_ma = -batt_current_now_ma;
        if(batt_current_now_ma > HVDCP_MAX_CHARGE_CURRENT * 1000){
                wake_lock_timeout(&fg_current_limit_wake_lock_timeout, 4*HZ);
                smbcharger_psy = power_supply_get_by_name("battery");
                pval.intval = (HVDCP_MAX_CHARGE_CURRENT-400) * 1000;
                smbcharger_psy->set_property(smbcharger_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX, &pval);
                printk(KERN_ERR "ljc charge current out of limit %d, decrease to %d", HVDCP_MAX_CHARGE_CURRENT, HVDCP_MAX_CHARGE_CURRENT-400);
        }
}
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	msoc = get_monotonic_soc_raw(chip);
	if (msoc == 0)
		initial_soc = EMPTY_CAPACITY;
	else if (msoc == FULL_SOC_RAW)
		initial_soc = FULL_CAPACITY;
	initial_soc =  DIV_ROUND_CLOSEST((msoc - 1) * (FULL_CAPACITY - 2),
			FULL_SOC_RAW - 2) + 1;
#endif
	...
	wake_lock_init(&fg_heart_beat_wake_lock_timeout, WAKE_LOCK_SUSPEND, "fg_heart_beat_wake_lock_timeout");
	wake_lock_init(&fg_current_limit_wake_lock_timeout, WAKE_LOCK_SUSPEND, "fg_current_limit_wake_lock_timeout");
	INIT_DELAYED_WORK(&chip->heart_beat, fg_heart_beat);
        schedule_delayed_work(&chip->heart_beat,msecs_to_jiffies(1000));
