...
#ifdef CONFIG_YL_FEATURE_CHARGE
//ljc add
       	unsigned char			chg_mode;
        int				call_in;
	int				hvdcp_detect_done;
        /* The framebuffer notifier block */
        struct notifier_block		fb_notif;
        int 				lcd_on;
        struct mutex			user_mode_lock;
        int 				user_scene_mode;
        int 				user_scene_reset;
       	struct work_struct		charge_adjust_work;
//ljc end
#ifdef CONFIG_BOARD_CP3600I
	int hvdcp_normal_fcc_ma;
	int hvdcp_fast_fcc_ma;
	int dcp_normal_fcc_ma;
	int dcp_fast_fcc_ma;
#endif
#endif
};

static struct smbchg_chip *this_chip;
static DECLARE_WAIT_QUEUE_HEAD(wait_hvdcp_detect_done);
#ifdef CONFIG_YL_FEATURE_CHARGE
static bool poweroff_charge=false;
static int chg_user_scene_mode = 0;

#define CHARGE_MODE_DEFAULT     0x00
#define CHARGE_MODE_USB_2_0     0x01
#define CHARGE_MODE_NORMAL      0x02
#define CHARGE_MODE_FAST        0x0F
#define CHARGE_MODE_FORBID      0xFF
...
static int smbchg_parallel_en = 1; //0->1 zkxiong
module_param_named(
	parallel_en, smbchg_parallel_en, int, S_IRUSR | S_IWUSR
);

#ifdef CONFIG_BOARD_CP3600I
static int smbchg_main_chg_fcc_percent = 20;
#else
...
static int is_hvdcp_adapter(struct smbchg_chip *chip)
{
        int rc;
        u8 reg;
        int wait_hvdcp_event;
        wait_hvdcp_event = wait_event_interruptible_timeout(wait_hvdcp_detect_done, chip->hvdcp_detect_done == 1, 4*HZ);
        if (wait_hvdcp_event < 0){
                pr_err("%s: wait for hvdcp detect work error\n", __func__);
        }
        rc = smbchg_read(chip, &reg,chip->usb_chgpth_base + 0x0c, 1);
        if (rc < 0) {
                LOGE("READ HVDCP STATUS FAIL!!\n");
                return -1;
        }
        else
        {
                if ((reg & BIT(0)) && (reg & BIT(2)))
                {
                        LOGE("HVDCP\n");
                        return 1;
                }
		...
		int total_current_ma)
{
	...
	printk(KERN_ERR "ljc in %s total bus current is %d, bq bus current is %d\n", __func__, total_current_ma,new_parallel_cl_ma);
	taper_irq_en(chip, true);
	power_supply_set_present(parallel_psy, true);
	power_supply_set_current_limit(parallel_psy,
...
static struct wake_lock  smbcharge_parallel_usb_wake_lock_timeout;
		...
		int *ret_total_current_ma)
{
	...
	if(chip->call_in || chip->lcd_on == MODE_LCD_ON){
                pr_smb(PR_STATUS, "Lcd on = %d, call in = %d\n", chip->lcd_on, chip->call_in);
                return false;
        }
...
#ifndef CONFIG_BOARD_CP3600I
	if(is_hvdcp_adapter(chip)!=1){
                pr_smb(PR_STATUS, "Not hvdcp chg skip enable\n");
                return false;
        }
#endif
	...
	printk(KERN_ERR "ljc in %s total_current_ma %d current_limit_ma %d parallel_cl_ma %d usb_icl_ma %d\n", __func__,
	...
	kt_since_last_disable = ktime_sub(ktime_get_boottime(),
        ...
        if (chip->parallel.current_max_ma == 0
                ...
                pr_smb(PR_STATUS, "Only been %lld since disable, skipping\n",
                                ktime_to_ms(kt_since_last_disable));
		wake_lock_timeout(&smbcharge_parallel_usb_wake_lock_timeout, 35*HZ);
                cancel_delayed_work(&chip->parallel_en_work);
                schedule_delayed_work(&chip->parallel_en_work,
                        msecs_to_jiffies(PARALLEL_REENABLE_TIMER_MS));
                return false;
        }
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (chip->bms_psy) {
		...
		//check 0x1242 charge en reg
        	rc = smbchg_read(chip, &reg, chip->bat_if_base + CMD_CHG_REG, 1);
		temp = get_effective_result(chip->battchg_suspend_votable);
		if(!!(reg&EN_BAT_CHG_BIT) != temp){
			printk(KERN_ERR "%s bat chg reg not match with suspend votable, try to set the reg\n", __func__);
			smbchg_charging_en(chip, !temp);
		}
...
#ifdef CONFIG_BOARD_CP3600I
#define USBIN_ADAPTER_5V_AND_9V		0x1
#endif
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	printk(KERN_ERR "ljc in %s is_hvdcp_present %d\n", __func__, is_hvdcp_present(chip));
	if (is_hvdcp_present(chip)) {
		printk(KERN_ERR "ljc in here1\n");
		if (!chip->hvdcp3_supported &&
			(chip->wa_flags & SMBCHG_HVDCP_9V_EN_WA)) {
			/* force HVDCP 2.0 */
			printk(KERN_ERR "ljc in here2\n");
			rc = force_9v_hvdcp(chip);
			if (rc)
				pr_err("could not force 9V HVDCP continuing rc=%d\n",
						rc);
		}
		printk(KERN_ERR "ljc in here3\n");
	...
	chip->hvdcp_detect_done = 1;
	wake_up_interruptible(&wait_hvdcp_detect_done);
}
...
#else
	/* allow 5 and 9V chargers */
	rc = smbchg_sec_masked_write(chip,
			chip->usb_chgpth_base + USBIN_CHGR_CFG,
			ADAPTER_ALLOWANCE_MASK, USBIN_ADAPTER_5V_AND_9V);
	if (rc < 0)
		pr_err("Couldn't write usb allowance rc=%d\n", rc);
#endif
...
#else
	pr_smb(PR_MISC, "Allow only %s charger\n",
			insertion ? "5V and 9V" : "9V only");
	rc = smbchg_sec_masked_write(chip,
			chip->usb_chgpth_base + USBIN_CHGR_CFG,
			ADAPTER_ALLOWANCE_MASK,
			insertion ?
			USBIN_ADAPTER_5V_AND_9V : USBIN_ADAPTER_9V);
	if (rc < 0) {
		pr_err("Couldn't write usb allowance rc=%d\n", rc);
		return rc;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = get_prop_batt_voltage_now(chip);
                res = get_prop_batt_current_now(chip) / 7; //edit 7 -> 10
                if(res < 0)
                       res = 0;
                if(res > 80000)  //edit 80000 -> 50000
                       res = 80000; //edit 80000 -> 50000
                val->intval += res;
		break;
...
static irqreturn_t batt_warm_handler(int irq, void *_chip)
{
	...
	if(((ocv_uv / 1000) > 4100) && (chip->batt_warm)) {
		pr_err("Battery warm and OCV more than 4100mV, stop charging\n");
		smbchg_charging_en(chip, false);
		smbchg_parallel_usb_disable(chip);
		smbchg_en = false;

	} else {
		pr_err("Battery not warm or OCV below 4100mV, enable smb charging\n");
		if(!smbchg_en) {
...
#ifdef CONFIG_BOARD_CP3600I
	of_property_read_u32(chip->spmi->dev.of_node, "yl,hvdcp-normal-fcc-ma",
			&chip->hvdcp_normal_fcc_ma);
	if (chip->hvdcp_normal_fcc_ma == -EINVAL)
		chip->hvdcp_normal_fcc_ma = DEFAULT_FCC_MA;
	pr_err("chip->hvdcp_normal_fcc_ma = %d\n", chip->hvdcp_normal_fcc_ma);
	of_property_read_u32(chip->spmi->dev.of_node, "yl,hvdcp-fast-fcc-ma",
			&chip->hvdcp_fast_fcc_ma);
	if (chip->hvdcp_fast_fcc_ma == -EINVAL)
		chip->hvdcp_fast_fcc_ma = DEFAULT_FCC_MA;
	pr_err("chip->hvdcp_fast_fcc_ma = %d\n", chip->hvdcp_fast_fcc_ma);
	of_property_read_u32(chip->spmi->dev.of_node, "yl,dcp-normal-fcc-ma",
			&chip->dcp_normal_fcc_ma);
	if (chip->dcp_normal_fcc_ma == -EINVAL)
		chip->dcp_normal_fcc_ma = DEFAULT_FCC_MA;
	pr_err("chip->dcp_normal_fcc_ma = %d\n", chip->dcp_normal_fcc_ma);
	of_property_read_u32(chip->spmi->dev.of_node, "yl,dcp-fast-fcc-ma",
			&chip->dcp_fast_fcc_ma);
	if (chip->dcp_fast_fcc_ma == -EINVAL)
		chip->dcp_fast_fcc_ma = DEFAULT_FCC_MA;
	pr_err("chip->dcp_fast_fcc_ma = %d\n", chip->dcp_fast_fcc_ma);
#endif
...
static ssize_t dump_regs_show(struct class *cls , struct class_attribute *attr, char *buf)
{
	u16 addr;
	struct smbchg_chip *chip = this_chip;
...
static CLASS_ATTR(dump_regs, 0664, dump_regs_show,NULL);
...
#ifdef CONFIG_YL_FEATURE_CHARGE
static int __init PoweroffCharge_info(char *bi)
{
        unsigned long type;
        type = simple_strtoul(bi,NULL,16);
        if(type == 5)
           poweroff_charge = true;
        pr_err("poweroff_charge=%d ,type=%lu\n",poweroff_charge,type);
        return 1;
}
__setup("yulong.boot_reason=",PoweroffCharge_info);

static int get_user_modes(struct smbchg_chip *chip){

        mutex_lock(&chip->user_mode_lock);
        if(chip->lcd_on == MODE_LCD_ON){
                 chg_user_scene_mode =
                        chg_user_scene_mode | MODE_LCD ;
        }else if(chip->lcd_on == MODE_LCD_OFF){
                 chg_user_scene_mode =
                        chg_user_scene_mode & (~MODE_LCD);
        }else
                LOGD("@@@ Invalid LCD Command! \n");

        if(chip->call_in == 1){
                chg_user_scene_mode =
                        chg_user_scene_mode | MODE_CALL ;
        }else if(chip->call_in == 0){
                 chg_user_scene_mode =
                        chg_user_scene_mode & (~MODE_CALL);
        }else
                LOGD("@@@ Invalid Call Command! \n");

       if(chip->user_scene_reset == MODE_RESET)
                chg_user_scene_mode = 0;
       mutex_unlock(&chip->user_mode_lock);
       return chg_user_scene_mode;
}

static ssize_t user_scene_mode_show(struct class *cls , struct class_attribute *attr, char *buf)
{
       LOGD("Show user_scene_mode:%d \n", chg_user_scene_mode);
        return sprintf(buf,"%d\n", (this_chip != NULL)?this_chip->user_scene_mode:chg_user_scene_mode);
}

static ssize_t user_scene_mode_store(struct class *class, struct class_attribute *attr,
        const char *buf, size_t count)
{
        ...
        if(!(bt&1))
                return count;
        call_in=(bt>>1)&1;
        if (this_chip->call_in ^ call_in) {
                this_chip->call_in = call_in;
                schedule_work(&this_chip->charge_adjust_work);
       }

       LOGD("@@@ Store Call In:%d \n", this_chip->call_in);
       return count;
}

static CLASS_ATTR(user_scene_mode, 0664, user_scene_mode_show,user_scene_mode_store);

static int smbchg_fb_notifier_callback(struct notifier_block *self,
                                 unsigned long event, void *data)
{
        ...
        //Screen On
                               chip->lcd_on = MODE_LCD_ON;
                       }
                        else if (*blank == FB_BLANK_POWERDOWN){
        //Screen Off
                                chip->lcd_on = MODE_LCD_OFF;
                        }
                       schedule_work(&chip->charge_adjust_work);
                       LOGD("@@@ LCD On:%d \n",chip->lcd_on % 2);
                }
        }
        return 0;
...
static void
qpnp_smbchg_print_log(struct smbchg_chip *chip){
	unsigned long   current_time;
	get_current_time(&current_time);
        LOGD("bat_v:%d,batt_i:%d,batt_capcity:%d,batt_temp:%d,batt_stat:%d\n",
...
static int calc_user_mode_limited_level(struct smbchg_chip *chip,int temp){
/* Default to Fast Charge
*/
	int level= 0;
	bool is_hvdcp_chg = (is_hvdcp_adapter(chip)==1) ? 1 : 0;
	bool QuickChargeMode;

	QuickChargeMode = (chip->chg_mode == CHARGE_MODE_FAST? true : false);
        if(level == 0 && is_hvdcp_chg)
                level = (QuickChargeMode ? 0 : 3); //quick charge:2100, normal charge:1200
        LOGD("@@@ User scene limit Mode levl:%d, user scene mode:%d \n",level,chip->user_scene_mode);
        return level;
}

static void smbchg_charge_adjust_work(struct work_struct *work)
{
        struct smbchg_chip *chip = container_of(work,
                                struct smbchg_chip,charge_adjust_work);
	...
	int rc;
        ...
        qpnp_smbchg_print_log(chip);

	QuickChargeMode = (chip->chg_mode == CHARGE_MODE_FAST? true:false);
	chip->user_scene_mode = get_user_modes(chip);
        user_mode_lvl = calc_user_mode_limited_level(chip,pmic_temp);
        lvl_sel = user_mode_lvl;
        LOGD("@@@ final lvl_sel:%d, old therm_lvl_sel:%d,total thermal_levels:%d \n",lvl_sel,chip->therm_lvl_sel,chip->thermal_levels);
        LOGD("@@@ Current Limit to %d \n" , chip->thermal_mitigation[lvl_sel]);
        smbchg_system_temp_level_set(chip,lvl_sel);
        if(chip->call_in || chip->lcd_on == MODE_LCD_ON)
#ifdef CONFIG_YL_XIANWANG_LCD_ON_CHARGE_CURRENT
                smbchg_set_fastchg_current_raw(chip, 300);
        else
                smbchg_set_fastchg_current_raw(chip, 1900);
        return;
#else
#ifdef CONFIG_BOARD_CP3600I
	{
		smbchg_parallel_usb_disable(chip);
		smbchg_set_fastchg_current_raw(chip, 600);
	}
#else
                smbchg_set_fastchg_current_raw(chip, 600);
#endif
...
#ifdef CONFIG_BOARD_CP3600I
	{
		if(is_hvdcp_adapter(chip)==1){
			if(QuickChargeMode) {
				limited_fcc_ma = chip->hvdcp_fast_fcc_ma;  //3700mA
				vote(chip->fcc_votable, ESR_PULSE_FCC_VOTER, false, limited_fcc_ma);
				vote(chip->fcc_votable, BATT_TYPE_FCC_VOTER, false, limited_fcc_ma);
				vote(chip->fcc_votable, RESTRICTED_CHG_FCC_VOTER, false, limited_fcc_ma);
			}else{
				limited_fcc_ma = chip->hvdcp_normal_fcc_ma;  //2100mA
			}
		}else {
			if(QuickChargeMode){
				limited_fcc_ma = chip->dcp_fast_fcc_ma;  //2000mA
			}else {
				limited_fcc_ma = chip->dcp_normal_fcc_ma;  //1000mA
				smbchg_parallel_usb_disable(chip);
				smbchg_set_fastchg_current_raw(chip, limited_fcc_ma);
			}
		}

		pr_err("limited_fcc_ma = %d\n", limited_fcc_ma);
		rc = vote(chip->fcc_votable, CHG_MODE_FCC_VOTER, true, limited_fcc_ma);
		if (rc < 0) {
			pr_err("Couldn't vote en rc %d\n", rc);
			return;
		}
...
static ssize_t charge_mode_show(struct class *class, struct class_attribute *attr,char *buf)
{
       if (!this_chip) {
                pr_err(" NULL POINTER\n");
                return -EINVAL;
...
static ssize_t charge_mode_store(struct class *class, struct class_attribute *attr,const char *buf, size_t count)
{
	unsigned char data = 0x00;
        ...
        if((data == 0x00)||(data == 0x01)||(data == 0x02)||(data == 0x0F)||(data == 0xFF))
        {
                this_chip->chg_mode = (unsigned char) data;
               	schedule_work(&this_chip->charge_adjust_work);
                return  count;
        }
        else
...
static CLASS_ATTR(charge_mode, 0664, charge_mode_show, charge_mode_store);
#endif
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	init_completion(&chip->usbin_uv_lowered);
	init_completion(&chip->usbin_uv_raised);
	wake_lock_init(&smbcharge_parallel_usb_wake_lock_timeout, WAKE_LOCK_SUSPEND, "smbcharge_parallel_usb_wake_lock_timeout");
...
#ifdef CONFIG_YL_FEATURE_CHARGE
       	chip->call_in = 0;
        chip->chg_mode = CHARGE_MODE_FAST;
#endif
...
#ifdef CONFIG_YL_FEATURE_CHARGE
        mutex_init(&chip->user_mode_lock);
#endif
	...
	if (chip->cfg_chg_led_support &&
      ...
      if (!chip->cfg_chg_led_support &&
			chip->schg_version == QPNP_SCHG_LITE)
     smbchg_sec_masked_write(chip,chip->bat_if_base + CMD_CHG_LED_REG, LED_BLINKING_CFG_MASK, 0);
	...
	if (NULL!=power_supply_class)
        {
                rc = class_create_file(power_supply_class, &class_attr_dump_regs);
                if (rc)
                {
                        pr_err("cannot create sys/power_supply/dump_regs sysfs file");
                }
        }
#ifdef CONFIG_YL_FEATURE_CHARGE
        rc = class_create_file(power_supply_class, (const struct class_attribute *)&class_attr_user_scene_mode.attr);
        if(rc < 0)
        {
                printk(KERN_ERR"failed to create class_attr_user_scene_mode!\n");
        }

#endif
#ifdef CONFIG_YL_FEATURE_CHARGE
	INIT_WORK(&chip->charge_adjust_work,smbchg_charge_adjust_work);
	chip->fb_notif.notifier_call = smbchg_fb_notifier_callback;
	rc = fb_register_client(&chip->fb_notif);
	if (rc)
		LOGD("@@@ Unable to register fb_notifier: %d\n",rc);
	//msm_register_hvdcp_ext_notification((hvdcp_ext_notify)get_hvdcp_current);
        if (NULL!=power_supply_class)
        {
                rc = class_create_file(power_supply_class, &class_attr_charge_mode);
                if (rc)
                {
                        pr_err("cannot create sys/power_supply/charge_mode sysfs file");
                }
        }
#endif
