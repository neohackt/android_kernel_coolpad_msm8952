/*******************************************************************************
 * Filename:
 * ---------
 *  mt6605.c
 *
 * Project:
 * --------
 *
 * Description:
 * ------------
 *
 * Author:
 * -------
 *  LiangChi Huang, ext 25609, LiangChi.Huang@mediatek.com, 2012-08-09
 * 
 *******************************************************************************/
/***************************************************************************** 
 * Include
 *****************************************************************************/ 
#include <linux/kernel.h>
#include <linux/module.h>  
#include <linux/fs.h>
#include <linux/slab.h>
#include <linux/init.h>
#include <linux/list.h>
#include <linux/i2c.h>
#include <linux/irq.h>
#include <linux/jiffies.h>
#include <linux/uaccess.h>
#include <linux/delay.h>
#include <linux/interrupt.h>
#include <linux/io.h>
#include <linux/platform_device.h>
#include <linux/gpio.h>
#include <linux/miscdevice.h>
#include <linux/spinlock.h>
#include <linux/dma-mapping.h>
    
/*#include <cust_gpio_usage.h>
#include <cust_eint.h>

#include <mach/mt_gpio.h>
#include <mach/eint.h>*/

#include <linux/interrupt.h>

#include <linux/of_gpio.h>
#include <linux/of.h>
#include <linux/of_irq.h>
#include <linux/irqreturn.h>
#include <mach/gpiomux.h>


/***************************************************************************** 
 * Define
 *****************************************************************************/

#define NFC_I2C_BUSNUM  1  // for 6572
#define I2C_ID_NAME "mtk-nfc"
#define MAX_BUFFER_SIZE	255
#define NFC_CLIENT_TIMING 400  //I2C speed
#define GPIO_DIR_IN 0
#define GPIO_DIR_OUT 1

#undef LOGD 
#define LOGD(...)  printk(__VA_ARGS__)

/***************************************************************************** 
 * GLobal Variable
 *****************************************************************************/ 
struct mt6605_dev *mt6605_dev_ptr = NULL;
//static struct i2c_board_info __initdata nfc_board_info = {I2C_BOARD_INFO(I2C_ID_NAME, 0x28)};
//For DMA
//static char *I2CDMAWriteBuf = NULL;
static unsigned char I2CDMAWriteBuf_pa[512];// = NULL;
//static char *I2CDMAReadBuf = NULL;
static unsigned char I2CDMAReadBuf_pa[512];// = NULL;
static int fgNfcChip = 0;
int forceExitBlockingRead = 0;
//static u32 nfc_irq; //DEVICE_TREE

/***************************************************************************** 
 * Function Prototype
 *****************************************************************************/ 
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int mt6605_remove(struct i2c_client *client);
//static int mt6605_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);
static int mt6605_dev_open(struct inode *inode, struct file *filp);
static long mt6605_dev_unlocked_ioctl(struct file *filp,unsigned int cmd, unsigned long arg);
static ssize_t mt6605_dev_read(struct file *filp, char __user *buf,size_t count, loff_t *offset);
static ssize_t mt6605_dev_write(struct file *filp, const char __user *buf,size_t count, loff_t *offset);
static irqreturn_t mt6605_dev_irq_handler(int irq, void *dev_id);
//static void mt6605_disable_irq(struct mt6605_dev *mt6605_dev);
//extern void mt_eint_soft_set(unsigned int eint_num);
static void reg_gpio_pin(struct device *dev);
static int get_gpio_dir(int gpio_num);

/***************************************************************************** 
 * Data Structure
 *****************************************************************************/

 struct mt6605_dev	{
	wait_queue_head_t	read_wq;
	struct mutex		read_mutex;
	struct i2c_client	*client;
	struct miscdevice	mt6605_device;
	unsigned int 		ven_gpio;
	unsigned int 		sysrstb_gpio;
	unsigned int		irq_gpio;  // Chip inform Host
	unsigned int        eint_gpio; // Host inform Chip
	unsigned int        nres_gpio; //NRES , AS39230 RST pin
	bool			    irq_enabled;
	
	struct mutex		irq_enabled_lock;
	//spinlock_t		irq_enabled_lock;
};

static struct of_device_id msm_match_table[] = {
	{.compatible = "mtk,mtk-nfc"},
	{}
};

struct mt6605_i2c_platform_data {
    unsigned int irq_gpio; // Chip inform Host
    unsigned int ven_gpio;
    unsigned int sysrstb_gpio;
    unsigned int eint_gpio; // Host inform Chip
    unsigned int nres_gpio; //NRES , AS39230 RST pin
};


static const struct i2c_device_id mt6605_id[] = {
	{ I2C_ID_NAME, 0 },
	{ }
};

//#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36))
//static struct i2c_client_address_data addr_data = { .forces = forces,};
//#endif
static struct i2c_driver mt6605_dev_driver = {
    .id_table	= mt6605_id,
    .probe		= mt6605_probe,
    .remove		= mt6605_remove,
    //.detect		= mt6605_detect,
    .driver		= {
        .owner	= THIS_MODULE,
        .name	= "mt6605",
        .of_match_table = msm_match_table,
        
    },
};

struct mt6605_i2c_platform_data mt6605_platform_data;
/*static struct mt6605_i2c_platform_data mt6605_platform_data = {
    .irq_gpio       = GPIO_IRQ_NFC_PIN,
    .ven_gpio       = GPIO_NFC_VENB_PIN,
    .sysrstb_gpio   = GPIO_NFC_RST_PIN,
    .eint_gpio      = GPIO_NFC_EINT_PIN,
    .nres_gpio      = GPIO_NFC_FIRM_PIN,  //  NRES , AS39230 RST pin 
};*/


static const struct file_operations mt6605_dev_fops = {
    .owner	= THIS_MODULE,
    .llseek	= no_llseek,
    .read	= mt6605_dev_read,
    .write	= mt6605_dev_write,
    .open	= mt6605_dev_open,
    .unlocked_ioctl = mt6605_dev_unlocked_ioctl,
};

typedef enum 
{
    MTK_NFC_GPIO_EN_B = 0x0,
    MTK_NFC_GPIO_SYSRST_B,
    MTK_NFC_GPIO_EINT,
    MTK_NFC_GPIO_IRQ,
    MTK_NFC_GPIO_MAX_NUM
} MTK_NFC_GPIO_E;

typedef enum 
{
    MTK_NFC_IOCTL_READ = 0x0,
    MTK_NFC_IOCTL_WRITE,
    MTK_NFC_IOCTL_MAX_NUM
} MTK_NFC_IOCTL;

typedef enum 
{
    MTK_NFC_PULL_LOW  = 0x0,
    MTK_NFC_PULL_HIGH,
    MTK_NFC_PULL_INVALID,
} MTK_NFC_PULL_E;

//static DECLARE_WAIT_QUEUE_HEAD(_gmt6605_dev.read_wq);


/***************************************************************************** 
 * Extern Area
 *****************************************************************************/ 
 

/***************************************************************************** 
 * Function
 *****************************************************************************/ 


static void mt6605_disable_irq(struct mt6605_dev *mt6605_dev)
{
    //unsigned long flags;
    //mutex_lock(&mt6605_dev->irq_enabled_lock);
    LOGD(KERN_DEBUG "%s : irq %d, enable=%d \n", __func__, mt6605_dev->client->irq, mt6605_dev->irq_enabled);
    if (mt6605_dev->irq_enabled) 
    {
    	//disable_irq(mt6605_dev->client->irq);  //DEVICE_TREE
    	disable_irq_nosync(mt6605_dev_ptr->client->irq);
        mt6605_dev->irq_enabled = false;
        LOGD(KERN_DEBUG "%s : irq %d disabled  <-----\n", __func__, mt6605_dev->client->irq);
    }  
    //mutex_unlock(&mt6605_dev->irq_enabled_lock);
  
}


static void mt6605_enable_irq(struct mt6605_dev *mt6605_dev)
{
    //unsigned long flags;
    //mutex_lock(&mt6605_dev->irq_enabled_lock);
    LOGD(KERN_DEBUG "%s : irq %d, enable=%d \n", __func__, mt6605_dev->client->irq, mt6605_dev->irq_enabled);
    if (!mt6605_dev->irq_enabled) 
    {
    		//mt_eint_mask(mt6605_dev->client->irq);
    	enable_irq(mt6605_dev->client->irq);   //DEVICE_TREE
        mt6605_dev->irq_enabled = true;
		 LOGD(KERN_DEBUG "%s : irq %d enabled --->n", __func__, mt6605_dev->client->irq);
    }  
    //mutex_unlock(&mt6605_dev->irq_enabled_lock);
  
}



struct mt6605_dev _gmt6605_dev;
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 
    
    int ret=0;
    int irqn = 0;
    struct mt6605_i2c_platform_data *platform_data;
    //struct mt6605_dev *mt6605_dev;
    //LOGD(KERN_DEBUG "%s : nfc probe enter\n", __func__);
    //return 0;
    
    LOGD(KERN_DEBUG "jiangban 0322-10:31 mt6605_dev_probe\n");
    reg_gpio_pin(&client->dev);
    platform_data = &mt6605_platform_data;

    if (platform_data == NULL) {
    	//pr_err("%s : nfc probe fail\n", __func__);
    	LOGD(KERN_DEBUG "%s : nfc probe fail\n", __func__);
    	return  -ENODEV;
    }

    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        //pr_err("%s : need I2C_FUNC_I2C\n", __func__);
        LOGD(KERN_DEBUG "%s : need I2C_FUNC_I2C\n", __func__);
        return  -ENODEV;
    }
	

    /* NFC_IRQ */
    //mt_set_gpio_mode( platform_data->irq_gpio        , GPIO_IRQ_NFC_PIN_M_EINT); // Set gpio mode
    //mt_set_gpio_dir( platform_data->irq_gpio         , GPIO_DIR_IN);  // Set Dir =  input
    //mt_set_gpio_pull_enable( platform_data->irq_gpio , GPIO_PULL_ENABLE);
    //mt_set_gpio_pull_select( platform_data->irq_gpio , GPIO_PULL_DOWN);
    
    if (gpio_is_valid(platform_data->irq_gpio)) {
    
		
		ret = gpio_request(platform_data->irq_gpio, "nfc_irq_gpio");
		
		if (ret) {
			LOGD(KERN_DEBUG "unable to request gpio [%d]\n",platform_data->irq_gpio);
			goto err_irq;
		}
		
		ret = gpio_direction_input(platform_data->irq_gpio);
		if (ret) {
			LOGD(KERN_DEBUG "unable to set direction for gpio [%d]\n",platform_data->irq_gpio);
			goto err_irq;
		}


	//	gpio_to_irq(0);
		irqn = gpio_to_irq(platform_data->irq_gpio);
		if (irqn < 0) {
			ret = irqn;
			goto err_irq;
		}
		client->irq = irqn;
	  } else {
		//pr_err("irq gpio not provided\n");
		LOGD(KERN_DEBUG "irq gpio not provided\n");
		goto err_free_dev;
	  }
	

    printk(KERN_DEBUG " mt6605_dev_probe irq 1 gpio ok\n");
//        return  -ENODEV;
    /* VENB_gpio */
    //mt_set_gpio_mode( platform_data->ven_gpio    , GPIO_NFC_VENB_PIN_M_GPIO); // Set gpio mode
    //mt_set_gpio_dir( platform_data->ven_gpio     , GPIO_DIR_OUT); // Set Dir =  output
    //mt_set_gpio_out( platform_data->ven_gpio     , GPIO_OUT_ONE);	// Set output High
    //usleep_range( 900, 1000); //debug
    
    if (gpio_is_valid(platform_data->ven_gpio)) {
				ret = gpio_request(platform_data->ven_gpio, "nfc_ven_gpio");
				if (ret) {
					//pr_err("unable to request gpio [%d]\n",platform_data->ven_gpio);
					LOGD(KERN_DEBUG "unable to request gpio [%d]\n",platform_data->ven_gpio);
					goto err_ven_gpio;
				}
				ret = gpio_direction_output(platform_data->ven_gpio,1);
				if (ret) {
					//pr_err("unable to set direction for gpio [%d]\n",platform_data->ven_gpio);
					LOGD(KERN_DEBUG "unable to set direction for gpio [%d]\n",platform_data->ven_gpio);
					goto err_ven_gpio;
				}
		} else {
			//pr_err("ven gpio not provided\n");
			LOGD(KERN_DEBUG "ven gpio not provided\n");
			//goto err_clk;
		}
//		usleep_range( 900, 1000); //debug

    printk(KERN_DEBUG " mt6605_dev_probe ven 2 gpio ok\n");
    /* RST_gpio */
    //mt_set_gpio_mode( platform_data->sysrstb_gpio, GPIO_NFC_RST_PIN_M_GPIO); // Set gpio mode
    //mt_set_gpio_dir( platform_data->sysrstb_gpio , GPIO_DIR_OUT);	// Set Dir =  output
	  //mt_set_gpio_out( platform_data->sysrstb_gpio , GPIO_OUT_ONE); // Set output High
    //usleep_range( 900, 1000); //debug
    
    if (gpio_is_valid(platform_data->sysrstb_gpio)) {
			ret = gpio_request(platform_data->sysrstb_gpio, "nfc_reset_gpio");
			if (ret) {
				//dev_err(&client->dev,
				LOGD(KERN_DEBUG "NFC: unable to request gpio [%d]\n",
					platform_data->sysrstb_gpio);
				goto err_free_dev;
			}
			ret = gpio_direction_output(platform_data->sysrstb_gpio, 1);
			if (ret) {
				//dev_err(&client->dev,
				LOGD(KERN_DEBUG "NFC: unable to set direction for gpio [%d]\n",
						platform_data->sysrstb_gpio);
				goto err_dis_gpio;
			}
		} else {
			//dev_err(&client->dev, "dis gpio not provided\n");
			LOGD(KERN_DEBUG  "dis gpio not provided\n");
			goto err_irq;
		}
//		usleep_range( 900, 1000); //debugi

    printk(KERN_DEBUG " mt6605_dev_probe 3 sysrstb gpio ok\n");

    /* EINT_gpio */
    //mt_set_gpio_mode( platform_data->eint_gpio   , GPIO_NFC_EINT_PIN_M_GPIO); // Set gpio mode
    //mt_set_gpio_dir( platform_data->eint_gpio    , GPIO_DIR_OUT);  // Set Dir =  output 
    //mt_set_gpio_out( platform_data->eint_gpio    , GPIO_OUT_ZERO); //Set output Low
    
    if (gpio_is_valid(platform_data->eint_gpio)) {
			ret = gpio_request(platform_data->eint_gpio, "nfc_reset_gpio");
			if (ret) {
				//dev_err(&client->dev,
				LOGD(KERN_DEBUG "NFC: unable to request gpio [%d]\n",
					platform_data->eint_gpio);
				goto err_free_dev;
			}
			ret = gpio_direction_output(platform_data->eint_gpio, 0);
			if (ret) {
				//dev_err(&client->dev,
				LOGD(KERN_DEBUG	"NFC: unable to set direction for gpio [%d]\n",
						platform_data->eint_gpio);
				goto err_eint_gpio;
			}
		} else {
			//dev_err(&client->dev, "dis gpio not provided\n");
			LOGD(KERN_DEBUG "dis gpio not provided\n");
			goto err_irq;
		}

	
    printk(KERN_DEBUG " mt6605_dev_probe 4 eint gpio ok\n");
#if 0 // following WCP2's prolicy "Not allow to use non-cached locks"
    mt6605_dev = kzalloc(sizeof(*mt6605_dev), GFP_KERNEL);
    if (mt6605_dev == NULL) 
    {
        //Thomas_tmp pr_err("pn544_dev: failed to allocate memory for module data\n");
        ret = -ENOMEM;
        goto err_exit;
    }
#endif
	
    _gmt6605_dev.irq_gpio       = platform_data->irq_gpio;
    _gmt6605_dev.ven_gpio       = platform_data->ven_gpio;
    _gmt6605_dev.sysrstb_gpio   = platform_data->sysrstb_gpio;
    _gmt6605_dev.eint_gpio      = platform_data->eint_gpio;
    _gmt6605_dev.nres_gpio      = platform_data->nres_gpio; // AS39230 RST pin
    _gmt6605_dev.client         = client;

    /* init mutex and queues */
    init_waitqueue_head(&_gmt6605_dev.read_wq);
    mutex_init(&_gmt6605_dev.read_mutex);
    //spin_lock_init(&mt6605_dev->irq_enabled_lock);
    mutex_init(&_gmt6605_dev.irq_enabled_lock);
	_gmt6605_dev.irq_enabled = false;

    _gmt6605_dev.mt6605_device.minor    = MISC_DYNAMIC_MINOR;
    _gmt6605_dev.mt6605_device.name     = "mt6605";
    _gmt6605_dev.mt6605_device.fops     = &mt6605_dev_fops;

    ret = misc_register(&_gmt6605_dev.mt6605_device); // create device node
    if (ret) {
        //Thomas_tmp pr_err("%s : misc_register failed\n", __FILE__);
        
        goto err_misc_register;
    }

    //I2CDMAWriteBuf = (char *)dma_alloc_coherent(NULL, MAX_BUFFER_SIZE, (dma_addr_t *)&I2CDMAWriteBuf_pa, GFP_KERNEL);
    //if (I2CDMAWriteBuf == NULL) {
    //	//Thomas_tmp pr_err("%s : failed to allocate dma buffer\n", __func__);
    //	goto err_request_irq_failed;
    //}

    //I2CDMAReadBuf = (char *)dma_alloc_coherent(NULL, MAX_BUFFER_SIZE, (dma_addr_t *)&I2CDMAReadBuf_pa, GFP_KERNEL);
    //if (I2CDMAReadBuf == NULL) {
    	//Thomas_tmp pr_err("%s : failed to allocate dma buffer\n", __func__);
    //	goto err_request_irq_failed;
    //}
//    LOGD(KERN_DEBUG "%s :I2CDMAWriteBuf_pa %d, I2CDMAReadBuf_pa,%d\n", __func__, I2CDMAWriteBuf_pa, I2CDMAReadBuf_pa);

    /* request irq.  the irq is set whenever the chip has data available
        *  for reading.  it is cleared when all data has been read.
        */
    //client->irq = gpio_to_irq(platform_data->eint_gpio);

    LOGD(KERN_DEBUG "%s : requesting IRQ %d\n", __func__, client->irq);
    LOGD(KERN_DEBUG "mt6605_Prob2\n");	
    
    //mt65xx_eint_set_sens(CUST_EINT_IRQ_NFC_NUM, CUST_EINT_IRQ_NFC_SENSITIVE);
    //mt65xx_eint_set_hw_debounce(CUST_EINT_IRQ_NFC_NUM,
    //                            CUST_EINT_IRQ_NFC_DEBOUNCE_CN);   
    //mt65xx_eint_registration(CUST_EINT_IRQ_NFC_NUM, 
    //                         CUST_EINT_IRQ_NFC_DEBOUNCE_CN, 
    //                         CUST_EINT_POLARITY_HIGH,
    //                         mt6605_dev_irq_handler, 0);
	//nfc_irq = platform_data->irq_gpio;
	//LOGD(KERN_DEBUG "jiangban nfc_irq=%d\n",nfc_irq);	  


#if 0
    {   //DEVICE_TREE
        struct device_node *node;
        u32 ints[2];
        
        node = of_find_compatible_node (NULL, NULL, "mediatek, IRQ_NFC-eint");

        if(node)
        {
            of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
            //mt_gpio_set_debounce(ints[0], ints[1]);
            
            nfc_irq = irq_of_parse_and_map(node, 0);
            ret = request_irq(nfc_irq, mt6605_dev_irq_handler, IRQF_TRIGGER_NONE, "IRQ_NFC-eint", NULL);

            //Thomas_tmp if(ret > 0 )
                //Thomas_tmp pr_err("%s : EINT IRQ LINE NOT AVAILABLE\n", __FILE__);
        }
        //Thomas_tmp else
        //Thomas_tmp {
            //Thomas_tmp pr_err("%s : can not find NFC eint compatible node\n", __FILE__);
        //Thomas_tmp }
    }
#endif
     ret = request_irq(client->irq, mt6605_dev_irq_handler,
			  IRQF_TRIGGER_RISING, client->name,&_gmt6605_dev );
	if (ret) {
		LOGD(KERN_DEBUG "mtk6605 probe: request_irq failed\n");	
	}

    //LC debug,20121021
    //mt_eint_unmask(CUST_EINT_IRQ_NFC_NUM);
    //mt6605_disable_irq(mt6605_dev);
    //mt_eint_mask(CUST_EINT_IRQ_NFC_NUM);
    mt6605_disable_irq(&_gmt6605_dev);   //DEVICE_TREE

    i2c_set_clientdata(client, &_gmt6605_dev);
		
    forceExitBlockingRead = 0;

    return 0;

//err_request_irq_failed:
//    misc_deregister(&_gmt6605_dev.mt6605_device); // delete device node

err_dis_gpio:
	/*ret = gpio_direction_input(platform_data->sysrstb_gpio);
	if (ret)
		dev_err("nfc-nci probe: Unable to set direction\n");
	if ((!strcmp(platform_data->clk_src, "GPCLK")) ||
            (!strcmp(platform_data->clk_src, "GPCLK2"))) {
		ret = gpio_direction_input(platform_data->clk_src_gpio);
		if (ret)
			dev_err("nfc-nci probe: Unable to set direction\n");
		gpio_free(platform_data->clk_src_gpio);
	}*/
	gpio_free(platform_data->sysrstb_gpio);
	
err_eint_gpio:
		gpio_free(platform_data->eint_gpio);
    
err_misc_register:
    mutex_destroy(&_gmt6605_dev.read_mutex);
	//kfree(mt6605_dev);
    //err_exit:
    gpio_free(platform_data->sysrstb_gpio);
    
err_ven_gpio:
		gpio_free(platform_data->ven_gpio);
		
err_irq:
		gpio_free(platform_data->irq_gpio);

err_free_dev:
		//kfree(mt6605_dev);
    
    return ret;      
}


static int mt6605_remove(struct i2c_client *client)
{	
    //struct mt6605_dev *mt6605_dev;
    	
    printk(KERN_DEBUG "mt6605_remove\n");

    //if ( I2CDMAWriteBuf ) {
    //    dma_free_coherent(NULL, MAX_BUFFER_SIZE, I2CDMAWriteBuf, I2CDMAWriteBuf_pa);
    //    I2CDMAWriteBuf = NULL;
    //    I2CDMAWriteBuf_pa = 0;
    //}

    //if ( I2CDMAReadBuf ) {
    //    dma_free_coherent(NULL, MAX_BUFFER_SIZE, I2CDMAReadBuf, I2CDMAReadBuf_pa);
    //    I2CDMAReadBuf = NULL;
    //    I2CDMAReadBuf_pa = 0;
    //}

    //mt6605_dev = i2c_get_clientdata(client);
    free_irq(client->irq, &_gmt6605_dev);
    misc_deregister(&_gmt6605_dev.mt6605_device);
    mutex_destroy(&_gmt6605_dev.read_mutex);
    gpio_free(_gmt6605_dev.irq_gpio);
    gpio_free(_gmt6605_dev.ven_gpio);
    gpio_free(_gmt6605_dev.sysrstb_gpio);
    gpio_free(_gmt6605_dev.eint_gpio);
    //kfree(mt6605_dev);
    return 0;
}


static irqreturn_t mt6605_dev_irq_handler(int irq, void *dev_id)
{
  //  struct mt6605_dev *mt6605_dev = ;
    LOGD(KERN_DEBUG "%s : mt6605_dev_ptr=%p ----->\n", __func__, mt6605_dev_ptr);
#if 1
    if (NULL == mt6605_dev_ptr) {
        LOGD(KERN_DEBUG "mt6605_dev_ptr NULL\n");
        return IRQ_HANDLED;
    }
    mt6605_disable_irq(&_gmt6605_dev);
    wake_up(&mt6605_dev_ptr->read_wq);
    //wake_up_interruptible(&mt6605_dev->read_wq);

    LOGD(KERN_DEBUG "%s : wake_up &read_wq changer <-----\n", __func__);
    //printk(KERN_DEBUG "mt6605_dev_irq_handler\n");
#endif
    return IRQ_HANDLED;
}



static ssize_t mt6605_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
    struct mt6605_dev *mt6605_dev = filp->private_data;
    //char read_buf[MAX_BUFFER_SIZE];
    int ret = 0;
    //int dir=0;

    LOGD(KERN_DEBUG "%s : %d, &mt6605_dev=%p", __func__, count, mt6605_dev);

    if (count > MAX_BUFFER_SIZE) {
        count = MAX_BUFFER_SIZE;
    }

    LOGD(KERN_DEBUG "%s : reading %zu bytes.\n", __func__, count);  
  
    //if (!mt_get_gpio_in(mt6605_dev->irq_gpio)) {
    if (!gpio_get_value(mt6605_dev->irq_gpio)) {
        if (filp->f_flags & O_NONBLOCK) {
            ret = -EAGAIN;
            LOGD(KERN_DEBUG "%s : goto fail\n", __func__);
            goto fail;
        }

        //mutex_lock(&mt6605_dev->irq_enabled_lock);
        //mt6605_dev->irq_enabled = true;
        //mutex_unlock(&mt6605_dev->irq_enabled_lock);

        mutex_lock(&mt6605_dev->read_mutex);
        if ( forceExitBlockingRead == 1 ) {
            LOGD(KERN_DEBUG "%s :forceExitBlockingRead.\n", __func__);
            forceExitBlockingRead = 0; // clear flag
            mutex_unlock(&mt6605_dev->read_mutex);
            goto fail;
        }
        mutex_unlock(&mt6605_dev->read_mutex);
        

        //mt_eint_unmask(mt6605_dev->client->irq); // Enable Interrupt
        //printk(KERN_DEBUG "%s : mt_eint_unmask %d, IRQ,%d \n",
        //        __func__,
        //        mt6605_dev->client->irq,
        //        mt_get_gpio_in(mt6605_dev->irq_gpio));
        mt6605_enable_irq(mt6605_dev);  //DEVICE_TREE
        LOGD(KERN_DEBUG "jiangban:%s : enable_irq %d,%d\n", __func__, gpio_get_value(mt6605_dev->irq_gpio),forceExitBlockingRead);
        //ret = wait_event_interruptible( mt6605_dev->read_wq, (mt_get_gpio_in(mt6605_dev->irq_gpio) || forceExitBlockingRead));
       ret = wait_event_interruptible( mt6605_dev->read_wq, ( gpio_get_value(mt6605_dev->irq_gpio) || forceExitBlockingRead));
		//mt6605_dev_ptr->read_wq
		//init_waitqueue_head(&_gmt6605_dev.read_wq);
		

      // LOGD(KERN_DEBUG "%s : wait_event_interruptible ret,%d, IRQ,%d \n",
        //        __func__, ret,mt_get_gpio_in(mt6605_dev->irq_gpio));
                //Thomas_tmp gpio_get_value(mt6605_dev->irq_gpio);

        if ( ret || forceExitBlockingRead ) {
            LOGD(KERN_DEBUG "%s : goto fail\n", __func__);
            mutex_lock(&mt6605_dev->read_mutex);
            if (forceExitBlockingRead == 1) { 
                LOGD(KERN_DEBUG "%s : clear flag for forceExitBlockingRead\n", __func__);
                forceExitBlockingRead = 0; //clear flag             
            }
            mutex_unlock(&mt6605_dev->read_mutex);
            goto fail;
        }
    }

    //mt6605_dev->client->addr = (mt6605_dev->client->addr & I2C_MASK_FLAG );

    //mt6605_dev->client->ext_flag |= I2C_DMA_FLAG;
    //mt6605_dev->client->ext_flag |= I2C_DIRECTION_FLAG;
    //mt6605_dev->client->ext_flag |= I2C_A_FILTER_MSG;
    //mt6605_dev->client->timing = NFC_CLIENT_TIMING;
    
    /* Read data */
    ret = i2c_master_recv(mt6605_dev->client, (unsigned char *)I2CDMAReadBuf_pa, count);

    //mutex_unlock(&mt6605_dev->read_mutex);

    LOGD(KERN_DEBUG "%s: i2c_master_recv returned %d\n", __func__, ret);
//usleep(5000);
    if (ret < 0) {
        LOGD(KERN_DEBUG "%s: i2c_master_recv returned %d\n", __func__, ret);
	//	mutex_unlock(&mt6605_dev->read_mutex);
        return ret;
    }
    if (ret > count) {
        LOGD(KERN_DEBUG "%s: received too many bytes from i2c (%d)\n",	__func__, ret);
	//	mutex_unlock(&mt6605_dev->read_mutex);
        return -EIO;
    }

    if (copy_to_user(buf, I2CDMAReadBuf_pa, ret)) {
        LOGD(KERN_DEBUG "%s : failed to copy to user space\n", __func__);
	//	mutex_unlock(&mt6605_dev->read_mutex);
        return -EFAULT;
    }
     

fail:
    //mutex_unlock(&mt6605_dev->read_mutex);
    LOGD(KERN_DEBUG "%s: return,ret2,%d\n", __func__, ret);
    mutex_unlock(&mt6605_dev->read_mutex);
    
    return ret;
   
}


//static int flagContiue = 0;
static ssize_t mt6605_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
    struct mt6605_dev  *mt6605_dev;
    //char write_buf[MAX_BUFFER_SIZE];
    int ret = 0, ret_tmp = 0, count_ori = 0,count_remain = 0, idx = 0;
    int MAX_BUFFER_SIZE_write = 50;
    LOGD(KERN_DEBUG "%s : %d byte enter  ---->", __func__, count);

    mt6605_dev = filp->private_data;

    count_ori = count;
    count_remain = count_ori;

    if (count > MAX_BUFFER_SIZE_write) {
        count = MAX_BUFFER_SIZE_write;
        count_remain -= count;
    }
   // if(flagContiue > 0 && count>1 ) return ret;
   // flagContiue ;
    //msleep(1);
    do {
        //if (copy_from_user(I2CDMAWriteBuf, &buf[(idx*255)], count)) {
        
        LOGD(KERN_DEBUG "test1 0304-1507 send max=%d,idx=%d\n",MAX_BUFFER_SIZE_write,idx);
        
	    if (copy_from_user(I2CDMAWriteBuf_pa, &buf[(idx*MAX_BUFFER_SIZE_write)], count)) {
            LOGD(KERN_DEBUG "%s : failed to copy from user space\n", __func__);
            return -EFAULT;
        }

        LOGD(KERN_DEBUG "%s : writing %zu bytes, remain bytes %zu.\n", __func__, count, count_remain);
        /* Write data */
        //mt6605_dev->client->addr = (mt6605_dev->client->addr & I2C_MASK_FLAG);
        //mt6605_dev->client->ext_flag |= I2C_DMA_FLAG;
        //mt6605_dev->client->ext_flag |= I2C_DIRECTION_FLAG;
        //mt6605_dev->client->ext_flag |= I2C_A_FILTER_MSG;
        //mt6605_dev->client->timing = NFC_CLIENT_TIMING;
        //usleep(50000);
        {
        	char pDeBugBuffer[512];
			uint32_t DeBugL = 0;   
        	int i = 0;
			LOGD(KERN_DEBUG "------before i2c_master_send output data start----->\n");

			memset(pDeBugBuffer, 0x00, 512);
			sprintf(pDeBugBuffer, "[send],");
			DeBugL = strlen(pDeBugBuffer);
			for( i =0;i < count;i++)
			{
			   sprintf((pDeBugBuffer+DeBugL),"%02x,",I2CDMAWriteBuf_pa[i]);
			   DeBugL = strlen(pDeBugBuffer);   
			}
			sprintf((pDeBugBuffer+DeBugL),"\n");        
			LOGD(KERN_DEBUG "%s", pDeBugBuffer);
			LOGD(KERN_DEBUG "------before i2c_master_send output data end<-----\n");
        }
        ret_tmp = i2c_master_send(mt6605_dev->client, (unsigned char *)I2CDMAWriteBuf_pa, count);

        if (ret_tmp != count) {
            LOGD(KERN_DEBUG "%s : i2c_master_send returned %d\n", __func__, ret);
            ret = -EIO;
            return ret;
        }
        LOGD(KERN_DEBUG "test3\n");
       // if(count > 1) return count;
        ret += ret_tmp;
        
        LOGD(KERN_DEBUG "%s : curret=%d,nowret=%d,org=%d\n", __func__, ret_tmp,ret,count_ori);

        if( ret ==  count_ori) {
        	LOGD(KERN_DEBUG "%s : ret== count_ori \n", __func__);
        	break;		
        } else {
            if(count_remain > MAX_BUFFER_SIZE_write) {
            	count = MAX_BUFFER_SIZE_write;
            	count_remain -= MAX_BUFFER_SIZE_write;
            } else {
            	count = count_remain;
            }
            idx++;		

            LOGD(KERN_DEBUG "%s :remain_bytes, %d,%d,%d,%d,%d\n", __func__, ret_tmp,ret,count,count_ori,idx);
        }

    }
    while(1);

	LOGD(KERN_DEBUG "%s : writing %zu bytes. Status %zu <----end\n", __func__, count_ori,ret);
	return ret;
}

static int mt6605_dev_open(struct inode *inode, struct file *filp)
{
	
    struct mt6605_dev *mt6605_dev = container_of(filp->private_data, struct mt6605_dev, mt6605_device);

    filp->private_data = mt6605_dev;
    mt6605_dev_ptr = mt6605_dev;

    LOGD(KERN_DEBUG "%s : %d,%d, %p\n", __func__, imajor(inode), iminor(inode), mt6605_dev_ptr);

    //20121009,LiangChi add
    //mt_set_gpio_out(mt6605_dev->ven_gpio, GPIO_OUT_ZERO);
    //mt_set_gpio_out(mt6605_dev->sysrstb_gpio, GPIO_OUT_ONE);

    forceExitBlockingRead = 0;
    return 0;
}


//#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36))
//static int mt6605_dev_ioctl(struct inode *inode, struct file *filp,
//		unsigned int cmd, unsigned long arg)
//{
//	return 0;
//}
//#else

static long mt6605_dev_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{

    struct mt6605_dev *mt6605_dev = filp->private_data;
    int result = 0;
    int gpio_dir,gpio_num, tmp_gpio;
    int ret = 0;
    int irqn = 0;

    LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl,cmd,0x%x, 0x%lx \n",cmd,arg);
    //(((arg & 0xFF00) >> 8) & 0x00FF),
    //(arg & 0x00FF));
         
    if ((cmd & 0xFFFF) == 0xFE00) {
        //mt_eint_mask(CUST_EINT_IRQ_NFC_NUM);	
         mt6605_disable_irq(&_gmt6605_dev);   //DEVICE_TREE
        LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl,mask_IRQ. \n");
        return 0;
        
    }  else if ((cmd & 0xFFFF) == 0xFE01) {
    
        //mt_set_gpio_mode(mt6605_dev->irq_gpio, GPIO_IRQ_NFC_PIN_M_EINT);
        //mt_set_gpio_dir(mt6605_dev->irq_gpio, GPIO_DIR_IN);
        //mt_set_gpio_pull_enable(mt6605_dev->irq_gpio, GPIO_PULL_ENABLE);
        //mt_set_gpio_pull_select(mt6605_dev->irq_gpio, GPIO_PULL_DOWN);
			if (gpio_is_valid(mt6605_dev->irq_gpio)) {
				ret = gpio_request(mt6605_dev->irq_gpio, "nfc_irq_gpio");
				if (ret) {
					//Thomas_tmp pr_err("unable to request gpio [%d]\n",mt6605_dev->irq_gpio);
					return 0;
				}
				ret = gpio_direction_input(mt6605_dev->irq_gpio);
				if (ret) {
					//Thomas_tmp pr_err("unable to set direction for gpio [%d]\n",mt6605_dev->irq_gpio);
					return 0;
				}
				//gpio_to_irq(0);
				irqn = gpio_to_irq(mt6605_dev->irq_gpio);
				if (irqn < 0) {
					ret = irqn;
					return 0;
				}
				mt6605_dev->client->irq = irqn;
			} else {
				//Thomas_tmp pr_err("irq gpio not provided\n");
				return 0;
			}


        //mt65xx_eint_set_sens(CUST_EINT_IRQ_NFC_NUM, CUST_EINT_IRQ_NFC_SENSITIVE);  
        //mt65xx_eint_set_hw_debounce(CUST_EINT_IRQ_NFC_NUM, CUST_EINT_IRQ_NFC_DEBOUNCE_CN);
        //mt65xx_eint_registration(   CUST_EINT_IRQ_NFC_NUM, 
        //                            CUST_EINT_IRQ_NFC_DEBOUNCE_CN, 
        //                            CUST_EINT_POLARITY_HIGH,
        //                            mt6605_dev_irq_handler, 0);

        //mt_eint_mask(CUST_EINT_IRQ_NFC_NUM);
#if 0
{   //DEVICE_TREE
     struct device_node *node;
     u32 ints[2];
     //int ret;
     
     node = of_find_compatible_node (NULL, NULL, "mediatek, IRQ_NFC-eint");
 
     if(node)
     {
         of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
         //mt_gpio_set_debounce(ints[0], ints[1]);
         
         nfc_irq = irq_of_parse_and_map(node, 0);
         ret = request_irq(nfc_irq, mt6605_dev_irq_handler, IRQF_TRIGGER_NONE, "IRQ_NFC-eint", NULL);
 
         //Thomas_tmp if(ret > 0 )
             //Thomas_tmp pr_err("%s : EINT IRQ LINE NOT AVAILABLE\n", __FILE__);
     }
     //Thomas_tmp else
     //Thomas_tmp {
         //Thomas_tmp pr_err("%s : can not find NFC eint compatible node\n", __FILE__);
     //Thomas_tmp }
     
		}
#endif
		ret = request_irq(mt6605_dev->client->irq, mt6605_dev_irq_handler,
			  			IRQF_TRIGGER_RISING, mt6605_dev->client->name, mt6605_dev);
		if (ret) {
			LOGD(KERN_DEBUG "mtk6605 ioctl: request_irq failed\n"); 
		}

		 mt6605_disable_irq(&_gmt6605_dev);
        LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl,Re-registered IRQ. \n");	
        return 0;
        
    } else if ((cmd & 0xFFFF) == 0xFEFF) { // EXIT EINT
    
        mutex_lock(&mt6605_dev->read_mutex);
        forceExitBlockingRead = 1;
        mutex_unlock(&mt6605_dev->read_mutex);     
        //mt_eint_soft_set(CUST_EINT_EXT_IRQ_NFC_NUM);
        //mt65xx_eint_mask(CUST_EINT_EXT_IRQ_NFC_NUM);
        wake_up_interruptible(&mt6605_dev->read_wq);	
        LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl,SW Release IRQ. \n");	
        return 0;
        
    } else if ((cmd & 0xFFFF) == 0xFEFE) { // Get ChipID
        return fgNfcChip;
    }  else if ((cmd & 0xFFFF) == 0xFEFD) { 
        fgNfcChip = (arg & 0xFFFF);
        return 0;
    }  
     
    tmp_gpio = (((arg & 0xFF00) >> 8) & 0x00FF);
     
    if(tmp_gpio == MTK_NFC_GPIO_EN_B) {
        gpio_num = mt6605_dev->ven_gpio;       
    } else if(tmp_gpio == MTK_NFC_GPIO_SYSRST_B) {
        gpio_num = mt6605_dev->sysrstb_gpio;
    } else if(tmp_gpio == MTK_NFC_GPIO_EINT) {
    	LOGD(KERN_DEBUG "%s:xxxx eint_gpio code disable-",__func__);
        gpio_num = mt6605_dev->eint_gpio;
       //return result;
    } else if(tmp_gpio == MTK_NFC_GPIO_IRQ) {
        gpio_num = mt6605_dev->irq_gpio;
    } else {
        result = MTK_NFC_PULL_INVALID;
        return result;
    }

    LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl 03191402,0x%X,0x%X. \n", tmp_gpio, gpio_num);
     
    if(result != MTK_NFC_PULL_INVALID) {
        if(cmd == MTK_NFC_IOCTL_READ) { // READ
            //gpio_dir = mt_get_gpio_dir(gpio_num); 
            gpio_dir = get_gpio_dir(gpio_num);
            
            if((gpio_dir == GPIO_DIR_IN) || (gpio_dir == GPIO_DIR_OUT)) {
                //result = mt_get_gpio_in(gpio_num);// 	
                result = gpio_get_value(gpio_num);	
            } else {
                result = MTK_NFC_PULL_INVALID;		
            }    		
            
        } else if(cmd == MTK_NFC_IOCTL_WRITE) { // WRITE
            //gpio_dir = mt_get_gpio_dir(gpio_num); 
            gpio_dir = get_gpio_dir(gpio_num);

            if(gpio_dir == GPIO_DIR_OUT) {
                int gpio_pol = (arg & 0x00FF);

                if ( gpio_pol == MTK_NFC_PULL_LOW) {
			LOGD(KERN_DEBUG "%s:xxxx gpio pull low",__func__);
                		result = gpio_direction_output(gpio_num, 0);
                    //result = mt_set_gpio_out(gpio_num, GPIO_OUT_ZERO);
                    
                        
                } else if ( gpio_pol == MTK_NFC_PULL_HIGH) {
			LOGD(KERN_DEBUG "%s:xxxx gpio pull high",__func__);
                		result = gpio_direction_output(gpio_num, 1);
                    //result = mt_set_gpio_out(gpio_num, GPIO_OUT_ONE);
                    
                }
            } else {
                result = MTK_NFC_PULL_INVALID;		
            }    		        
        } else { // ERROR
            result = MTK_NFC_PULL_INVALID;	
        }	
    }         
		
    LOGD(KERN_DEBUG "mt6605_dev_unlocked_ioctl,%d\n",result);
    return result;
}

#if 0
static int mt6605_detect(struct i2c_client *client, int kind, struct i2c_board_info *info)
{

   LOGD(KERN_DEBUG "mt6605_detect\n");
   //strcpy(info->type, "mt6605");
   return 0;
}
#endif

static void reg_gpio_pin(struct device *dev)
{
	struct device_node *np = dev->of_node;
	mt6605_platform_data.irq_gpio = of_get_named_gpio(np, "mtk,irq-gpio", 0);
	mt6605_platform_data.ven_gpio = of_get_named_gpio(np, "mtk,env-gpio", 0);
	mt6605_platform_data.sysrstb_gpio = of_get_named_gpio(np, "mtk,rst-gpio", 0);
	mt6605_platform_data.eint_gpio = of_get_named_gpio(np, "mtk,eint-gpio", 0);
	LOGD(KERN_DEBUG "dtsi gpio vale:irq_gpio(%d),ven_gpio(%d),sysrstb_gpio(%d),eint_gpio (%d)\n",
				mt6605_platform_data.irq_gpio,
				mt6605_platform_data.ven_gpio,
				mt6605_platform_data.sysrstb_gpio,
				mt6605_platform_data.eint_gpio);
}

static int get_gpio_dir(int gpio_num)
{
	if(gpio_num == mt6605_platform_data.irq_gpio)
	{
		//input
		return GPIO_DIR_IN;
	}
	else if((gpio_num == mt6605_platform_data.ven_gpio) || (gpio_num == mt6605_platform_data.sysrstb_gpio) || (gpio_num == mt6605_platform_data.eint_gpio))
	{
		//output
		return GPIO_DIR_OUT;
	}
	else
	{
		return MTK_NFC_PULL_INVALID;		
	}
}

/*
 * module load/unload record keeping
 */
static int __init mt6605_dev_init(void)
{
    LOGD(KERN_DEBUG "mt6605_dev_init\n");
    //i2c_register_board_info(NFC_I2C_BUSNUM, &nfc_board_info, 1);
    LOGD(KERN_DEBUG "mt6605_dev_init2\n");
    i2c_add_driver(&mt6605_dev_driver);
    LOGD(KERN_DEBUG "mt6605_dev_init success\n");	
    return 0;
}

static void __exit mt6605_dev_exit(void)
{
    LOGD(KERN_DEBUG "mt6605_dev_exit\n");
    i2c_del_driver(&mt6605_dev_driver);
}


module_init(mt6605_dev_init);
module_exit(mt6605_dev_exit);

MODULE_AUTHOR("LiangChi Huang");
MODULE_DESCRIPTION("MTK NFC driver");
MODULE_LICENSE("GPL");
