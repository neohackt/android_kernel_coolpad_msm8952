...
char RegulatorOnOff[4] = {0};
static struct class *regulatorprintonoff_class = NULL;
#endif
...
#ifdef CONFIG_YL_PM_DEBUG
static int _regulator_get_current_limit_rm_mutex(struct regulator_dev *rdev);
static void print_constraints(struct regulator_dev *rdev);
static void print_constraints_rm_mutex(struct regulator_dev *rdev);
#endif
...
#ifdef CONFIG_YL_PM_DEBUG
int regulatorstatusprint_yl(int force)
{
        ...
        int enabled=0;
        char *value = "on";
        if(!force && strcmp(value, RegulatorOnOff)){
        ...
        list_for_each_entry(rdev, &regulator_list, list) {
                printk(KERN_ERR"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
                print_constraints_rm_mutex(rdev);
                mutex_lock(&rdev->mutex);
                voltage=0;
                enabled=0;
                if (rdev->desc->ops->get_voltage)
                        voltage = rdev->desc->ops->get_voltage(rdev);
                if (rdev->desc->ops->is_enabled)
                enabled = rdev->desc->ops->is_enabled(rdev);
                printk(KERN_ERR"name=%s,voltage=%duV, is_eabled=%d\n",rdev->desc->name,voltage,enabled);
                list_for_each_entry(regulator, &rdev->consumer_list, list) {
                        printk(KERN_ERR"supply_name=%s,dev_name=%s, eable_times=%d,[%d-%d]uV, %duA\n",regulator->supply_name,regulator->dev?dev_name(regulator->dev):"NULL",
                                regulator->enabled,regulator->min_uV,regulator->max_uV,regulator->uA_load);
                }
...
EXPORT_SYMBOL_GPL(regulatorstatusprint_yl);

static ssize_t regulator_onoff_show(struct class *cls,
                                    struct class_attribute *attr, char *buf)
{
        printk(KERN_ERR"%s,regulator_onoff status=%s\n",__func__,buf);
        return sprintf(buf, "%s\n", RegulatorOnOff);
}
static ssize_t regulator_onoff_store(struct class *cls,
                                    struct class_attribute *attr, const char *buf, size_t count)
{
        ...
        if( count > 4 )
                ...
                sscanf(buf,"%s",RegulatorOnOff);
        }
        return count;
}

static CLASS_ATTR(regulator_onoff, 0664, regulator_onoff_show,regulator_onoff_store );

static ssize_t regulator_print_yl_show(struct class *cls,
                                    struct class_attribute *attr, char *buf)
{
        printk(KERN_ERR"%s\n",__func__);
        return regulatorstatusprint_yl(1);
}
static CLASS_ATTR(regulator_print_yl, 0664, regulator_print_yl_show, NULL);
#endif
...
static void print_constraints_rm_mutex(struct regulator_dev *rdev)
{
        ...
        if (!constraints->min_uA ||
            constraints->min_uA != constraints->max_uA) {
                ret = _regulator_get_current_limit_rm_mutex(rdev);
                if (ret > 0)
                        count += sprintf(buf + count, "at %d mA ", ret / 1000);
        }
...
#ifdef CONFIG_YL_PM_DEBUG
        if(NULL ==regulatorprintonoff_class)
        {
                regulatorprintonoff_class = class_create(THIS_MODULE,"regulatorprintonoff");
                if (IS_ERR(regulatorprintonoff_class))
                {
                        printk(KERN_ERR " %s: failed to create regulatorprintonoff_class\n",__func__);
                }
        }
        ret = class_create_file(regulatorprintonoff_class, (const struct class_attribute * )&class_attr_regulator_onoff.attr );
        if( ret < 0 )
        {
                printk(KERN_ERR"failed to create class_attr_regulator_onoff\n");
        }

        ret = class_create_file(regulatorprintonoff_class, (const struct class_attribute * )&class_attr_regulator_print_yl.attr );
        if( ret < 0 )
        {
                printk(KERN_ERR"failed to create class_attr_regulator_print_yl\n");
        }
#endif
