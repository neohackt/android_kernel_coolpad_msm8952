...
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION3
        if (!strcmp(rdev->desc->name,"pm8950_l11") || !strcmp(rdev->desc->name,"pm8950_l12")) {
            if (sd_slot_present) {
                printk(KERN_ERR "mmc1: sd slot is present,enable vreg %s\n",rdev->desc->name);
            } else {
                printk(KERN_ERR "mmc1: sd slot is not present,will not enable vreg %s\n", rdev->desc->name);
                return 0;
            }
        }
#endif
...
static int rpm_vreg_disable(struct regulator_dev *rdev)
{
	...
	u32 prev_enable;
        ...
        if (!strcmp(rdev->desc->name,"pm8950_l11") || !strcmp(rdev->desc->name,"pm8950_l12")) {
            if (sd_slot_present) {
                printk(KERN_ERR "mmc1: sd slot is present,disable vreg %s\n",rdev->desc->name);
            } else {
                printk(KERN_ERR "mmc1: sd slot is not present,disnable vreg %s\n", rdev->desc->name);
            }
        }
#endif
