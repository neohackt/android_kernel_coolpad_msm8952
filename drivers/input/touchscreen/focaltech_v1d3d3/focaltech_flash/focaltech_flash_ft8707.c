...
*******************************************************************************/
#define FOCALTECH_FLASH_FT8707_INFO  "File Version of  focaltech_flash_ft8707.c:  V1.0.0 2016-03-21"

#define FTS_REG_FT8707_VENDOR_ID 		0xA8
...
*******************************************************************************/
static int  fts_ic_program_ft8707_write_pram( u8* pbt_buf, u32 dw_lenth);
static int fts_ic_program_ft8707_upgrade( u8 *pbt_buf, u32 dw_lenth);
static int fts_ft8707_get_i_file(u8 **file_buff, int *file_len);
...
static int  fts_ic_program_ft8707_write_pram( u8* pbt_buf, u32 dw_lenth)
{
	...
	FTS_COMMON_DBG("8707 dw_lenth= %d",dw_lenth);
	...
	{
		//如果软件复位不成功，采用硬件复位
		if(0 != fts_upgrade_hardware_reset(200, 0x87, 0x07))
			return -EIO;
	}
	...
	*/
	...
	if (reg_val[0] != bt_ecc) 
	{
		FTS_COMMON_DBG( "[FTS]--ecc error! FW=%02x bt_ecc=%02x", reg_val[0],bt_ecc);	
		return -EIO;
	}
	FTS_COMMON_DBG(KERN_WARNING "checksum %X %X ", reg_val[0],bt_ecc);
	DBG("Read flash and compare");
		*/
	msleep(100);
	//-----------------------------------------------------------------------------------------------------
	FTS_COMMON_DBG( "[FTS]--nowAddress=%02x dw_lenth=%02x", nowAddress,dw_lenth);	
	...
	while(1)
	{
		...
		{			
			if(!fts_upgrade_read_pram(StartFlashAddr, pCheckBuffer+FlashAddr, dw_lenth-FlashAddr))
			{
		...
		{
			if(!fts_upgrade_read_pram(StartFlashAddr, pCheckBuffer+FlashAddr, FTS_PACKET_LENGTH))
			{
				FTS_COMMON_DBG("read out checksum error");
				return -EIO;
	...
	FTS_COMMON_DBG( "[FTS]--FlashAddr=%02x dw_lenth=%02x", FlashAddr,dw_lenth);
	if(FlashAddr == dw_lenth)
	{
...
/************************************************************************
*   Name: fts_ic_program_ft8707_upgrade
* Brief:  fw upgrade
...
static int fts_ic_program_ft8707_upgrade( u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		if( i == 30) FTS_COMMON_DBG("Query 6a reg time out value: 0x%x, driver set value:0x%x",   temp, uCheckOff + uCheckStart);
	}
...
static int fts_ft8707_get_i_file(u8 **file_buff, int *file_len)
{
	...
	{
		i_ret = fts_read_reg(FTS_REG_FT8707_VENDOR_ID, &uc_vender_id);
		if(i_ret < 0)
		{
...
int fts_ft8707_upgrade_with_i_file(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	/*get i file*/
	i_ret = fts_ft8707_get_i_file(&pbt_buf, &fw_len);
	if(i_ret < 0)
	{
		return -1;
	}
	...
	if(NULL == pbt_buf)
	{
	 ...
	 /* call the write pram function */
    	i_ret = fts_ic_program_ft8707_write_pram( BUFFER_PRAM_BOOT_FILE, g_len_pram_boot_file);
	
	if (i_ret != 0)
	{
		FTS_COMMON_DBG( "%s:WritePram failed. err.", __func__);
		return -EIO;
	}
	
	/*call upgrade function*/
	i_ret = fts_ic_program_ft8707_upgrade( pbt_buf, fw_len);

   	if (i_ret != 0) 
	{
    		FTS_COMMON_DBG( "[FTS] upgrade failed. err=%d.",  i_ret);
    	} 
	else 
	{
		#ifdef AUTO_CLB
...
int fts_ft8707_get_i_file_version(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
...
int fts_ft8707_upgrade_with_bin_file(u8 *file_buff, int file_len)
{
	u8 *pbt_buf = NULL;
	int i_ret= 0;
	int fw_len = 0;
