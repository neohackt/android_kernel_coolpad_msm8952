...
static int invalide[TX_NUM_MAX][RX_NUM_MAX] = {{0,0}};
...
static int AllocateMemory(void)
{
	...
	if(NULL == g_pStoreMsgArea)
		g_pStoreMsgArea = kmalloc(BUFF_LEN_STORE_MSG_AREA, GFP_ATOMIC);
	if(NULL == g_pStoreMsgArea)
		goto ERR;
	
	g_pMsgAreaLine2 =NULL;	
	if(NULL == g_pMsgAreaLine2)
		g_pMsgAreaLine2 = kmalloc(BUFF_LEN_MSG_AREA_LINE2, GFP_ATOMIC);
	if(NULL == g_pMsgAreaLine2)
		goto ERR;
	
	g_pStoreDataArea =NULL;	
	if(NULL == g_pStoreDataArea)
		g_pStoreDataArea = kmalloc(BUFF_LEN_STORE_DATA_AREA, GFP_ATOMIC);
	if(NULL == g_pStoreDataArea)
		goto ERR;
		
	g_pTmpBuff =NULL;	
	if(NULL == g_pTmpBuff)
		g_pTmpBuff = kmalloc(BUFF_LEN_TMP_BUFFER, GFP_ATOMIC);
	if(NULL == g_pTmpBuff)
		goto ERR;
			...
			*/
			...
			//	printk("Tx%2d:  		"  , iRow);
				...
				//	RawDataMax = g_stCfg_MCap_DetailThreshold.RawDataTest_High_Max[iRow][iCol];
					...
					{
						btmpresult = false;
						FTS_TEST_DBG("Tx Linearity Out Of Range.  Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n", \
...
//	iAdcData = new int[iAllAdcDataNum];
	...
	memset(iAdcData, 0, iAllAdcDataNum*sizeof(int));
	for( i = 0; i < 1; i++)
	{
...
//	strAdc = "\r\n\r\nShort Circuit (Channel and Channel):\r\n";
	...
	*/
	...
	memset(fMShortResistance,0,iChannelNum*sizeof(int));
...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
	...
	for(i = 0; i < 100; i++)//准备好数据后，FW将0x07寄存器置为0
	{
		SysDelay(10);
