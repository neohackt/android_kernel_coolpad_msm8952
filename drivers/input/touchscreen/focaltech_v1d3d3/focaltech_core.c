...
int fts_i2c_read(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
	...
	{
		printk(KERN_ERR "%s: i2c client is empty!",  __func__);
		return -1;
	}
	...
	if (writelen > 0) {
		...
		if (ret < 0){
			printk(KERN_ERR "%s: i2c read error.",  __func__);
		}
...
static int fts_read_tp_psensor_data(struct fts_ts_data *data)
{
	...
	fts_read_reg(FTS_REG_PSENSOR_STATUS, &psensor_status);
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		{
                        if(pdata->xflip){
                        	event->au16_x[i] = pdata->x_max - event->au16_x[i];
                        }
		
                        if(pdata->yflip){
                        	event->au16_y[i] = pdata->y_max - event->au16_y[i];
                        }
...
static int fts_ts_stop(struct device *dev)
{
	...
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		...
		fts_i2c_write(txbuf, sizeof(txbuf));
	}
...
int fts_ts_suspend(struct device *dev)
{
	...
	#if FTS_GESTRUE_EN
	         fts_write_reg(0xd0, 0x01);
		  if (fts_upgrade_info_curr.chip_id==0x54 || fts_upgrade_info_curr.chip_id==0x58 || fts_upgrade_info_curr.chip_id==0x86 || fts_upgrade_info_curr.chip_id==0x87)
		  {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	pdata->xflip = of_property_read_bool(np, "focaltech,x-flip");

	pdata->yflip = of_property_read_bool(np, "focaltech,y-flip");
...
int fts_tp_active(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 1;
}

int fts_tp_need_update(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_do_update(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_need_calibrate(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_calibrate(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

touch_mode_type fts_tp_get_mode(void)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_set_mode(touch_mode_type work_mode)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_get_version(char* version)
{
	...
	unsigned char * fver = NULL;
        ...
        vendor = "BOEN";
	fver = fts_wq_data->fw_ver;

	return sprintf(version, "%s:%s:F%02x%02x:P%02x", vendor,"ft8716",
		fver[1],
		fver[0],
		fver[2]);
}

int fts_tp_debug(int mode)
{
	FTS_DBG("%s.\n", __FUNCTION__);
	return 0;
}

int fts_tp_vendor(char* vendor)
{
...
touchscreen_ops_tpye fts_tp_ops = {
	.touch_id               = 0,
	.touch_type             = 1,
	.active                 = fts_tp_active,
	.firmware_need_update   = fts_tp_need_update,
	.firmware_do_update     = fts_tp_do_update,
	.need_calibrate         = fts_tp_need_calibrate,
	.calibrate              = fts_tp_calibrate,
	.get_mode               = fts_tp_get_mode,
	.set_mode               = fts_tp_set_mode,
	.get_firmware_version   = fts_tp_get_version,
	.debug                  = fts_tp_debug,
	.get_vendor             = fts_tp_vendor,
	.get_wakeup_gesture     = fts_get_wakeup_gesture,
	.gesture_ctrl           = fts_gesture_ctrl,
};

static int register_yl_interface(void)
{
	int ret = -1;

	ret = touchscreen_set_ops(&fts_tp_ops);
	if (ret) {
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = register_yl_interface();
        if (err < 0) {
                dev_err(&client->dev,
                        "register yulong interface fail\n");
                goto err_gpio_req;
        }
	...
	reg_addr = FTS_REG_ID;
	err = fts_i2c_read(&reg_addr, 1, &reg_value, 1);
	if (err < 0) {
	...
	fts_i2c_read(&reg_addr, 1, &reg_value, 1);
	if (err < 0)
	...
	/* add code for openshort test xuhaifeng*/
	fts_test_init();
