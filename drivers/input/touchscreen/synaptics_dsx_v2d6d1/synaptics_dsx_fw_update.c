...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (retval < 0) {
        printk(KERN_ERR "Syna: not get image fw id.\n");
		flash_area = NONE;
		goto exit;
	}
	dev_err(rmi4_data->pdev->dev.parent,
...
#if	0	//def CONFIG_LPM_MODE
    if (1 == ftm_mode)
    {
        printk( KERN_ERR "Syna:in fastmmi mode, so not do update firmware!\n");
        return;
    }
#endif

    if( NULL == SynaFirmware )
    {
        printk( KERN_ERR "Syna:SynaFirmware is NULL,stop firmware update.\n");
        return;
    }
...
//==>
int syna_update_check(void)
{
    int retval = 0;
    enum flash_area flash_area;

    if(NULL == SynaFirmware) {
        printk(KERN_ERR "Syna: %s tp id is not match\n", __func__);
        return 0;
    }
    ...
    if (flash_area == NONE){
        retval = 0;
        printk(KERN_ERR "%s:no need to update!\n",__func__);
    }else{
        retval = 1;
        printk(KERN_ERR "%s:need to update!\n",__func__);
    }

    return retval;
}
EXPORT_SYMBOL(syna_update_check);

int syna_do_update(void)
{
    int retval = 0;
...
EXPORT_SYMBOL(syna_do_update);

int get_dev_version_id(char* version, bool enable)
{
    int i = 0;
    int retval = 0;
    unsigned char config_id[32] = {0};
    unsigned char ic_type[8] = {0};
    ...
    retval = synaptics_rmi4_reg_read(fwu->rmi4_data,
            ...
            sizeof(ic_type) - 1);
    if (retval < 0) {
        printk("%s: Failed to read IC name.\n", __func__);
        return retval;
    }
    ...
    retval = synaptics_rmi4_reg_read(fwu->rmi4_data,
            ...
            sizeof(config_id));
    if (retval < 0) {
        printk(KERN_ERR "Syna:Failed to read device config ID\n");
        return retval;
    }

    sensor_id   = config_id[28];
    fw_version  = config_id[30];
    cfg_version = config_id[31];

    if(!enable){
        for(i = 0; i < ARRAY_SIZE(syna_tw_fw); i++)
        {
            if( sensor_id == syna_tw_fw[i].tp_id ) {
                SynaProductor = syna_tw_fw[i].tp_name;
                SynaFirmware = syna_tw_fw[i].firmware;
                printk(KERN_ERR "Syna:tp name is %s\n",SynaProductor);
                break;
            }
        }

        if(NULL == SynaFirmware) {
            printk(KERN_ERR "Syna:tp id is not match\n");
            return -1;
        }
    }

    sprintf(version,"%s:%s:0x%02x:0x%02x",
            SynaProductor, ic_type, fw_version, cfg_version);
...
//==>
	retval = get_dev_version_id(tp_version, false);
	if (retval < 0){
		printk(KERN_ERR "Syna: get_dev_version_id failed");
	}
//==>
