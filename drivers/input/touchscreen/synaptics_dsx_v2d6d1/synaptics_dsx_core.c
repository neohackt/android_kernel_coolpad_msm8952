...
#endif
unsigned SYNA_DEBUG = false;
#define WAKEUP_GESTURE false
#define USE_PINCTRL_INIT
...
*/
#define DELAY_HARDWARE_RESET       15
#define DELAY_HARDWARE_RESET_READY 150
#define RPT_TYPE (1 << 0)
...
//==>
extern int get_dev_version_id(char* version, bool enable);
extern int syna_update_check(void);
extern int syna_do_update(void);
//==>
...
int syna_tp_get_version(char* version)
{
    ...
    retval = get_dev_version_id(version, true);
    if (retval < 0)
    {
...
{
    return sprintf(vendor,"%s%02x","synaptics_", sensor_id);
}
		...
		unsigned int intr_count)
{
	...
	rmi4_data->stylus_enable = 0;// del ctrl_23->stylus_enable;
	rmi4_data->eraser_enable = 0;// del ctrl_23->eraser_enable;
...
err_set_input_dev:
	...
	if (rmi4_data->ts_pinctrl) {
		if (IS_ERR_OR_NULL(rmi4_data->pinctrl_state_release)) {
			devm_pinctrl_put(rmi4_data->ts_pinctrl);
			rmi4_data->ts_pinctrl = NULL;
            printk(KERN_ERR "release TW pinctrl in syna.\n");
		} else {
...
static int synaptics_rmi4_suspend(struct device *dev)
{
    ...
    printk(KERN_ERR"syna: suapend inter\n");
	if (rmi4_data->stay_awake)
		return 0;
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->stay_awake)
    {
        printk(KERN_ERR "syna: tw in upgrade mode\n");
        return 0;
    }
    if (!rmi4_data->suspend)
    {
        printk(KERN_ERR "syna: tw in suspend now\n");
        return 0;
    }
    printk(KERN_ERR"syna: resume");
    synaptics_rmi4_hardware_reset_tp(rmi4_data);
