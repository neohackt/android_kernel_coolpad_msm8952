...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (prop && prop->length) {
		...
		bdata->power_gpio = -1;
		printk("%s synaptics,power-gpio no this dts node !\n",__func__);
	}
	...
	if (prop && prop->length) {
		...
		bdata->vir_button_map->nbuttons = 0;
		bdata->vir_button_map->map = NULL;
		printk("%s synaptics,vir-button-codes dts node not found!\n",__func__);
	}
		...
		const struct i2c_device_id *dev_id)
{
	...
	printk("synaptics-dsx-v261 probe\n");
	...
	printk("synaptics-dsx-v261 probe sucesss\n");
	return 0;
}
...
int synaptics_rmi4_bus_init(void)
{
	...
	printk("%s synaptics, start ret  =%d \n",__func__, ret);
	ret = i2c_add_driver(&synaptics_rmi4_i2c_driver);
	printk("%s synaptics,ret  =%d \n",__func__, ret);
	return ret ;
}
