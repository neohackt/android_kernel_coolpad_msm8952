...
#define FW_IMAGE_NAME "synaptics/startup_fw_update.img"
 ...
 #define DO_STARTUP_FW_UPDATE    //modify by liuxinyuan for update firmware

#ifdef DO_STARTUP_FW_UPDATE
#ifdef CONFIG_FB
#define WAIT_FOR_FB_READY
#define FB_READY_WAIT_MS 20
#define FB_READY_TIMEOUT_S 30
#endif
#endif

extern unsigned int fwu_flag;   //shihaobin@yulong.com add for firmware update flag 20151211
...
static int fwu_start_reflash(void)
{
	...
	case UI_FIRMWARE:
        ...
        //shihaobin@yulong.com modify for delete rebuild work when update firmware 20151207 begin
		rmi4_data->reset_device(rmi4_data, false);
		break;
	case UI_CONFIG:
		retval = fwu_check_ui_configuration_size();
		...
		retval = fwu_write_ui_configuration();
		rmi4_data->reset_device(rmi4_data, false);
        //shihaobin@yulong.com modify for delete rebuild work when update firmware 20151207 end
		break;
	case NONE:
	default:
		rmi4_data->reset_device(rmi4_data, false);
...
//add by liuxinyuan for fw_update 20151013
static int syna_fw_choose(void)
{
	if(!memcmp("0xac",syna_data.vendor_id,4)){
		syna_data.firmware_data = SYNA_FW_boen;   ///boen firmware
		SYNA_INFO("yl_syna_update_debug:%s,vendor_id is ac\n",__func__);
      return 0;
	}else if(!memcmp("0xa0",syna_data.vendor_id,4)){
		syna_data.firmware_data = SYNA_FW_ofilm;     ////ofilm firmware
		SYNA_INFO("yl_syna_update_debug:%s,vendor_id is a0\n",__func__);
       return 0;
	}else{
	
		printk(KERN_ERR"yl_syna_update_debug:%s,vendor_id is default\n",__func__);
		return -1;
    }
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	//shihaobin@yulong.com modify for do judgement of rb_ready twice 20151205 begin
	if (!do_once)
		return;
...
wait_for_fb_ready:
	...
	if (do_once) {
		...
		goto wait_for_fb_ready;
	}
	//shihaobin@yulong.com modify for do judgement of rb_ready twice 20151205 end
#endif
        retval = syna_fw_choose();
	        if(retval < 0)
	        {
		        SYNA_ERROR("FW init failed\n");
		        return;
	        }
    ...
    if (1 != ftm_mode)
    {
        synaptics_fw_updater(syna_data.firmware_data);
    }
    else
    {
        printk("smart phone in fastmmi mode, so not do update firmware!\n");
    }
    #else
    {
        synaptics_fw_updater(syna_data.firmware_data);
    ...
    #endif
    //shihaobin@yulong.com add for close tp work only when power-off-charging and recovery mode 20151123 end
...
#if 0
/******************************add by liuxinyuan@yulong add begin********************************/

#include <linux/input/touchscreen_yl.h>

int register_yulong_platform_interface(void)
{
        int error = -1;

	error = syna_firmware_init();//get sensorID & init FW
        if(error) {
		printk(KERN_ERR "SYNA:YLLOG:fw init failed!\n");
                return error;
        }

	error = touchscreen_set_ops(&syna_tp_ops);
	if (error) {
...
//add by liuxinyuan for update check 20151013 begin

int syna_s2333_update_check()
{
	int retval;
	enum flash_area flash_area;
	retval = fwu_read_flash_status();
	if (retval < 0){
		SYNA_ERROR("read flash status fail!\n");
		return 0;
	}
	flash_area = fwu_go_nogo();
	if (flash_area == NONE){
		retval = 0; //noneed to update
		printk("%s:noneed to update!\n",__func__);
	}else{
		retval = 1;
		printk("%s:need to update!\n",__func__);
	}
	return retval;
}
int syna_s2333_do_update()
{
	int retval;
	retval = fwu_start_reflash();
	if (retval < 0) {
	printk("%s: Failed to update\n",
			__func__);
	}
	return retval;
}

//// add end for updata check 

//add begin by liuxinyuan to get version_id
//extern int get_device_info(char *buf);

static int find_tp_handler(unsigned char *tp_id)
{
	int i;
	int tp_handler;
	int tp_info_total = sizeof(syna_tw_fw)/sizeof(syna_tw_fw[0]);
	for(i = 0;i < tp_info_total;i++)
	{
		if(!strncmp(tp_id,syna_tw_fw[i].tp_id,strlen(tp_id)))
		{
			tp_handler = i;
			break;
		}
//tp_handler = 1;
	...
	if (i == tp_info_total)
	{
		printk("DO not find tp_id,use default!");
		tp_handler = tp_info_total-1;
	}
	return tp_handler;
}


int get_dev_version_id(char* version)
{
	int retval;
	unsigned char tp_id[11];
	int tp_handler;
	unsigned char config_id[4];
	memset(tp_id,0,sizeof(tp_id));

    retval = synaptics_rmi4_reg_read(fwu->rmi4_data,
               fwu->rmi4_data->f01_query_base_addr+11,
               tp_id,sizeof(tp_id)-1);
	if (retval < 0) {
		printk("%s: Failed to read Sensor_ID\n",
				__func__);
		return retval;
	}

	memcpy(syna_data.vendor_id,&tp_id[6],strlen(&tp_id[6]));
printk(KERN_ERR"yl_syna_update version tp_id is %s ,syna_data.vendor_id=%s\n", tp_id,syna_data.vendor_id);

	tp_handler = find_tp_handler(&tp_id[6]);
	...
	retval = synaptics_rmi4_reg_read(fwu->rmi4_data,
				...
				sizeof(config_id));
	if (retval < 0) {
		printk("%s: Failed to read device config ID\n",
				__func__);
		return retval;
	}
    ...
    sprintf(version,"%s:%s:0x%02x:0x%02x",syna_tw_fw[tp_handler].tp_name,"synaS2333",config_id[1],config_id[3]);
...
//add begin by liuxinyuan to get version_id
	memset(syna_data.S2333_dev_version,0,sizeof(syna_data.S2333_dev_version));

printk(KERN_ERR"yl_syna_update_debug:syna_data.S2333_dev_version= %s success\n",syna_data.S2333_dev_version);

	retval = get_dev_version_id(syna_data.S2333_dev_version);
	if (retval < 0){
		printk("%s:get_dev_version_id failed",__func__);
	}
...
#if 0
	// *********************************yulong add begin************
	if ( register_yulong_platform_interface() ) {
		printk(KERN_ERR "Failed to register yl platform UI.\n");
		retval = -ENODEV;
		goto exit_remove_attrs;
	}
	// *********************************yulong add end************
