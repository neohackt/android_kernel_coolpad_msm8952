...
/*
//modify by liuxinyuan@yulong.com for reset device
#define FB_READY_RESET
#define FB_READY_WAIT_MS 100
#define FB_READY_TIMEOUT_S 30
///end modify
*/
#define DELAY_HARDWARE_RESET       15 //modify for reset TP when resume by liuxinyuan@yulong.com in 20151124
#define DELAY_HARDWARE_RESET_READY 150 //modify for reset TP when resume by liuxinyuan@yulong.com in 20151124
#if 0
// for 0D button //modify by liuxinyuan to add  key name in 20151011
...
#endif
//modify by liuxinyuan to add  key name end in 20151011
...
unsigned int fwu_flag = 0;      //shihaobin@yulong.com add for firmware update flag 20151211
//shihaobin@yulong.com modify chage bit address 20151214 begin
#if CONFIG_TOUCHSCREEN_CHARGER_STATUS_CHECK
unsigned int charge_bit_addr = 0;
#endif
//shihaobin@yulong.com modify chage bit address 20151214 end
...
// add by liuxinyuan@yulong.com for gesture wakeup
...
//shihaobin@yulong.com add for compatible of tp 20151121 begin
#if defined(CONFIG_BOARD_CPY72_821) || defined(CONFIG_BOARD_CPY72_921)
unsigned int syna_addr;
struct i2c_client *syna_client;
#endif
//shihaobin@yulong.com add for compatible of tp 20151121 end
...
//add by liuxinyuan for charger_check 20151126 begin
#ifdef CONFIG_TOUCHSCREEN_CHARGER_STATUS_CHECK
//#define REG_CHARGER_STATUS  0x0037
extern unsigned int touch_get_charger_status(void);
extern void touch_register_charger_notify(void (*fn)(void));
static void yl_chg_status_changed(void)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	unsigned int chg_status;
	u8 write_val=0;
	int retval;
    ////shihaobin@yulong.com add for firmware update flag 20151211 begin
    if (1 == fwu_flag){
        printk(KERN_ERR"yl_syna_debug tp is in firmware update now! So do not judgement if charging or not!!!!");
        return;
    }
    //shihaobin@yulong.com add for firmware update flag 20151211 end
	printk(KERN_ERR"yl_syna_debug:%s\n",__func__);
	...
	if(chg_status){
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					sizeof(write_val));
		if (retval < 0){
			SYNA_ERROR("yl_syna_debug%s,read error!\n",__func__);
			return;
		}
        //shihaobin@yulong.com add for not write charge bit when tp is in charge mode 20151214 begin
        if (0x20 == (write_val & 0x20)){
            printk("yl_syna_debug touch panel is already in charger mode! So return!\n");
            return;
        }
        //shihaobin@yulong.com add for not write charge bit when tp is in charge mode 20151214 end
		write_val |= (1<<5);
		printk(KERN_ERR"yl_syna_debug:charging %d\n",write_val);
		retval = synaptics_rmi4_reg_write(rmi4_data,
					charge_bit_addr,
					&write_val,
					sizeof(write_val));
		if (retval < 0){
			SYNA_ERROR("yl_syna_debug:%s,write error!\n",__func__);
			return;
		}
	}else{
		retval = synaptics_rmi4_reg_read(rmi4_data,
        ...
        //shihaobin@yulong.com add for not write disable charge bit when tp is in disable charge mode 20151214 begin
        if (0x0 == (write_val & 0x20)){
            printk("yl_syna_debug touch panel charge already disabled!So return!\n");
            return;
        }
        //shihaobin@yulong.com add for not write disable charge bit when tp is in disable charge mode 20151214 end
		write_val &= (~(1<<5));
		printk(KERN_ERR"yl_syna_debug:not charging %d\n",write_val);
		retval = synaptics_rmi4_reg_write(rmi4_data,
					charge_bit_addr,
					&write_val,
					sizeof(write_val));
...
//add by liuxinyuan for gesture_wakeup 20150601
#ifdef SYNA_GESTURE_WAKEUP
typedef enum
{
	DOZE_DISABLED = 0,
...
//shihaobin@yulong.com add for compatible of tp 20151121 begin
#if defined(CONFIG_BOARD_CPY72_821) || defined(CONFIG_BOARD_CPY72_921)
static struct slide_wakeup tp_slide_wakeup[]={
	{0x01,TW_SUPPORT_DOUBLE_CLICK_WAKEUP,{"GESTURE=DOUBLE_CLICK",NULL}, "double_click"},
	{0x08,TW_SUPPORT_O_SLIDE_WAKEUP,{"GESTURE=O",NULL}, "o"},
...
static unsigned char gesture_type=0;
#if defined(CONFIG_BOARD_CPY72_821) || defined(CONFIG_BOARD_CPY72_921)
static unsigned char gesture_data[8] = {0};
static char wakeup_slide[32];
static u32 support_gesture = TW_SUPPORT_NONE_SLIDE_WAKEUP;
#else
unsigned char gesture_data[8] = {0};
char wakeup_slide[32];
u32 support_gesture = TW_SUPPORT_NONE_SLIDE_WAKEUP;
#endif
//shihaobin@yulong.com add for compatible of tp 20151121 end

static void syna_enable_irq_wake(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
        static u8 irq_wake_status = 0;
        if(irq_wake_status != enable) {
                if(enable){
                        enable_irq_wake(rmi4_data->irq);
                        printk(KERN_INFO "SYNA:YLLOG: enable wakeup irq success.\n");
                } else {
                        disable_irq_wake(rmi4_data->irq);
                        printk(KERN_INFO "SYNA:YLLOG:disable wakeup irq success.\n");
                }
                irq_wake_status = enable;
        }
}

static void synaptics_rmi4_gesture_report(struct synaptics_rmi4_data *rmi4_data)
{
	char **envp = NULL;
	int i;
	unsigned char slide_x_abs = 0;
	unsigned char slide_y_abs = 0;
	unsigned char gesture_code = 0;
	struct device *touchscreen_dev;
printk(KERN_ERR"yl_syna_debug: %s inter gesture report\n",__func__);
	touchscreen_dev = touchscreen_get_dev();

	if(gesture_type == 0x01) {//double click
		gesture_code = 0x01;
	} else if(gesture_type == 0x08) {//slide o
		gesture_code = 0x08;
	} else if(gesture_type == 0x40) {//slide c,e,w,m
		gesture_code = gesture_data[2];
	} else if (gesture_type == 0x02) {//slide left,right,up,down
		if(gesture_data[1] & 0xFF)
			slide_x_abs = ~gesture_data[0];
		else
			slide_x_abs = gesture_data[0];
		if(gesture_data[3] & 0xFF)
			slide_y_abs = ~gesture_data[2];
		else
			slide_y_abs = gesture_data[2];

		if((gesture_data[1] & 0xFF) && (slide_y_abs < 0xA)) {
			gesture_code = 0xBB;//left
		} else if(!(gesture_data[1] & 0xFF) && (slide_y_abs < 0xA)) {
			gesture_code = 0xAA;//right
		} else if((gesture_data[3] & 0xFF) && (slide_x_abs < 0xA))  {
			gesture_code = 0xBA;//up
		} else if(!(gesture_data[3] & 0xFF) && (slide_x_abs < 0xA)) {
			gesture_code = 0xAB;//down
		}
	}
printk(KERN_ERR"yl_syna_debug: %s inter gesture support Slide(0x%x) \n",__func__,gesture_code);
	for(i = 0; i < ARRAY_SIZE(tp_slide_wakeup); i++) {
		if((gesture_code == tp_slide_wakeup[i].code) &&	\
...
*/
		...
		printk(KERN_INFO "SYNA:YLLOG:send <%s> kobject uevent!\n", wakeup_slide);
	} else {
		printk(KERN_INFO "SYNA:YLLOG:gesture not support.\n");
	}
	//syna_enter_doze(rmi4_data);
	synaptics_rmi4_f11_wg(rmi4_data, true);
	syna_enable_irq_wake(rmi4_data, true);
	doze_status = DOZE_ENABLED;

	return;
}
//add end
#endif //SYNA_GESTURE_WAKEUP

///  add by liuxinyuan for QCOM8916 20151013
struct syna_platform_data syna_data;
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data);
static int syna_tp_debug(int mode)
{
	if(mode)
	{
		syna_data.syna_debug_log = true;
		SYNA_INFO("syna_debug_log open!\n");
	}
	else
	{
		syna_data.syna_debug_log = false;
		SYNA_INFO("syna_debug_log close!\n");
	}
    return 0;
}

static int syna_tp_active(void)
{
    printk("SYNA:YLLOG:%s.\n", __FUNCTION__);
    return 1;
}

static int syna_tp_need_update(void)//TODO
{
	int retval;
    printk("SYNA:YLLOG:%s.\n", __FUNCTION__);
	retval = synaptics_rmi4_query_device(exp_data.rmi4_data);
	if (retval < 0) {
		SYNA_ERROR("query device failed!\n");
		return 0;
	}
	return syna_s2333_update_check();
}


static int syna_tp_do_update(void)//TODO
{
    printk("SYNA:YLLOG:%s.\n", __FUNCTION__);
    return syna_s2333_do_update();
}

static int syna_tp_need_calibrate(void)
{
    printk("SYNA:YLLOG:%s.\n", __FUNCTION__);
    return 0;
}

static int syna_tp_calibrate(void)
{
    printk("SYNA:YLLOG:%s.\n", __FUNCTION__);
    return 0;
}

touch_mode_type syna_tp_get_mode(void)
{
	return MODE_NORMAL;
}
...
static int synaptics_gesture_ctrl(const char* gesture_buf)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	char *gesture;
	int buf_len;
	...
	printk(KERN_INFO "SYNA:YLLOG:%sbuf_len = %d.\n", __func__, buf_len);
	printk(KERN_ERR "SYNA:YLLOG:%s gesture_buf:%s.\n", __func__, gesture_buf);

	gesture_p = kzalloc(buf_len + 1, GFP_KERNEL);
	if(gesture_p == NULL) {
		printk(KERN_INFO "%s: alloc mem error.\n", __func__);
		return -1;
	}
	temp_p = gesture_p;
	...
	if (suspend_flag) {
		if(support_gesture & TW_SUPPORT_GESTURE_IN_ALL) {
			printk(KERN_ERR "SYNA:YLLOG:Enter into gesture mode.\n");
			synaptics_rmi4_sleep_enable(rmi4_data, false); ///modify by liuxinyuan
			...
			printk(KERN_ERR "SYNA:YLLOG:Enter into deep sleep mode.\n");
			synaptics_rmi4_irq_enable(rmi4_data, false,false);
			synaptics_rmi4_sleep_enable(rmi4_data, true);  //////modify by liuxinyuan
            rmi4_data->suspend = true;
            gesture_flag = 1;
		}
	}
...
int synaptics_wakeup_gesture(char* gesture)
{
	int i;
...
int syna_tp_set_mode(touch_mode_type work_mode)
{
    return 0;
}
...
int syna_tp_get_version(char* version)
{
	int retval;
		...
		retval = get_dev_version_id(version);
		if (retval < 0)
		{
			strcpy(version,syna_data.S2333_dev_version);
		}
...
{
    return sprintf(vendor,"%s%s","synaptics_",syna_data.vendor_id);
}

#ifdef SYNA_GESTURE_WAKEUP
int syna_get_wakeup_gesture(char* gesture)
{
	return synaptics_wakeup_gesture(gesture);
}

int syna_gesture_ctrl(const char* gesture_buf)
{
	return synaptics_gesture_ctrl(gesture_buf);
}
#endif
static touchscreen_ops_tpye syna_tp_ops = {
        .touch_id               = 0,
        .touch_type             = 1,
...
#ifdef SYNA_GESTURE_WAKEUP
		.get_wakeup_gesture		= syna_get_wakeup_gesture,
		.gesture_ctrl			= syna_gesture_ctrl,
#endif
};
//add end
...
//	unsigned char detected_gestures;
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(gesture_type));
		if (retval < 0){
			printk(KERN_ERR"yl_syna_gesture_debug:Failed to get gesture_type!\n");
				return 0;
			}
//add  by liuxinyuan for gesture_wakeup in 20151102
		printk(KERN_ERR"yl_syna_gesture_debug:Gesture detected:%s,gesture_type=%d\n",__func__,gesture_type);
		if (gesture_type) {
			retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(gesture_data));
			if (retval < 0){
				SYNA_ERROR("Failed to get gesture code!\n");
				return 0;
			}
printk(KERN_ERR"yl_syna_gesture_debug:Gesture detected:%s,gesture_data=%s",__func__,gesture_data);
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			dev_dbg(rmi4_data->pdev->dev.parent,
					...
					finger_status,
					x, y, wx, wy);
/// printk(KERN_ERR"yl_syna_debug: liuxinyuan x = %d,y = %d,wx = %d, wy = %d\n", x,y,wx,wy);
			touch_count++;
		}
	}
...
{
	int retval = 0;
    unsigned char data[MAX_INTR_REGISTERS]; //shihaobin@yulong.com modify to resolve deadlock when start up 20151223
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	//shihaobin@yulong.com add mutex when irq enable 20151205 begin
	...
	if (enable) {
		...
		if (retval < 0)
         ...
         //shihaobin@yulong.com modify to resolve deadlock when start up 20151223 begin
         /* Clear interrupts */
        retval = synaptics_rmi4_reg_read(rmi4_data,
        ...
        //shihaobin@yulong.com modify to resolve deadlock when start up 20151223 end
...
exit:
	...
	//shihaobin@yulong.com add mutex when irq enable 20151205 end
	return retval;
}
		...
		unsigned int intr_count)
{
 ...
 //   rmi4_data->sensor_max_x = 540;
 //   rmi4_data->sensor_max_y = 960;
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: Function %02x max x = %d max y = %d\n",
			__func__, fhandler->fn_number,
			rmi4_data->sensor_max_x,
			rmi4_data->sensor_max_y);
    /*printk(KERN_ERR"yl_syna_debug : control_6_9.sensor_max_x_pos_7_0 = %d, control_6_9.sensor_max_x_pos_11_8 = %d\n",control_6_9.sensor_max_x_pos_7_0,control_6_9.sensor_max_x_pos_11_8);
    printk(KERN_ERR"yl_syna_debug : control_6_9.sensor_max_y_pos_7_0 = %d, control_6_9.sensor_max_y_pos_11_8 = %d\n",control_6_9.sensor_max_y_pos_7_0,control_6_9.sensor_max_y_pos_11_8);*/
    printk(KERN_ERR"yl_syna_debug :%d rmi4_data->sensor_max_x = %d, rmi4_data->sensor_max_y = %d\n",__LINE__,rmi4_data->sensor_max_x,rmi4_data->sensor_max_y);
		...
		unsigned int intr_count)
{
    ...
    //shihaobin@yulong.com modify for delete unused config 20151216 begin
	rmi4_data->stylus_enable = 0;   //ctrl_23->stylus_enable;
	rmi4_data->eraser_enable = 0;   //ctrl_23->eraser_enable;
    //shihaobin@yulong.com modify for delete unused config 20151216 end
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    //shihaobin@yulong.com modify chage bit address 20151214 begin
    #if CONFIG_TOUCHSCREEN_CHARGER_STATUS_CHECK
    charge_bit_addr = rmi4_data->f01_ctrl_base_addr;
    printk("yl_syna_debug charge_bit_addr = 0x%x\n", charge_bit_addr);
    #endif
    //shihaobin@yulong.com modify chage bit address 20151214 end
...
flash_prog_mode:
	...
	retval = secure_memcpy(rmi->product_id_string,
			...
			&f01_query[11],
			(unsigned int)(sizeof(f01_query) - 11),
			PRODUCT_ID_SIZE);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
       ...
       printk(KERN_ERR"yl_syna_debug:buf=%s\n",buf);
       printk(KERN_ERR"yl_syna_debug:GPIO=%d\n",gpio);
...
printk(KERN_ERR"yl_syna_debug:get gpio %d (code: %d)\n",gpio, retval);
...
printk(KERN_ERR"yl_syna_debug:set gpio %d direction\n",gpio);
	return retval;
}
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    printk(KERN_ERR"yl_syna_debug:%s rmi4_data->sensor_max_x = %d\n", __func__, rmi4_data->sensor_max_x);
	input_set_abs_params(rmi4_data->input_dev,
			ABS_MT_POSITION_Y, 0,
			rmi4_data->sensor_max_y, 0, 0);
    printk(KERN_ERR"yl_syna_debug:%s rmi4_data->sensor_max_y = %d\n", __func__, rmi4_data->sensor_max_y);
#ifdef REPORT_2D_W
	input_set_abs_params(rmi4_data->input_dev,
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    printk(KERN_ERR"yl_syna_debug: bdata->max_y_for_2d = %d\n", bdata->max_y_for_2d);
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    printk("yl_syna_debug: %s bdata->irq_gpio = %d, bdata->power_gpio = %d,bdata->reset_gpio = %d \n", __func__, bdata->irq_gpio, bdata->power_gpio, bdata->reset_gpio);
...
printk(KERN_ERR"yl_syna_debug:configure attention GPIO success\n");
...
printk(KERN_ERR"yl_syna_debug:configure power GPIO success\n");
...
printk(KERN_ERR"yl_syna_debug:configure reset GPIO success\n");
	if (bdata->power_gpio >= 0) {
	...
	if (bdata->reset_gpio >= 0) {
        //shihaobin@yulong.com add for pull up 20ms before reset work 20151214 begin
        gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
        msleep(bdata->reset_active_ms);
        //shihaobin@yulong.com add for pull up 20ms before reset work 20151214 end
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		msleep(bdata->reset_active_ms);
		gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
		...
		bool enable)
{
	...
	if (rmi4_data->pwr_reg) {
		...
		msleep(bdata->power_delay_ms);
// printk(KERN_ERR"yl_syna_debug:enable power regulator rmi4_data->pwr_reg=%u \n",rmi4_data->pwr_reg);
	}
...
//add by liuxinyuan@yulong.com  for hardware reset TW in 20151124
static void synaptics_rmi4_hardware_reset_tp(struct synaptics_rmi4_data *rmi4_data)
{
if (NULL == rmi4_data)
    {
     printk(KERN_ERR"yl_syna_debug:%s rmi4_data is NULL \n",__func__);
     return;
    }
	//shihaobin@yulong.com modify for delete disable int 20151205 begin
    //synaptics_rmi4_irq_enable(rmi4_data, false, false);
    ...
    //synaptics_rmi4_irq_enable(rmi4_data, true, false);
	//shihaobin@yulong.com modify for delete disable int 20151205 end
    return;
}
// add end
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data,
		...
		bool enable)
{
	...
	rmi4_data->sensor_sleep = enable;  //shihaobin modify 20151205 for set sensor_sleep mode
...
/// add by liuxinyuan for interrupt pin alway is low in 20151026
...
//add end  by liuxinyuan for interrupt pin alway is low in 20151026
//shihaobin@yulong.com modify for goodix touch not use when start 20151229 begin
#if defined(CONFIG_BOARD_CPY72_821) || defined(CONFIG_BOARD_CPY72_921)
unsigned int syna_probe_ok = 0;
#endif
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
    ...
    //shihaobin@yulong.com add for compatible of tp 20151121 begin
    #if defined(CONFIG_BOARD_CPY72_821) || defined(CONFIG_BOARD_CPY72_921)
    struct i2c_msg check_msg[1];
    unsigned char buf;

    check_msg[0].addr = syna_addr;
    check_msg[0].flags = 0;
    check_msg[0].len = 1;
    check_msg[0].buf = &buf;
    
    if(1 != i2c_transfer(syna_client->adapter,check_msg,1))
    {
        printk(KERN_ERR"%s: have no this device!\n",__func__);
        return -ENODEV;
    }
    else
    {
    ...
    #endif
    //shihaobin@yulong.com add for compatible of tp 20151121 end
//shihaobin@yulong.com modify for goodix touch not use when start 20151229 end

printk(KERN_ERR"yl_syna_debug %s  into rmi4 probe succeed\n",__func__);
    ...
    printk("yl_syna_debug: %s hw_if->board_data->irq_gpio = %d, hw_if->board_data->power_gpio = %d,hw_if->board_data->reset_gpio = %d \n", __func__, hw_if->board_data->irq_gpio, hw_if->board_data->power_gpio, hw_if->board_data->reset_gpio);
    ...
    //shihaobin@yulong.com modify for put gesture_flag in SYNA_GESTURE_WAKEUP define 20151118 begin
    #ifdef SYNA_GESTURE_WAKEUP
        gesture_flag = 0;
    #endif
    //shihaobin@yulong.com modify for put gesture_flag in SYNA_GESTURE_WAKEUP define 20151118 end
	syna_data.syna_debug_log = false;
#ifdef SYNA_GESTURE_WAKEUP 
    ...
    touchscreen_set_ops(&syna_tp_ops);     //add by liuxinyuan
...
#endif
    printk(KERN_ERR"yl_syna_debug %s  probe end\n",__func__);
		...
		bool enable)
{
	...
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
printk(KERN_ERR"yl_syna_debug: %s inter\n",__func__);
	rmi = &(rmi4_data->rmi4_mod_info);
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
  ...
  //add by liuxinyuan for gesture_wakeup 20151102 begin
		if(support_gesture & TW_SUPPORT_GESTURE_IN_ALL){
...
static int synaptics_rmi4_suspend(struct device *dev)
{
  ...
  printk(KERN_ERR"yl_syna_debug:rmi4 suapend inter\n");
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    printk("yl_syna_debug:%s inter\n",__func__);
	//shihaobin@yulong.com modify for do resume when tp is suspend mode 20151205 begin
    if (rmi4_data->stay_awake)
    {
        printk(KERN_ERR"yl_syna_debug %s not done because tp is in upgrade firmware now !\n", __func__);
        return 0;
    }
    if (!rmi4_data->suspend)
    {
        printk(KERN_ERR"yl_syna_debug %s not done because tp is in suspend now !\n", __func__);
        return 0;
    }
	//shihaobin@yulong.com modify for do resume when tp is suspend mode 20151205 end
    synaptics_rmi4_hardware_reset_tp(rmi4_data);  // add by liuxinyuan@yulong.com for hardware reset TW in 20151124
...
static int __init synaptics_rmi4_init(void)
{
	int retval;
    int retval2;
    //shihaobin@yulong.com add for close tp work only when power-off-charging and recovery mode 20151123 begin
    #ifdef CONFIG_LPM_MODE
    if(1 == poweroff_charging)
    {
        printk("%s: poweroff_charging = %d.\n", __func__, poweroff_charging);
        return 0;
    }
    #endif
    //shihaobin@yulong.com add for close tp work only when power-off-charging and recovery mode 20151027 end

    printk(KERN_ERR"yl_syna_debug %s :inter success\n",__func__);
	retval = synaptics_rmi4_bus_init();
     
	if (retval){
        printk(KERN_ERR"yl_syna_debug:rmi4 bus init fail\n");
        return retval;
    }
  
    //mdelay(100);
	
   retval2 = platform_driver_register(&synaptics_rmi4_driver);
 printk(KERN_ERR"yl_syna_debug %d:\n",retval2);
return retval2;
}
