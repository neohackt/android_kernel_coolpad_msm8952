...
int gt1x_init_tool_node(void)
{
	...
	gt1x_tool_proc_entry = proc_create(GOODIX_ENTRY_NAME, 0666, NULL, &gt1x_tool_fops);
...
static ssize_t gt1x_tool_read(struct file *filp, char __user * buffer, size_t count, loff_t * ppos)
{
	GTP_DEBUG_FUNC();
       if(*ppos)
        {
		#ifdef __LP64__
		GTP_DEBUG("[PARAM]size: %lu, *ppos: %d", count, (int)*ppos);
		#else
		GTP_DEBUG("[PARAM]size: %d, *ppos: %d", count, (int)*ppos);
	        #endif
			*ppos = 0;
            return 0;
        }
