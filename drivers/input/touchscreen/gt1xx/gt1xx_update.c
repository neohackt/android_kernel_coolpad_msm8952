...
int gt1x_update_prepare(char *filename)
{
	...
	while (retry > 0) {
		...
		if (update_info.firmware == NULL) {
			#ifdef __LP64__
			GTP_DEBUG("Alloc %lu bytes memory fail.", sizeof(struct fw_info));
			#else
			GTP_DEBUG("Alloc %d bytes memory fail.", sizeof(struct fw_info));
			#endif
			continue;
		} else {
			#ifdef __LP64__
			GTP_DEBUG("Alloc %lu bytes memory success.", sizeof(struct fw_info));
			#else
			GTP_DEBUG("Alloc %d bytes memory success.", sizeof(struct fw_info));
			#endif
			break;
		}
	}
...
void read_reg(u16 addr, int len)
{
	...
	GTP_DEBUG("Read address: 0x%04X, Length: %d", addr, len);
	while (len > 0) {
