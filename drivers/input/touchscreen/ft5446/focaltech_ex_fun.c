...
static bool fts_scap_test = false;
...
static int fts_get_testparam_from_ini(char *config_name)
{
	...
	if (fts_ReadInIData(config_name, filedata)) {
		...
		pr_info("fts_ReadInIData successful\n");
	}
...
int CSV(void)
{
	...
	if (databuf == NULL) {
		FTS_DBG("alloc data buf fail !\n");
		return -1;
	}

	//memset(databuf, 0, sizeof(databuf));
	memset(databuf, 0, 10 * 1024);
	...
	if (err1 < 0)
		FTS_DBG("write scap sample fail!\n");
	filp_close(pfile, NULL);
	set_fs(old_fs);
	kfree(databuf);
	kfree(databuf1);
...
static ssize_t fts_scaptest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	//TXT();
	//CSV();
...
static ssize_t fts_scaptest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(fts_get_testparam_from_ini(cfgname) <0)
    {
        fts_scap_test = false;
		printk(KERN_ERR "get testparam from ini failure\n");
    }
	else
    {
        printk(KERN_ERR "tp test Start...\n");
        if(true == StartTestTP())
        {
            fts_scap_test = true;
            printk(KERN_ERR "tp test pass\n");
        }
        else
        {
            fts_scap_test = false;
            printk(KERN_ERR "tp test failure\n");
        }
    }
...
/* read chip type: adr 0xA3 */
static ssize_t fts_chiptype_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t ret = 0;
    u8 chip_type = 0;
    ...
    if (fts_read_reg(fts_i2c_client, FTS_REG_ID, &chip_type) < 0)
    {
        mutex_unlock(&fts_input_dev->mutex);
        return -1;
    }

    ret = snprintf(buf, 128, "0x%x\n", chip_type);
...
#ifdef FTS_SCAP_TEST
static DEVICE_ATTR(ftsscaptest, S_IRUGO|S_IWUSR, fts_scaptest_show, fts_scaptest_store);
#endif

static DEVICE_ATTR(ftschiptype, 0444, fts_chiptype_show, NULL);
