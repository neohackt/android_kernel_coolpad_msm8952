...
//////////////////////////////////////////////Others 
static boolean AllocateMemory(void);
...
static boolean AllocateMemory(void)
{
	...
	if(NULL == g_pStoreMsgArea){
		g_pStoreMsgArea = kmalloc(1024*32, GFP_KERNEL);
		if(!g_pStoreMsgArea){
	...
	if(NULL == g_pMsgAreaLine2){
		g_pMsgAreaLine2 = kmalloc(1024*32, GFP_KERNEL);
		if(!g_pMsgAreaLine2){
	...
	if(NULL == g_pStoreDataArea){
		g_pStoreDataArea = kmalloc(1024*32, GFP_KERNEL);
		if(!g_pStoreDataArea){
