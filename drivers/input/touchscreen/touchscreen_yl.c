...
void touch_charger_status_changed(struct power_supply *psy)
{
	...
	if(psy1 == NULL)
		return;
	//mask the bit 1 of motg inputs to represent online/offline
	charger_status = power_supply_am_i_supplied(psy1) & 0x02;
...
static ssize_t  touchscreen_firmware_update_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	{
		//printk("BJ_BSP_Driver:CP_Touchscreen:string is %s,count=%lu not update!\n",buf,count);
		return -ENOMEM;
	}
...
{
       //char vendor[32]={0};
       char vendor[64]={0};/*yangmingjin modified for tw chip_id and m_id show*/
