...
#define FT_openshort_test //modify ft5346 factory TW test by jingyuantao@yulong.com in 20160308
...
extern char lcd_name[];  //modfiy corrent TW firmware for HX8398 lcd by jingyuantao@yulong.com
...
#endif
/* yulong end */
/*start jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
#ifdef CONFIG_TOUCHSCREEN_WINDOW
bool fts_window_mode_flag = false;
...
#endif
/*end jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
int result = 0;
...
#if defined(CONFIG_BOARD_CPY92_900) || defined(CONFIG_BOARD_CPY91_S00) || defined(CONFIG_BOARD_CPY91_S00_U00) || defined(CONFIG_BOARD_CP3600I)
...
{
    int err = 0;  //modify TW factory shorttest by jingyuantaoi@yulong.com
	char *filedata = NULL;
...
/*start modify TW factory shorttest by jingyuantaoi@yulong.com*/
    err = set_param_data(filedata);
    if (err < 0)
    {
        printk("set_param_data failed in function:%s\n", __func__);
        //return err;
    }
/*end modify TW factory shorttest by jingyuantaoi@yulong.com*/
	return 0;
}
	...
	const char *buf, size_t count)
{
	...
	testdata = kmalloc(1024*16, GFP_ATOMIC);//用于获取存放在库中的测试数据,注意分配空间大小.
	if(NULL == testdata)
	{
	...
	///********************************************************************************
    focaltech_test_main_init();  //modify TW shorttest init by jingyuantao@yulong.com
...
/**********************************************************************/
#define FOCALTECH_TS_GPIO_RESET	        (64 + 890)
#define FOCALTECH_TS_GPIO_IRQ		(65 + 890)
...
#ifdef CONFIG_TOUCHSCREEN_WINDOW
    unsigned int res[4] = {0}; //modify jingyuantao@yulong.com add for Y92-931 window mode debug 20160309
#endif
	int ret;
...
/*start jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
#ifdef CONFIG_TOUCHSCREEN_WINDOW
    ret = of_property_read_u32_array(np, "focaltech,config_window", res, 4);
    if (ret) {
        printk("Looking up focaltech,cover failed\n");
        return -ENODEV;
    }
    fts_cover_window.win_x_min = res[0];
    fts_cover_window.win_y_min = res[1];
    fts_cover_window.win_x_max = res[2];
    fts_cover_window.win_y_max = res[3];
    printk("cover_window (%d, %d) (%d, %d)\n",res[0],res[1],res[2],res[3]);
#endif
/*end jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
...
static int fts_ctpm_chk_upg(void)
{
	...
	if( uc_fw_ver < uc_if_ver)  //modify TW firmware upgrade by jingyuantao@yulong.com in 20160301
	{
...
#elif defined(CONFIG_BOARD_CPSK3_I01)||defined(CONFIG_BOARD_CPSK3_S00)||defined(CONFIG_BOARD_CPSK3_U00)||defined(CONFIG_BOARD_CPSK3_I01_CT)||defined(CONFIG_BOARD_SK3_01)
	if(type!=0x3B&&type!=0xA0)
		fts_ctpm_fw_upgrade_ReadVendorID(this_client, &type);
#elif defined(CONFIG_BOARD_CPY92_900) || defined(CONFIG_BOARD_CPY91_S00) || defined(CONFIG_BOARD_CPY91_S00_U00) || defined(CONFIG_BOARD_CPY91_U00) || defined(CONFIG_BOARD_CP3600I) || defined(CONFIG_BOARD_CPY92_9_CU)
	if(type!=0x80&&type!=0xA0&&type!=0x51)
		fts_ctpm_fw_upgrade_ReadVendorID(this_client, &type);
#endif
    return (type);
}
...
void fts_get_current_tp_info(int num)
{
	...
	if(0x0C==buf)//0x0C is GF structure
	{
	    index++;
	}
    }
	...
	if(0x43==buf)//0x0C is GF structure
	{
	    index++;
	}
    }
/*start modfiy current TW firmware for HX8398 lcd by jingyuantao@yulong.com*/
    if (!strncmp(lcd_name, "LCD_TYPE_HX", 11)) {
        printk("[FT]:TW firmware for HX8398!!!\n");
        current_tp_supported = true;
        current_tp_name  = "Ofilm_hx8398";
        current_fw_to_tp = CTPM_FW_OFILM_HX8398;
        current_fw_size  = sizeof(CTPM_FW_OFILM_HX8398);
    }

    else {
        printk("[FT]:TW firmware for otm1901 or ili9885!!!\n");
        current_tp_supported = true;
...
/*end modfiy current TW firmware for HX8398 lcd by jingyuantao@yulong.com*/
...
static int ft5x0x_read_data(void)
{
    ...
    ||defined(CONFIG_BOARD_CPY92_900) || defined(CONFIG_BOARD_CPY91_S00) || defined(CONFIG_BOARD_CPY91_S00_U00) || defined(CONFIG_BOARD_CPY91_U00)\
	||defined(CONFIG_BOARD_SK3_01)||defined(CONFIG_BOARD_CPSK3_U00) || defined(CONFIG_BOARD_CP3600I) || defined(CONFIG_BOARD_CPY92_9_CU)
	{
...
/*start jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
#ifdef CONFIG_TOUCHSCREEN_WINDOW
    if(fts_window_mode_flag &&
        !( (x > fts_cover_window.win_x_min)
        && (x < fts_cover_window.win_x_max)
        && (y > fts_cover_window.win_y_min)
        && (y < fts_cover_window.win_y_max)))
        return;
#endif
/*end jingyuantao@yulong.com add for Y92-931 window mode debug 20160309*/
...
static void ft5x0x_ts_suspend(struct device *dev)
{
    ...
    ft5x0x_ts_release();  //modfiy TW touch release for gesture wakeup by jingyuantao@yulong.com in 20160316
...
/*start modify when in charger status close glove mode by jingyuantao@yulong.com*/
    ...
    if(MODE_GLOVE==current_mode_flag)
    {
        ...
        if (ret < 0)
        {
            pr_err("%s write reg 0xc0 to close glove_mode is failed!\n",__func__);
        }
        else {
            pr_err("%s write reg 0xc0 to close glove_mode is success!\n",__func__);
        }
    }
        mutex_unlock(&ft5x0x_ts->device_mode_mutex);
/*end modify when in charger status close glove mode by jingyuantao@yulong.com*/
	mutex_lock(&input_dev->mutex);
...
void glove_windows_switch(int in_hall)
{
    ...
    if (pre_hall_state)
    {
        fts_window_mode_flag = true;
        printk("[FT]:entry window mode success !!!\n");
    }
    else {
         fts_window_mode_flag = false;
    }
    mutex_unlock(&ft5x0x_ts->device_mode_mutex);
#endif
    return;
...
//		return 1;      //changed by taokai
	...
	if( uc_fw_ver < uc_if_ver || debug_flag) //modify TW firmware upgrade by jingyuantao@yulong.com in 20160301
		return 1;
	else
		return 0;
 }
...
int focaltech_gesture_ctrl(const char*  gesture_buf)
{
    ...
    if(0==backlight_on)
    {
/*start modify TW suspend and resume status gesture wakeup by jingyuantao@yulong.com*/
        disable_irq_nosync(this_client->irq);
        if(ft5x0x_ts->pdata->resume)
            ft5x0x_ts->pdata->resume();
        if (support_gesture & TW_SUPPORT_GESTURE_IN_ALL) {
	...
	kfree(temp_p);
    mutex_unlock(&input_dev->mutex);
/*end modify TW suspend and resume status gesture wakeup by jingyuantao@yulong.com*/
#endif
	return 1;
}
...
static int tw_poweroff_notifier_call(struct notifier_block *this,
...
static struct notifier_block tw_poweroff_notifier = {
    .notifier_call = tw_poweroff_notifier_call,
};

void fts_ctpm_hw_reset(void)
{
    if(gpio_is_valid(FOCALTECH_TS_GPIO_RESET))
    {
        gpio_direction_output(FOCALTECH_TS_GPIO_RESET, 1);
        mdelay(10);
        gpio_direction_output(FOCALTECH_TS_GPIO_RESET, 0);
        ...
        pr_err("FOCALTECH_TS_GPIO_RESET is invalid!\n");
    }
}
...
int fts_ctpm_fw_preupgrade_hwreset(struct i2c_client * client)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        for(j = 0; j < 3; j++)
        {
            i_ret = ft5x0x_read_reg(0xA8, &tw_on_test);
            if (i_ret >= 0)
                break;
        }
        if (j == I2C_ERR_RETRY) {
    ...
    ftxxxx_i2c_Write(client, packet_buf, 8 + 6);
    ...
    focaltech_i2c_Write(client, packet_buf, 8 + 6);
    }
...
//    printk("TOUCH:maleijie:probe start\n");   //changed by taokai
    ...
    if (!ft5x0x_ts)
    {
        printk("[FT]:ft5x0x_ts kzalloc fail !!!\n");
        ret = -ENOMEM;
...
#if defined(CONFIG_BOARD_CPSK3_I01) || defined(CONFIG_BOARD_CPSK3_S00)|| defined(CONFIG_BOARD_CPSK3_U00)||defined(CONFIG_BOARD_CPY92_900) || defined(CONFIG_BOARD_CP3600I) || defined(CONFIG_BOARD_CPY92_9_CU) || defined(CONFIG_BOARD_CPY91_S00) || defined(CONFIG_BOARD_CPY91_S00_U00) || defined(CONFIG_BOARD_CPY91_U00)
...
*/
    if (fts_ctpm_fw_preupgrade_hwreset(client) < 0){
		pr_err("%s fail!IC offline\n",__func__);
        goto exit_reset;
    }
...
#endif
        register_reboot_notifier(&tw_poweroff_notifier);

    ret = sysfs_create_group(&client->dev.kobj, &ft5x0x_attribute_group);//maleijie
	if(ret<0)
	{
