...
extern void free_struct_DetailThreshold(void);
extern void malloc_struct_DetailThreshold(void);

#define FTS_DRIVER_LIB_INFO  "Test_Lib_Version  V1.5.8 2016-03-07"
...
int set_param_data(char * TestParamData)
{
    ...
    err = ini_get_key_data(g_testparamstring);//get param to struct
    if (err < 0)
    {
        printk("ini_get_key_data failed in function:%s\n", __func__);
        //return err;
    }
