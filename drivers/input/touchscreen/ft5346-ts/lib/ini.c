...
int init_key_data(void)
{
	...
	if(NULL == g_st_ini_file_data)
		g_st_ini_file_data = kmalloc(sizeof(ST_INI_FILE_DATA)*MAX_KEY_NUM, GFP_ATOMIC);
...
//		return CFG_ERR_OPEN_FILE; 
    ...
    err = init_key_data();/*init*/
    if (err < 0)
    {
        printk("init_key_data failed in function:%s\n", __func__);
        return err;
    }
	...
	while(1) {/*搜找项section */
		...
		if(buf1[0] == CFG_SSL) {	
			...
			if(MAX_KEY_NAME_LEN < g_st_ini_file_data[g_used_key_num].iSectionNameLen)
			{
				...
				FTS_TEST_DBG("MAX_KEY_NAME_LEN: CFG_ERR_OUT_OF_LEN, iSectionNameLen = %d\n\n",
		...
		if(MAX_KEY_NAME_LEN < g_st_ini_file_data[g_used_key_num].iKeyNameLen)
			{
				ret = CFG_ERR_OUT_OF_LEN;
				FTS_TEST_DBG("MAX_KEY_NAME_LEN: CFG_ERR_OUT_OF_LEN, iKeyNameLen = %d\n\n",
