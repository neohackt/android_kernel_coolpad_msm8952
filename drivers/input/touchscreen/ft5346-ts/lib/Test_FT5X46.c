...
boolean FT5X46_StartTest()
{
  ...
  bool test_result = true;
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		{
            for (test_time = 0; test_time < TEST_ERR_RETRY; test_time++){
	...
	//--------------4. return result
	return bTestResult & test_result;
}
/************************************************************************
* Name: InitTest
...
static void AllocateMemory(void)
{
	...
	if(NULL == g_pStoreMsgArea){
        g_pStoreMsgArea = kmalloc(1024*4, GFP_ATOMIC);
		if(!g_pStoreMsgArea){
			printk(KERN_ERR "%s: alloc memory page fail 01\n",__func__);
			goto err_pStoreMsgArea;
		}
	}
	g_pMsgAreaLine2 =NULL;	
	if(NULL == g_pMsgAreaLine2){
        g_pMsgAreaLine2 = kmalloc(1024*4, GFP_ATOMIC);
		if(!g_pMsgAreaLine2){
			printk(KERN_ERR "%s: alloc memory page fail 02\n",__func__);
			goto err_pMsgAreaLine2;
		}
	}
	g_pStoreDataArea =NULL;	
	if(NULL == g_pStoreDataArea){
        g_pStoreDataArea = kmalloc(1024*4, GFP_ATOMIC);
		if(!g_pStoreDataArea){
			printk(KERN_ERR "%s: alloc memory page fail 03\n",__func__);
			goto err_pStoreDataArea;
		}
	}
	...
	if(NULL == g_pTmpBuff){
		...
		if(!g_pTmpBuff){
			printk(KERN_ERR "%s: alloc memory page fail 04\n",__func__);
			goto err_pTmpBuff;
		}
	}
	return;
