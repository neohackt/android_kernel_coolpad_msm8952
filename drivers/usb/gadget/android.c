...
#ifdef CONFIG_USB_YL_CHARGE_ONLY
#include <linux/usb/g_hid.h>  ///charge only HID device.added by yl linronghui. 2013.11.05
#endif
...
#ifdef CONFIG_USB_YL_CHARGE_ONLY
#include "f_hid.c"   //HID for charge only mode. linronghui. 2013.11.05
#endif
#include "f_ccid.c"
#include "f_mtp.c"
...
/*-------------------------------------------------------------------------*/
/* Supported hid functions initialization. linronghui. 2013.11.05. start*/

#ifdef CONFIG_USB_YL_CHARGE_ONLY
static struct hidg_func_descriptor coolpad_hid_desc = {
...
static int
hid_function_init(struct android_usb_function *f,
		struct usb_composite_dev *cdev)
{
	f->config = kzalloc(sizeof(struct hid_data), GFP_KERNEL);
	if (!f->config)
		return -ENOMEM;
...
static int
hid_function_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
	return hidg_bind_config(c,&coolpad_hid_desc,0);
}

static void hid_android_function_enable(struct android_usb_function *f)
{
	struct hid_data *data = f->config;

	data->enabled = true;

}

static void hid_android_function_disable(struct android_usb_function *f)
{
	struct hid_data *data = f->config;
...
/*
int hid_ctrlrequest(struct android_usb_function *hid_func,
                    struct usb_composite_dev *composite_dev,
                    const struct usb_ctrlrequest *ctrlreq)
{
     return hidg_setup(hid_func, ctrlreq);
}*/

static struct android_usb_function hid_function = {
	.name		= "hid",
	.enable		= hid_android_function_enable,
	.disable	= hid_android_function_disable,
	.init		= hid_function_init,
	.cleanup	= hid_function_cleanup,
	.bind_config	= hid_function_bind_config,
 //       .ctrlrequest    = hid_ctrlrequest,
};
#endif

/* Supported hid functions initialization. linronghui. 2013.11.05. end*/
/*-------------------------------------------------------------------------*/
...
#ifndef CONFIG_USB_YL_CHARGE_ONLY
/* Charger */
