...
//#define P61_ERR_MSG(msg...)  printk(KERN_ALERT "[NXP-P61] :  " msg);

/*
#define P61_ERR_MSG(msg...)  \
        switch(debug_level)      \
        {                        \
        case P61_DEBUG_OFF:      \
        break;                 \
        case P61_FULL_DEBUG:     \
        printk(KERN_ALERT "[NXP-P61] :  " msg); \
        break; \
        default:                 \
        ...
        unsigned long arg)
{
    ...
    printk("rbm========== Enter p61_dev_ioctl========= cmd %u arg = %ld\n", cmd, arg);
    P61_ERR_MSG(KERN_ALERT "p61_dev_ioctl-Enter %u arg = %ld\n", cmd, arg);
    ...
    case P61_SET_PWR:
        ...
        {
#ifdef P61_HARD_RESET
            P61_ERR_MSG(KERN_ALERT " Disabling p61_regulator");
            if (p61_regulator != NULL)
            {
                regulator_disable(p61_regulator);
                msleep(50);
               ret = regulator_enable(p61_regulator);
                P61_ERR_MSG(KERN_ALERT " Enabling p61_regulator");
            }
            else
            {
			...
			P61_ERR_MSG(KERN_ALERT " Soft Reset");
    ...
    case P61_SET_DBG:
        ...
        P61_ERR_MSG(KERN_ALERT"[NXP-P61] -  Debug level %d", debug_level);
        break;
    ...
    case P61_SET_POLL:
        ...
        {
            P61_ERR_MSG(KERN_ALERT"[NXP-P61] - IRQ Mode is set \n");
        }
        else
        {
            P61_ERR_MSG(KERN_ALERT"[NXP-P61] - Poll Mode is set \n");
        ...
        loff_t *offset)
{
    ...
    P61_ERR_MSG(KERN_ALERT "p61_dev_write -Enter count %d\n", (int)count);
    ...
    /* Write data */
	//ruanbanmao add for debug.
	//gpio_set_value(p61_dev->cs_gpio, 0);
    ret = spi_write(p61_dev->spi, &tx_buffer[0], count);
    //ruanbanmao add for debug.
	//gpio_set_value(p61_dev->cs_gpio, 1);
    ...
    mutex_unlock(&p61_dev->write_mutex);
    P61_ERR_MSG(KERN_ALERT "p61_dev_write ret %d- Exit \n", ret);
    return ret;
}
        ...
        loff_t *offset)
{
    ...
    P61_ERR_MSG("p61_dev_read count %d - Enter \n", (int)count);
    ...
    {
        P61_ERR_MSG(" %s Poll Mode Enabled \n", __FUNCTION__);

        do
        {
            sof = 0x00;
            P61_ERR_MSG(KERN_ALERT"rbm -----> SPI_READ [1] returned 0x%x\n", sof);
            ...
            P61_ERR_MSG(KERN_ALERT"rbm -----> SPI_READ [2] returned 0x%x\n", sof);
    ...
    {
#ifdef P61_IRQ_ENABLE
        P61_ERR_MSG(" %s Interrrupt Mode Enabled \n", __FUNCTION__);
        if (!gpio_get_value(p61_dev->irq_gpio))
        {
            ...
            while (1)
            {
                P61_ERR_MSG(" %s waiting for interrupt \n", __FUNCTION__);
    ...
    P61_ERR_MSG(" %s P61_IRQ_ENABLE not Enabled \n", __FUNCTION__);
		...
		if ((0 > ret) || (sof != SOF))
        {
            P61_ERR_MSG(KERN_ALERT"SPI_READ returned 0x%x", sof);
    ...
    /* Get the data length */
    count = rx_buffer[2];
    P61_ERR_MSG(KERN_ALERT"Data Lenth = %d", (int)count);
    ...
    P61_ERR_MSG(KERN_ALERT"total_count = %d", total_count);
    ...
    ret = total_count;
    P61_ERR_MSG("p61_dev_read ret %d Exit\n", ret);
       ...
       struct p61_dev *p61_dev, struct spi_device *spi)
{
    ...
    p61_regulator = regulator_get( &spi->dev, "pm8950_l5");
   // p61_regulator = regulator_get( &spi->dev, "nfc_voltage");
    ...
    {
        P61_ERR_MSG("successfully got regulator\n");
    }
    ...
    {
       ret = regulator_enable(p61_regulator);
        P61_ERR_MSG("successfully set regulator voltage\n");
    ...
    struct p61_spi_platform_data *data)
{
    ...
    int errorno = 0;
        ...
        if ((!gpio_is_valid(data->rst_gpio)))
                return -EINVAL;
		//ruanbanmao add for debug CS.
		/*data->cs_gpio = of_get_named_gpio(np, "nxp,p61-cs", 1);
