...
static struct sensors_classdev sensors_n2dm_cdev = {
         .name = "accelerometer",
         .vendor = "n2dm",
         .version = 1,
...
static ssize_t attr_get_vendor(struct device *dev,
                struct device_attribute *attr,  char *buf)
{
        return sprintf(buf,"n2dm\n");
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(click_timewindow, 0664, attr_get_click_tw, attr_set_click_tw),
        ...
        __ATTR(vendor, 0444, attr_get_vendor, NULL),
...
static long n2dm_acc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case N2DM_IOCTL_APP_CALIBRATE:
		...
		/* write to flash */
		sensparams_write_to_flash(SENSPARAMS_TYPE_ACCEL,
	...
	case N2DM_IOCTL_APP_OFFSET:
		...
		sensparams_read_from_flash(SENSPARAMS_TYPE_ACCEL,
			(unsigned char *)&acc_cal_data, sizeof(struct acc_offset));
...
static struct file_operations n2dm_acc_fops = {	
    ...
    //shihaobin@yulong.com add for compatible between 32bit and 64bit 20151119 begin
    #ifdef CONFIG_COMPAT
    .compat_ioctl   = n2dm_acc_ioctl,
    #endif
    //shihaobin@yulong.com add for compatible between 32bit and 64bit 20151119 end
		...
		const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "%s: probed\n", N2DM_ACC_DEV_NAME);
        ...
        printk(KERN_EMERG "register n2dm start 1 \n");

        #ifdef CONFIG_SENSORS
        /* Register to sensors class */
          printk(KERN_EMERG "register n2dm start 2\n");
        err = sensors_classdev_register(&client->dev, &sensors_n2dm_cdev);
        if (err) {
                printk(KERN_EMERG "%s: Unable to register n2dm to sensors class: %d\n",
                                __func__, err);
               sensors_classdev_unregister(&sensors_n2dm_cdev);
         }
        #endif
...
//shihaobin@yulong.com add for low power management 20151123 begin
#ifdef CONFIG_LPM_MODE
extern unsigned int poweroff_charging;
extern unsigned int recovery_mode;
#endif
//shihaobin@yulong.com add for low power management 20151123 end
