...
/* 0: i2c retry OK; 1: i2c retry FAIL; */
...
static int pa12200001_fast_run_calibration(struct i2c_client *client)
{
    ...
    /* Max Value is 150,and must be grater than x-talk cal value */
    if((xtalk_data>=data->crosstalk) && (xtalk_data < (data->crosstalk + this_data->pdata->pa12_fast_cal_tolerance))) {
        APS_LOG("%s: sum_of_pdata = %d   cross_talk = %d\n",
...
static int pa12200001_parse_dt(struct device *dev, struct pa12200001_platform_data *pdata)
{
    ...
    pdata->gpio_int = of_get_named_gpio_flags(np, "pa12200001,gpio_int", 0, &pdata->irq_gpio_flags);

    rc = of_property_read_u32(np, "pa12200001,prox_version", &pdata->prox_version);
    if (rc) {
        printk(KERN_ERR "%s: parse prox_version fail\n", __func__);
        return -ENODEV;
    }
...
/* para 1: node name; para 2: permission; para 3: permission function of read; para 4: permission function of write; */
static DEVICE_ATTR(readflash, S_IRUGO, pa12200001_show_flash, NULL);
...
#ifdef SENSORS_IRQ_WAIT_QUEUE
    int ret;
    ret = wait_event_interruptible(wait_sensors_resume, sensors_resume == 1);
    if (ret < 0) {
        APS_ERR("%s: wait_event_interruptible fail\n", __func__);
        return;
    }
#endif
...
static long pa12200001_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case PA12_IOCTL_PROX_ON:
            ...
            // temp is not exit the first time
            if(((temp_flag==1)&&(first_open==1))||((temp_flag != 1)&&(temp_flag == val))) {
                if(temp_flag!=3) {
        ...
        case PA12_IOCTL_PROX_CALIBRATE:
            APS_LOG("%s: PA12_IOCTL_PROX_CALIBRATE\n", __func__);
            ...
            cal_tmp[4] = (unsigned short) this_data->pdata->prox_version;
            ...
            if (copy_to_user((unsigned short*)arg, cal_tmp, sizeof(cal_tmp))) {
                printk(KERN_ERR "%s: calibration copy_to_user() fail\n", __func__);
                return -EFAULT;
            } else {
                printk(KERN_ERR "%s: calibration copy_to_user() success\n", __func__);
            }

            break;

        case PA12_IOCTL_PROX_OFFSET:
            APS_LOG("%s: PA12_IOCTL_PROX_OFFSET\n", __func__);

            sensparams_read_from_flash(SENSPARAMS_TYPE_PROX, prox_param, 5);

            printk(KERN_ERR "%s: this_data->pdata->prox_version = %d, prox_param[4] = %d\n",__func__, this_data->pdata->prox_version, prox_param[4]);

            if ((this_data->pdata->prox_version) != prox_param[4]) {
                prox_update = 1;
                printk(KERN_ERR "%s: need for recalibration\n", __func__);
            } else {
                prox_update = 0;
                printk(KERN_ERR "%s: same version and not recalibration\n", __func__);
            }

            ret = copy_to_user((unsigned long*)arg, &prox_update, sizeof(unsigned long));
            if (ret) {
                printk(KERN_ERR "%s: offset copy_to_user() fail,ret = %d\n", __func__, ret);
                ret = -EFAULT;
            }
...
static int /*__devinit*/ pa12200001_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    set_bit(EV_MSC, data->light_input_dev->evbit);
    input_set_capability(data->light_input_dev, EV_MSC, MSC_SCAN);
    ...
    /* create class node */
    ret = sysfs_create_group(&client->dev.kobj, &pa12200001_attr_group);
    if (ret) {
    ...
    if (!als_polling || !ps_polling)
    {
        ...
        data->als_ps_int = gpio_to_irq(pdata->gpio_int);

        err = request_irq(data->als_ps_int,
                          pa12200001_irq,
