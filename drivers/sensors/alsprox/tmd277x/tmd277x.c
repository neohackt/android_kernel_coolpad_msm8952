...
//#include <mach/gpio.h>//del by hugh 
...
//#include <asm/mach/time.h> // del by hugh
...
#include <linux/of_gpio.h>
	...
	#define TAOS_DBG(fmt, args...) pr_info(fmt, ##args)
#else
	#define TAOS_DBG(fmt, args...)
#endif
...
static long taos_ioctl_compat(struct file *file, unsigned int cmd, unsigned long arg) {
    int ret = 0;
    ...
    switch (cmd) {
        case TAOS_IOCTL_ALS_ON32:
            TAOS_DBG("%s: ALS_ON Entry\n", __func__);
            tmd277x_dev->als_enabled = 1;
            tmd277x_dev->als_on = 1;
	    
	    tmd277x_ftm = yl_get_ftm();
	    printk(KERN_ERR "TMD277x light 32bit tmd277x_ftm= %d\n",tmd277x_ftm);
        /* update tmd277x power state, modify by Jay.HF 2012-12-21 */
            ...
            TAOS_DBG("%s: ALS_ON Exit\n", __func__);
            break;

        case TAOS_IOCTL_ALS_OFF32:
            TAOS_DBG("%s: ALS_OFF Entry\n", __func__);
            tmd277x_dev->als_enabled = 0;
            tmd277x_dev->als_on = 0;
            /* update tmd277x power state, modify by Jay.HF 2012-12-21 */
        #ifdef CONFIG_YULONG_MODULE_MANAGEMENT
            ...
            TAOS_DBG("%s: ALS_OFF Exit\n", __func__);
            break;

        case TAOS_IOCTL_PROX_ON32:
            TAOS_DBG("%s: PROX_ON Entry\n", __func__);
            ...
            TAOS_DBG("%s: PROX_ON Exit\n", __func__);
            break;

        case TAOS_IOCTL_PROX_OFF32:
            TAOS_DBG("%s: PROX_OFF Entry\n", __func__);
            tmd277x_dev->prox_enabled = 0;
            tmd277x_dev->prox_on = 0;
        /* update tmd277x power state, modify by Jay.HF 2012-12-21 */
        #ifdef CONFIG_YULONG_MODULE_MANAGEMENT
            ...
            TAOS_DBG("%s: PROX_OFF Exit\n", __func__);
            break;

        case TAOS_IOCTL_PHONE_STATE32:
            TAOS_DBG("%s: PHONE_STATE!\n", __func__);
        ...
        case TAOS_IOCTL_PROX_CALIBRATE32:
            TAOS_DBG("%s: PROX_CALIBRATE\n", __func__);
        ...
        case TAOS_IOCTL_PROX_OFFSET32:
            TAOS_DBG("%s: PROX_OFFSET\n", __func__);
            if (((struct prox_offset *)arg == NULL ) || \
...
static long taos_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
	    ...
	    tmd277x_ftm = yl_get_ftm();
	    printk(KERN_ERR "TMD277x light tmd277x_ftm= %d\n",tmd277x_ftm);
...
static struct file_operations tmd277x_fops = {
    ...
    #ifdef CONFIG_COMPAT
    .compat_ioctl = taos_ioctl_compat,
    #endif
