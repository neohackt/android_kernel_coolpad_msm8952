...
#define SENSORS_STEP_C_UPDATE//yulong chenshuanglong add for data updata
...
enum hrtimer_restart lsm6ds3_poll_function_read_acc(struct hrtimer *timer)
{
...
enum hrtimer_restart lsm6ds3_poll_function_read_gyr(struct hrtimer *timer)
{
	struct lsm6ds3_sensor_data *sdata;
...
int lsm6ds3_set_drdy_irq(struct lsm6ds3_sensor_data *sdata, bool state)
{
	...
	case LSM6DS3_STEP_COUNTER:/*YuLong add for step counter 20151224*/
...
#ifdef SENSORS_STEP_C_UPDATE
static void lsm6ds3_step_c_updata(struct lsm6ds3_sensor_data *sdata)
{
	u16 steps_c;
	int err;
	...
	err = lsm6ds3_get_step_c_data(sdata, &steps_c);
	if (err < 0) {
		pr_err("%s: error while reading step counter data\n", __func__);
		return;
	}
	
	lsm6ds3_report_single_event(sdata,
			steps_c,
			sdata->timestamp);
	pr_err("%s: updata step_c = %u\n", __func__,steps_c);
}
#endif
...
static int lsm6ds3_enable_sensors(struct lsm6ds3_sensor_data *sdata)
{
	...
	case LSM6DS3_GYRO:
		...
		if (err < 0)
			return err;
		/*YuLong add start for accel&gyro gain 20151224*/
		if (sdata->sindex == LSM6DS3_ACCEL)
			err = lsm6ds3_set_fs(sdata, lsm6ds3_fs_table[0].fs_avl[1].gain);
		if (err < 0)
			return err;

		if (sdata->sindex == LSM6DS3_GYRO)
			err = lsm6ds3_set_fs(sdata, lsm6ds3_fs_table[1].fs_avl[3].gain);
		if (err < 0)
			return err;
		/*add end*/
...
#ifdef SENSORS_STEP_C_UPDATE
static int lsm6ds3_enable_steps(struct lsm6ds3_data *cdata)
{
	int err;
	struct lsm6ds3_sensor_data *sdata;
	sdata = &cdata->sensors[LSM6DS3_STEP_COUNTER];
	...
	pr_err("%s: LSM6DS3_ENABLE_STEPS Enter\n", __func__);
	...
	pr_err("%s: LSM6DS3_ENABLE_STEPS End\n", __func__);
...
static int lsm6ds3_init_sensors(struct lsm6ds3_data *cdata)
{
	...
	cdata->sensors[LSM6DS3_ACCEL].hr_timer.function =
						&lsm6ds3_poll_function_read_acc;
	cdata->sensors[LSM6DS3_GYRO].hr_timer.function =
						&lsm6ds3_poll_function_read_gyr;
	...
	/*YuLong add start for step counter 20151224*/
	default_reg_value = 0x8E;/*4G, 14*32mg=448mg*/
	err = sdata->cdata->tf->write(sdata->cdata,
					0x0F,
					1,
					&default_reg_value, false);
	...
	default_reg_value = 0x5F;/*DEB_TIME=0x0B(11*80ms=880ms), DEB_STEP=0x07(7Step)*/
	err = sdata->cdata->tf->write(sdata->cdata,
					0x14,
					1,
					&default_reg_value, false);
...
static int lsm6ds3_set_odr(struct lsm6ds3_sensor_data *sdata, u32 odr)
{
	...
	if (1/*sdata->enabled*/) {/*YuLong modify for step counter 20151224*/
				...
				size_t count)
{
	...
	if (!(err < 0)) {
		...
		sdata->ktime = ktime_set(0, MS_TO_NS(polling_rate-1));
	}
...
static ssize_t lsm6ds3_debug_regs(struct device *dev,
			struct device_attribute *attr, char *buf)
{
        int i;
        ssize_t err;
	...
	for (i = 0; i < 108; i++) {
		err = sdata->cdata->tf->read(sdata->cdata, i, 1, &data[i], true);
		if (err < 0) {
	...
	for (i = 0; i < 108; i++) {
		ptr += sprintf(ptr, "0x%02X=0x%02X ", i, data[i]);
		if ((i % 8) == 7)
			ptr += sprintf(ptr, "\n");
	}
	ptr += sprintf(ptr, "\nPrint the regs for debug.\n");

	return (ptr - buf);
}
static DEVICE_ATTR(enable, 0666/*S_IWUSR | S_IRUGO*/, get_enable, set_enable);
static DEVICE_ATTR(sampling_freq, 0666/*S_IWUSR | S_IRUGO*/, get_sampling_freq,
							set_sampling_freq);

static DEVICE_ATTR(polling_rate, 0666/*S_IWUSR | S_IRUGO*/, get_polling_rate,
							set_polling_rate);

static DEVICE_ATTR(reset_steps, 0666/*S_IWUSR*/, NULL, reset_steps);
static DEVICE_ATTR(max_delivery_rate, 0666/*S_IWUSR | S_IRUGO*/, get_max_delivery_rate,
							set_max_delivery_rate);
static DEVICE_ATTR(debug_regs, 0664/*S_IWUSR | S_IRUGO*/, lsm6ds3_debug_regs, NULL);
static DEVICE_ATTR(sampling_freq_avail, 0666/*S_IRUGO*/, get_sampling_frequency_avail, NULL);
static DEVICE_ATTR(scale_avail, 0666/*S_IRUGO*/, get_scale_avail, NULL);
static DEVICE_ATTR(scale, 0666/*S_IWUSR | S_IRUGO*/, get_cur_scale, set_cur_scale);
...
#else
	cdata->drdy_int_pin = of_get_named_gpio_flags(np, "st,drdy-int-pin", 0, NULL);
	if (gpio_request_one(cdata->drdy_int_pin, GPIOF_IN, "lsm6ds3_drdy_int"))
		dev_err(cdata->dev, "failed to request drdy interrupt gpio.\n");
#endif
	return 0;
}
...
int lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq, u16 bustype)
{
	...
	if (irq >= 0) {
		...
		if (err < 0)
			return err;
                irq = gpio_to_irq(cdata->drdy_int_pin);
#else /* CONFIG_OF */
	...
	#ifdef SENSORS_STEP_C_UPDATE	
	lsm6ds3_enable_steps(cdata);//chenshuanglong add for enable steps after restart
	#endif
...
int lsm6ds3_common_suspend(struct lsm6ds3_data *cdata)
{
	...
	for (i = 0; i < LSM6DS3_SENSORS_NUMB; i++){
		if (i != LSM6DS3_STEP_COUNTER) {
		sdata = &cdata->sensors[i];
		sdata->on_before_suspend = sdata->enabled;
		if (sdata->enabled) {
