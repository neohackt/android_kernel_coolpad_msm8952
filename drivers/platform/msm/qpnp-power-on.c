...
#ifdef CONFIG_YL_DISABLE_PON_RESIN_WAKEUP
extern unsigned int voldown_irq;
#endif
...
int qpnp_pon_set_restart_reason(enum pon_restart_reason reason)
{
	...
	rc = qpnp_pon_masked_write(pon, QPNP_PON_SOFT_RB_SPARE(pon->base),
					PON_MASK(4, 0), reason);//bit0 --bit4(5 bit),modify by Aliang 15.07.17
#else
	rc = qpnp_pon_masked_write(pon, QPNP_PON_SOFT_RB_SPARE(pon->base),
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	#ifdef CONFIG_YL_KICKOFF_REBOOT_BACKLIGHT_SET
		printk(KERN_ERR "yangguang1 pon_rt_sts = %u \n",pon_rt_sts);
		if (pon_rt_bit == 1)
		{
			if (pon_rt_sts == 1)
				power_status = 1;
...
EXPORT_SYMBOL(power_key_status);
#endif
...
#ifdef CONFIG_YL_POWER_ON_REASON
	//yulong debug:change the 0x85A to 1 for longpress RESIN to download   
