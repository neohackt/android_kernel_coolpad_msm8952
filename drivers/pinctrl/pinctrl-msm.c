...
#ifdef CONFIG_YL_PM_DEBUG_PINCTRL
#include "msm8916_gpiocfg.h"
#endif
...
#ifdef CONFIG_YL_PM_DEBUG_PINCTRL	
struct msm_pinctrl_dd *ext_msm_pinctrl_dd;
#endif
...
#ifdef CONFIG_YL_PM_DEBUG_PINCTRL
int msm_pconf_all_pins(bool to_suspend)
{
        ...
        dd = ext_msm_pinctrl_dd;
        pindesc = dd->msm_pindesc;


        for (cnt = 0; cnt < dd->pctl_dev->desc->npins; cnt++){
                pinfo = pindesc[cnt].pin_info;
                //pr_info("=========cnt = %d and num_pins = %d need_config = %d\n",cnt, dd->pctl_dev->desc->npins, act_sus_configs[cnt].need_config);
                if(act_sus_configs[cnt].reconfig_func)
                        pinfo->prg_func(cnt, act_sus_configs[cnt].func, true, pinfo);
                if(act_sus_configs[cnt].reconfig_config)
                        msm_pconf_set(dd->pctl_dev, cnt, to_suspend ? act_sus_configs[cnt].suspend:act_sus_configs[cnt].active);
