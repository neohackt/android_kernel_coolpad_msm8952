...
int  yl_gpio_get_out(struct gpio_chip *gc,unsigned offset)
{
	return msm_tlmm_gp_get(gc, offset);
}

int  yl_gpio_get(struct gpio_chip *gc, unsigned offset)
{
	int rc;
	struct msm_pintype_info *pinfo = gc_to_pintype(gc);
	void  __iomem *cfg_reg = TLMM_GP_CFG(pinfo, offset);
	//pr_info("%s[%d]offset %d  label %s pinfo = %p and regbase = %p cfg_reg = %p\n",__func__, __LINE__, offset, gc->label, pinfo, pinfo->reg_base, cfg_reg);
	rc = readl_relaxed(cfg_reg) &  0x3FF;
	mb();
	return rc;
}
#endif
