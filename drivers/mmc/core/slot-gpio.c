...
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
int SD_card_slot_present = 0;
#endif

#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION4
int SD_card_slot_present = 0;
extern int YL_sdhci_msm_setup_vreg_L11_L12(void);
#endif
...
static irqreturn_t mmc_gpio_cd_irqt(int irq, void *dev_id)
{
	...
	if (status ^ ctx->status) {
		printk(KERN_ERR"%s: slot status change detected (%d -> %d), GPIO_ACTIVE_%s\n",
		...
		#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
		if(0 == !!status) {
			SD_card_slot_present = 0;   //No Slot insert.
		}
		else SD_card_slot_present = 1;
		#endif
		...
		if(0 == !!status) {
			SD_card_slot_present = 0;   //No Slot insert.
			ret = YL_sdhci_msm_setup_vreg_L11_L12();
                       if(ret)
		        printk(KERN_ERR"power off l11 and l12 fail %d \n",ret);
                       else
		        printk(KERN_ERR"power off l11 and l12 success %d \n",ret);
		}		
		else SD_card_slot_present = 1;
		#endif
