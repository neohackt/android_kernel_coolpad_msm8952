...
static  unsigned char other_vendor[] = "vendor Not Samsung";
...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	case 1: /* MMC v1.4 */
		...
		if(card->cid.manfid == 0x15)					// wangjun8_3
			{
				pr_info("wangjun8_mmc_decode_cid()	-	card->cid.manfid = 0x%d, (Samsung)\n\n",card->cid.manfid);
				emmc_vendor = samsung_vendor;
			}
		else if(card->cid.manfid == 0x90)					// wangjun8_3
			{
				pr_info("wangjun8_mmc_decode_cid()	-	card->cid.manfid = 0x%d, (Hynix)\n\n",card->cid.manfid);
				emmc_vendor = Hynix_vendor;
			}
		else
			{
				
				pr_info("wangjun8_mmc_decode_cid()	-	card->cid.manfid = 0x%d, (Not Samsung)\n\n",card->cid.manfid);
	...
	case 4: /* MMC v4 */
		...
		if(card->cid.manfid == 0x15)					// wangjun8_4
			{
				pr_info("wangjun8_mmc_decode_cid()	-	card->cid.manfid = 0x%d, (Samsung)\n\n",card->cid.manfid);
				emmc_vendor = samsung_vendor;
			}
...
MMC_DEV_ATTR(emmc_information, "\n\nEMMC_Info as followed:\n"
	...
	card->cid.prv,						
	card->cid.serial,					
	card->cid.year,		card->cid.month		
	);

/*
MMC_DEV_ATTR(emmc_information, "\n\nEMMC_Info as followed:\n"	
	"EMMC_Vendor	:	%s	(Manufactor_ID = 0x%x) \n"			
	"OEM_ID			:	0x%x\n"			
	"Product_Name	:	%s\n"		
	"Product_Revision	:	%x \n"		
	"Product_Serial_Name	:	0x%x\n"	
	"Product_Date		:	%d-%d\n\n",		
	emmc_vendor, 	card->cid.manfid,		
	card->cid.oemid,					
	card->cid.prod_name,				
	card->cid.prv,						
	card->cid.serial,					
...
static struct attribute *mmc_std_attrs[] = {
	...
	&dev_attr_emmc_information.attr,		// wangjun8_1
	NULL,
};
