 ...
 */
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION3
bool sd_slot_present = false;
#endif
	...
	if (host->ios.power_mode == MMC_POWER_ON)
        ...
        if (!strcmp(mmc_hostname(host),"mmc1")) {
            if (host->ops->get_cd && host->ops->get_cd(host) == 0) {
                sd_slot_present = false;
            } else {
                sd_slot_present = true;
            }
        }
#endif
	mmc_host_clk_hold(host);
...
void mmc_power_off(struct mmc_host *host)
{
	...
	*Sometime system suspend fail and will resume, sd card will init again.
	*at thistime some sd card maybe init fail, because in mmc_sd_resume()
	*we cancel sd power cycle and sd power cannot reduce when last fail suspend.
	*So we delay more power off time when sd is present.
	*/
	if(!strcmp(mmc_hostname(host),"mmc1"))
	{
		if(host->ops->get_cd && host->ops->get_cd(host) == 0){
			pr_debug("mmc1: power off delay 1ms when sd is absent\n");
			mmc_delay(1);
		}
		else
		{
			pr_debug("mmc1: power off delay 100ms when sd is present\n");
			mmc_delay(100);
		}
	}
	else
