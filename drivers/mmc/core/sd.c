...
extern	bool SD_card_slot_present;
#endif 
...
int mmc_attach_sd(struct mmc_host *host)
{
	...
	printk("yulong: %s send_app_op return %d when mmc_attach_sd\n",mmc_hostname(host),err);

	#if defined (CONFIG_SIM_SD_COMM_SLOT) || defined (CONFIG_SIM_SD_COMM_SLOT_VERSION2)
	/*
	*Add by wangshujun:On SK1-02,SD card and Sim card share common slot.
	*In order to avoid damage sim card,we cancel sd card power up when boot,
	*but this maybe lead to ACMD41 return error when system boot with sd card,
	*so here we power cycle and try again.
	*/
	if(err) {
		if(!strcmp(mmc_hostname(host),"mmc0"))//For emmc,we don't need this power cycle.
		return err;

#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
		if(strcmp(mmc_hostname(host),"mmc1")==0) {
                        SD_card_slot_present = !!mmc_gpio_get_status(host);
                        if(SD_card_slot_present == 1)
                        printk(KERN_ERR "zzz mmc1 update SD_card_slot_present falg = 1.\n");
                }
#endif
                mmc_power_off(host);
		usleep_range(5000, 5500);
		...
		err = mmc_send_app_op_cond(host, 0, &ocr);
		printk("yulong: %s send_app_op_2 return %d when mmc_attach_sd\n",mmc_hostname(host),err);
