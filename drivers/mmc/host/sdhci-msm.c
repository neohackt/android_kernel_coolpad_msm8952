						...
						u8 poll)
{
	...
	while (ck_out_en != poll) {
		if (--wait_cnt == 0) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: CK_OUT_EN bit is not %d\n",
...
static int msm_enable_cdr_cm_sdc4_dll(struct sdhci_host *host)
{
	...
	goto out;
err:
	pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed\n", mmc_hostname(host->mmc), __func__);
out:
	return rc;
}
...
static int msm_config_cm_dll_phase(struct sdhci_host *host, u8 phase)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: Enter %s\n", mmc_hostname(mmc), __func__);
	spin_lock_irqsave(&host->lock, flags);
...
err_out:
	pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: Failed to set DLL phase: %d\n",
		mmc_hostname(mmc), __func__, phase);
out:
	spin_unlock_irqrestore(&host->lock, flags);
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: Exit %s\n", mmc_hostname(mmc), __func__);
	return rc;
}
				...
				u8 *phase_table, u8 total_phases)
{
	...
	if (!total_phases || (total_phases > MAX_PHASES)) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: invalid argument: total_phases=%d\n",
	...
	if (ret >= MAX_PHASES) {
		ret = -EINVAL;
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: invalid phase selected=%d\n",
...
static int msm_init_cm_dll(struct sdhci_host *host)
{
	...
	while (!(readl_relaxed(host->ioaddr + CORE_DLL_STATUS) &
		...
		if (--wait_cnt == 0) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: DLL failed to LOCK\n",
				mmc_hostname(mmc), __func__);
			rc = -ETIMEDOUT;
			goto out;
		}
...
static int sdhci_msm_cdclp533_calibration(struct sdhci_host *host)
{
	...
	if (ret == -ETIMEDOUT) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: CDC Calibration was not completed\n",
				mmc_hostname(host->mmc), __func__);
		goto out;
	}
	...
	if (cdc_err) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: CDC Error Code %d\n",
	...
	writel_relaxed((readl_relaxed(host->ioaddr + CORE_DDR_200_CFG)
			...
			host->ioaddr + CORE_DDR_200_CFG);
out:
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: Exit %s, ret:%d\n", mmc_hostname(host->mmc),
			__func__, ret);
	return ret;
}
...
static int sdhci_msm_cm_dll_sdc4_calibration(struct sdhci_host *host)
{
	...
	if (ret == -ETIMEDOUT) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: CM_DLL_SDC4 Calibration was not completed\n",
				mmc_hostname(host->mmc), __func__);
		goto out;
	}
		...
		u8 drv_type)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: set card drive type to %d\n",
			mmc_hostname(mmc), __func__,
			drv_type);
}
...
retry:
	...
	do {
			...
			while (sts_retry) {
				...
				if (sts_cmd.error ||
				   ...
				   != R1_STATE_TRAN)) {
					...
					pr_debug("wangjun8    ---	sdhci-msm    :    %s: phase %d sts cmd err %d resp 0x%x\n",
		...
		if (!cmd.error && !data.error &&
			...
			pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: found *** good *** phase = %d\n",
				mmc_hostname(mmc), __func__, phase);
		} else {
			pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: found ## bad ## phase = %d\n",
				mmc_hostname(mmc), __func__, phase);
		}
	} while (++phase < 16);
	...
	if ((tuned_phase_cnt == NUM_TUNING_PHASES) &&
		 ...
		 */
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: tuned phases count: %d\n", mmc_hostname(mmc),
				tuned_phase_cnt);
	...
	if (tuned_phase_cnt) {
		...
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: finally setting the tuning phase to %d\n",
				mmc_hostname(mmc), __func__, phase);
	} else {
		if (--tuning_seq_cnt)
			goto retry;
		/* tuning failed */
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: no tuning point found\n",
			mmc_hostname(mmc), __func__);
		rc = -EIO;
	}
...
static int sdhci_msm_setup_gpio(struct sdhci_msm_pltfm_data *pdata, bool enable)
{
	...
	for (i = 0; i < curr->size; i++) {
		if (!gpio_is_valid(curr->gpio[i].no)) {
			ret = -EINVAL;
			pr_err("wangjun8    ---	sdhci-msm    :    %s: Invalid gpio = %d\n", __func__,
					curr->gpio[i].no);
			goto free_gpios;
		}
		if (enable) {
			ret = gpio_request(curr->gpio[i].no,
						curr->gpio[i].name);
			if (ret) {
				pr_err("wangjun8    ---	sdhci-msm    :    %s: gpio_request(%d, %s) failed %d\n",
		...
		bool enable)
{
	...
	if (ret < 0)
		pr_err("wangjun8    ---	sdhci-msm    :    %s state for pinctrl failed with %d\n",
						...
						struct sdhci_host *host)
{
	...
	//	if (gpio_is_valid(pdata->status_gpio) & !(flags & OF_GPIO_ACTIVE_LOW))
		...
		//	pr_err("wangjun8_sd_detct_MMC_CAP2_CD_ACTIVE_HIGH--- gpio ( %d )\n", gpio);
		pr_err("wangjun8_sd_detct_MMC_CAP2_CD_ACTIVE_HIGH--- gpio\n");
					     ...
					     unsigned long *flags)
{
	...
	if (vote != msm_host->msm_bus_vote.curr_vote) {
		...
		spin_lock_irqsave(&host->lock, *flags);
		if (rc) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: msm_bus_scale_client_update_request() failed: bus_client_handle=0x%x, vote=%d, err=%d\n",
				mmc_hostname(host->mmc),
					...
					struct sdhci_msm_reg_data *vreg)
{
	...
	if (IS_ERR(vreg->reg)) {
		...
		pr_err("wangjun8    ---	sdhci-msm    :    %s: devm_regulator_get(%s) failed. ret=%d\n",
	...
	if (regulator_count_voltages(vreg->reg) > 0) {
		...
		if (!vreg->high_vol_level || !vreg->hpm_uA) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s invalid constraints specified\n",
			       __func__, vreg->name);
			ret = -EINVAL;
		}
	}
						  ...
						  *vreg, int uA_load)
{
	...
	if (vreg->set_voltage_sup) {
		...
		if (ret < 0)
			pr_err("wangjun8    ---	sdhci-msm    :    %s: regulator_set_optimum_mode(reg=%s,uA_load=%d) failed. ret=%d\n",
					...
					int min_uV, int max_uV)
{
	...
	if (vreg->set_voltage_sup) {
		...
		if (ret) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: regulator_set_voltage(%s)failed. min_uV=%d,max_uV=%d,ret=%d\n",
...
static int sdhci_msm_vreg_enable(struct sdhci_msm_reg_data *vreg)
{
	...
	if (ret) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: regulator_enable(%s) failed. ret=%d\n",
				__func__, vreg->name, ret);
		return ret;
	}
...
static int sdhci_msm_vreg_disable(struct sdhci_msm_reg_data *vreg)
{
	...
	if (vreg->is_enabled && !vreg->is_always_on) {
		...
		if (ret) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: regulator_disable(%s) failed. ret=%d\n",
				__func__, vreg->name, ret);
			goto out;
		}
...
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
struct regulator {
        struct device *dev;
...
extern int SD_card_slot_present;
static char *sup_name;
#endif

#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION4
struct regulator {
        struct device *dev;
        struct list_head list;
        unsigned int always_on:1;
...
struct sdhci_msm_reg_data *yl_sd_vreg_table[2]={NULL,NULL};
static char *sup_name;
#endif
			...
			bool enable, bool is_init)
{
	...
	if (!curr_slot) {
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: vreg info unavailable,assuming the slot is powered by always on domain\n",
			 __func__);
		goto out;
	}
...
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
	if(0 == SD_card_slot_present) {
	sup_name = ((struct regulator *)curr_slot->vdd_data->reg)->supply_name;
		if(!strcmp(sup_name, "7864900.sdhci-vdd")) {
		//in case to fire SIM Card. frankie. 2016-01-26.	
		printk(KERN_ERR "zzz mmc1 No SD card slot, disable VDD LDO\n");
		enable = 0;
		}
	}
#endif
     ...
     sup_name = ((struct regulator *)curr_slot->vdd_data->reg)->supply_name;
      		if(!strcmp(sup_name, "7864900.sdhci-vdd")) {
                     yl_sd_vreg_table[0]=vreg_table[0];
		     printk(KERN_ERR "vdd supply name is %s ,enbale=%d \n",sup_name,enable);
		}

      sup_name = ((struct regulator *)curr_slot->vdd_io_data->reg)->supply_name;
		if(!strcmp(sup_name, "7864900.sdhci-vdd-io")) {
                     yl_sd_vreg_table[1]=vreg_table[1];
                     printk(KERN_ERR "vdd io supply name is %s enable=%d \n",sup_name,enable);
		}
...
int YL_sdhci_msm_setup_vreg_L11_L12(void)
{
	int ret =100, i;

	if ((yl_sd_vreg_table[0] !=NULL) & (yl_sd_vreg_table[1] !=NULL) )
	{
		for (i = 0; i < ARRAY_SIZE(yl_sd_vreg_table); i++) {
			
				ret = sdhci_msm_vreg_disable(yl_sd_vreg_table[i]);
				if (ret)
					goto out;
			}
				...
				bool is_init)
{
	...
	#ifdef CONFIG_SIM_SD_COMM_SLOT
	/*
	*add by wangshujun:On SK1-02,SD card and Sim card share common slot.
	*When boot, sdhci_msm_vreg_reset()  will power on sd, and if Sim card
	*insert at this time,it maybe damage sim card.So we cancel sd card power
	*reset.(but not modify emmc).(notice:before kernel boot,sd will also power on,and
	*we must handle that in lK or sbl1)
	*/
	if(strcmp(dev_name(dev),"7864900.sdhci")==0)
	/*Notice:different platform,dev_name is maybe not "7864900.sdhci"*/
	pr_debug("mmc1: cancel sd card power reset\n");
	else
			...
			unsigned int voltage_level)
{
	...
	if (vdd_io_reg && vdd_io_reg->is_enabled) {
		...
		case VDD_IO_SET_LEVEL:
			...
			break;
		default:
			pr_err("wangjun8    ---	sdhci-msm    :    %s: invalid argument level = %d",
					__func__, level);
			ret = -EINVAL;
			return ret;
		}
...
static irqreturn_t sdhci_msm_sdiowakeup_irq(int irq, void *data)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: irq (%d) received\n", __func__, irq);
...
void sdhci_msm_dump_pwr_ctrl_regs(struct sdhci_host *host)
{
	...
	pr_err("wangjun8    ---	sdhci-msm    :    %s: PWRCTL_STATUS: 0x%08x | PWRCTL_MASK: 0x%08x | PWRCTL_CTL: 0x%08x\n",
		mmc_hostname(host->mmc),
...
static irqreturn_t sdhci_msm_pwr_irq(int irq, void *data)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: Received IRQ(%d), status=0x%x\n",
	...
	while (irq_status &
		...
		if (retry == 0) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: Timedout clearing (0x%x) pwrctl status register\n",
	...
	if (likely(retry < 10))
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: success clearing (0x%x) pwrctl status register, retries left %d\n",
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: Handled IRQ(%d), ret=%d, ack=0x%x\n",
...
static void sdhci_msm_check_power_status(struct sdhci_host *host, u32 req_type)
{
	...
	spin_lock_irqsave(&host->lock, flags);
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: request %d curr_pwr_state %x curr_io_level %x\n",
	...
	if (req_type & (REQ_IO_HIGH | REQ_IO_LOW)) {
		if (!(io_sig_sts & SWITCHABLE_SIGNALLING_VOL) ||
			...
			pr_debug("wangjun8    ---	sdhci-msm    :    %s: do not wait for power IRQ that never comes\n",
					mmc_hostname(host->mmc));
			spin_unlock_irqrestore(&host->lock, flags);
			return;
		}
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: request %d done\n", mmc_hostname(host->mmc),
			__func__, req_type);
}
...
static int sdhci_msm_enable_controller_clock(struct sdhci_host *host)
{
	...
	if (!IS_ERR(msm_host->pclk)) {
		...
		if (rc) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the pclk with error %d\n",
	...
	if (rc) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the host-clk with error %d\n",
		       mmc_hostname(host->mmc), __func__, rc);
		goto disable_pclk;
	}
	...
	if (!IS_ERR(msm_host->ice_clk)) {
		...
		if (rc) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the ice-clk with error %d\n",
				mmc_hostname(host->mmc), __func__, rc);
			goto disable_host_clk;
		}
	}
	atomic_set(&msm_host->controller_clock, 1);
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: enabled controller clock\n",
			mmc_hostname(host->mmc), __func__);
	goto out;
...
static int sdhci_msm_prepare_clocks(struct sdhci_host *host, bool enable)
{
	...
	if (enable && !atomic_read(&msm_host->clks_on)) {
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: request to enable clocks\n",
				mmc_hostname(host->mmc));
		...
		if (!IS_ERR_OR_NULL(msm_host->bus_clk)) {
			...
			if (rc) {
				pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the bus-clock with error %d\n",
					mmc_hostname(host->mmc), __func__, rc);
		...
		if (!IS_ERR(msm_host->ff_clk)) {
			...
			if (rc) {
				pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the ff_clk with error %d\n",
					mmc_hostname(host->mmc), __func__, rc);
		...
		if (!IS_ERR(msm_host->sleep_clk)) {
			...
			if (rc) {
				pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed to enable the sleep_clk with error %d\n",
					mmc_hostname(host->mmc), __func__, rc);
				goto disable_ff_clk;
			}
		}
		...
		if (host->mmc->card_clock_off)
			return 0;
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: request to disable clocks\n",
				mmc_hostname(host->mmc));
...
static void sdhci_msm_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (curr_ios.timing == MMC_TIMING_MMC_HS400) {
		...
		if (!host->mmc->ios.old_rate && !msm_host->use_cdclp533) {
			...
			if (rc == -ETIMEDOUT)
				pr_err("wangjun8    ---	sdhci-msm    :    %s: Unable to get DLL_LOCK/DDR_DLL_LOCK, dll_status: 0x%08x\n",
						mmc_hostname(host->mmc),
						dll_lock);
		}
	} else {
	...
	if (sup_clock != msm_host->clk_rate) {
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: setting clk rate to %u\n",
				mmc_hostname(host->mmc), __func__, sup_clock);
		rc = clk_set_rate(msm_host->clk, sup_clock);
		if (rc) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: Failed to set rate %u for host-clk : %d\n",
					mmc_hostname(host->mmc), __func__,
					sup_clock, rc);
			return;
		}
					...
					unsigned int uhs)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s-clock:%u uhs mode:%u ctrl_2:0x%x\n",
...
static void sdhci_msm_disable_data_xfer(struct sdhci_host *host)
{
	...
	if (ret) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: can't stop ongoing AHB bus access by ADMA\n",
				mmc_hostname(host->mmc), __func__);
		BUG();
	}
...
int sdhci_msm_notify_load(struct sdhci_host *host, enum mmc_load state)
{
	...
	if (!IS_ERR(msm_host->ice_clk)) {
		...
		if (msm_host->ice_clk_rate == clk_rate)
			return 0;
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: changing ICE clk rate to %u\n",
				mmc_hostname(host->mmc), clk_rate);
		ret = clk_set_rate(msm_host->ice_clk, clk_rate);
		if (ret) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: ICE_CLK rate set failed (%d) for %u\n",
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_SIM_SD_COMM_SLOT
	char *msm_plat = "8952"; //msm8952's sdc clock is different from 8916/8939.
	#endif
	...
	/*
	*add by wangshujun:On SK1-02,SD card and Sim card share common slot.
	*When system boot,if insert sim card and then is powing on sdcard
	*it maybe damage sim card,so cancel power up sd card early.
	*Also we must set clock false here,or it will lead to system don't suspend
	*when system boot without sd slot.
	*/
	if(strcmp(mmc_hostname(host->mmc),"mmc1")==0) {
		msm_host->mmc->caps2 |= MMC_CAP2_NO_PRESCAN_POWERUP;
		if (atomic_read(&msm_host->controller_clock)) {
			if(strstr(machine_name, msm_plat)) {
			pr_debug("mmc1 msm8952 does not need to close the SD clock here\n");
			}
			else {
			pr_debug("mmc1 the SD clock is open...we close here\n");
			sdhci_msm_prepare_clocks(host, false);
			}
		}
	}	
	...
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		...
		#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
		if(strcmp(mmc_hostname(host->mmc),"mmc1")==0) {
		  	SD_card_slot_present = !!mmc_gpio_get_status(host->mmc);
			if(SD_card_slot_present == 1)	
			printk(KERN_ERR "zzz mmc1 SLOT is inserted when bootup.\n");	
		}
		#endif
	}
	...
	if (ret)
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: pm_runtime_set_active failed: err: %d\n",
	...
	if (ret) {
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: failed creating auto-cmd21 attr: %d\n",
		       mmc_hostname(host->mmc), __func__, ret);
	...
	if (msm_host->pdata->mpm_sdiowakeup_int != -1) {
		...
		if (ret) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: enabling wakeup: failed: ret: %d\n",
			       mmc_hostname(host->mmc), ret);
			ret = 0;
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s\n", dev_name(&pdev->dev), __func__);
	if (!gpio_is_valid(msm_host->pdata->status_gpio))
...
out:
	if (ret)
		pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: %sable wakeup: failed: %d gpio: %d mpm: %d\n",
...
skip_disable_host_irq:
	...
	if (host->is_crypto_en) {
		...
		if (ret < 0)
			pr_err("wangjun8    ---	sdhci-msm    :    %s: failed to suspend crypto engine %d\n",
					mmc_hostname(host->mmc), ret);
	}
	return 0;
}
...
static int sdhci_msm_runtime_resume(struct device *dev)
{
	...
	if (host->is_crypto_en) {
		...
		if (ret) {
			pr_err("wangjun8    ---	sdhci-msm    :    %s: Failed to enable reqd clocks\n",
					mmc_hostname(host->mmc));
			goto skip_ice_resume;
		}
		ret = sdhci_msm_ice_resume(host);
		if (ret)
			pr_err("wangjun8    ---	sdhci-msm    :    %s: failed to resume crypto engine %d\n",
					mmc_hostname(host->mmc), ret);
	}
skip_ice_resume:
...
static int sdhci_msm_suspend(struct device *dev)
{
	...
	if (pm_runtime_suspended(dev)) {
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: already runtime suspended\n",
		mmc_hostname(host->mmc), __func__);
		goto out;
	}
...
static int sdhci_msm_resume(struct device *dev)
{
	...
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		ret = mmc_gpio_request_cd(msm_host->mmc,
				msm_host->pdata->status_gpio);
		if (ret)
			pr_err("wangjun8    ---	sdhci-msm    :    %s: %s: Failed to request card detection IRQ %d\n",
					mmc_hostname(host->mmc), __func__, ret);
	}

	if (pm_runtime_suspended(dev)) {
		pr_debug("wangjun8    ---	sdhci-msm    :    %s: %s: runtime suspended, defer system resume\n",
		mmc_hostname(host->mmc), __func__);
		goto out;
	}
...
static int sdhci_msm_suspend_noirq(struct device *dev)
{
	...
	if (atomic_read(&msm_host->clks_on)) {
		pr_warn("%s: %s: clock ON after suspend, aborting suspend\n",
			mmc_hostname(host->mmc), __func__);
#ifdef CONFIG_SIM_SD_COMM_SLOT_VERSION2
                        pr_warn("mmc/SD clock is open...we close here\n");
                        sdhci_msm_prepare_clocks(host, false);
#endif
#ifdef CONFIG_SIM_SD_COMM_SLOT
			if(strcmp(mmc_hostname(host->mmc),"mmc1") == 0) {
			pr_warn("mmc1 the SD clock is open...we close here\n");
			sdhci_msm_prepare_clocks(host, false);
			}
#endif
		ret = -EAGAIN;
