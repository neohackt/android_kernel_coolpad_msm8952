...
static DECLARE_RWSEM(yl_params_pm_sem);
...
static ssize_t yl_params_read_locked(char *buf)
{
	...
	if (len == ONE_BLOCK_SIZE)
		...
		pr_err("read block %s failed, ret %ld\n", buf, len);
	return len;
}
...
static int yl_params_pm_notify(struct notifier_block *b,
			unsigned long event, void *p)
{
	switch (event) {
	case PM_SUSPEND_PREPARE:
		down_write(&yl_params_pm_sem);
		break;

	case PM_POST_SUSPEND:
		up_write(&yl_params_pm_sem);
		break;
	}
...
static struct notifier_block yl_params_pm_notifier = {
	.notifier_call = yl_params_pm_notify,
};
...
int yl_params_init(struct mmc_card *card)
{
	...
	register_pm_notifier(&yl_params_pm_notifier);
	return 0;
