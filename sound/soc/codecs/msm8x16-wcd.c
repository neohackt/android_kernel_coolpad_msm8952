...
static bool msm8x16_codec_registered = false;
bool is_8x16_wcd_codec_registered(void)
{
        return msm8x16_codec_registered;
}
/* Add end */
...
void msm8x16_wcd_spk_boost_ctl_cb(
		int (*codec_spk_boost_ctl_cb)(struct snd_soc_codec *codec,
			int enable), struct snd_soc_codec *codec)
{
	struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);

	pr_err("axl %s: Enter\n", __func__);
	msm8x16_wcd->codec_spk_boost_ctl_cb = codec_spk_boost_ctl_cb;
}
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);
        ...
        pr_err("axl in %s,event:%d,control name:%s\n",__func__,event,w->name);
	...
	case SND_SOC_DAPM_POST_PMU:
		usleep_range(CODEC_DELAY_1_MS, CODEC_DELAY_1_1_MS);
                pr_err("axl in %s,set the GPIO to high\n",__func__);
                if (msm8x16_wcd->codec_spk_boost_ctl_cb)
			msm8x16_wcd->codec_spk_boost_ctl_cb(codec, 1);
		switch (msm8x16_wcd->boost_option) {
		case BOOST_SWITCH:
			if (msm8x16_wcd->spk_boost_set)
				snd_soc_update_bits(codec,
	...
	case SND_SOC_DAPM_PRE_PMD:
                 pr_err("axl in %s,set the GPIO to low\n",__func__);
                if (msm8x16_wcd->codec_spk_boost_ctl_cb)
			msm8x16_wcd->codec_spk_boost_ctl_cb(codec, 0);
		snd_soc_update_bits(codec,
...
static int msm8x16_wcd_spmi_remove(struct spmi_device *spmi)
{
	...
	/* Added by Yulong for boot optimization, fengdongjie */
        msm8x16_codec_registered = false;
        /* Add end */
	return 0;
}
