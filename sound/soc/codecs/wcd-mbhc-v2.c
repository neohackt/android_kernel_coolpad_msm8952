...
static void wcd_program_btn_threshold(const struct wcd_mbhc *mbhc, bool micbias)
{
	...
	mbhc->mbhc_cb->set_btn_thr(codec, btn_low, btn_high, btn_det->num_btn,
				   micbias);
//function :  resoving problem for headset  ; name:sunyonggnag ; date:2/1/2015
...
snd_soc_update_bits(codec, 0x156, 0xFC, 0x80);//on msm8952 platform,for compatible Syllable headset of volumedown in voice by fengdongjie
...
//func:support headset 
//type tips  and selfie stick
//date:8/1/2016
//name:sun/yonggang

#if  defined (CONFIG_HEADSET_TYPE_TIPS_AND_SELFIE_STICK)

   else if ((jack_type == SND_JACK_HEADSET)||
   (jack_type == (SND_JACK_HEADSET | SND_JACK_UNSUPPORTED))){
#else
		else if (jack_type == SND_JACK_HEADSET) {
#endif
...
//func:support iphone headset record support
//date:9/7/2016
//name:sunyonggang
#if defined (CONFIG_HEADSET_IPHONE_SUPPORT)

                pr_debug("func = %s GND_MIC_SWAP to HEADSET \n", __func__);
                WCD_MBHC_RSC_LOCK(mbhc);
...
#if defined (CONFIG_HEADSET_IPHONE_SUPPORT)
         {
             pr_debug("func = %s HIGH_HPH to HEADSET \n", __func__);
             plug_type = MBHC_PLUG_TYPE_HEADSET;
             mbhc->micbias_enable = true;
          }
#else
...
if (mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic && mbhc->micbias_enable) {
#else
                        if (mbhc->micbias_enable) {
#endif
...
//name:sunyonggang
                ...
                pr_debug("%s: cable is extension cable HIGH_HPH to HEADSET\n", __func__);
                plug_type = MBHC_PLUG_TYPE_HEADSET;
                 mbhc->micbias_enable = true;
               if(mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE)
                 {   
                      pr_debug("%s: syg:cable is extension cable HEADPHONE to HIGH_HPH to HEADSET\n", __func__);
                      goto report;
                 }
...
//func:support headset selfie stick
//date:8/1/2016
//name:sun/yonggang
#if defined (CONFIG_HEADSET_TYPE_TIPS_AND_SELFIE_STICK)
if(plug_type == MBHC_PLUG_TYPE_GND_MIC_SWAP)
...
#if  defined (CONFIG_HEADSET_TYPE_TIPS_AND_SELFIE_STICK)
                          if(mbhc->hph_status & SND_JACK_UNSUPPORTED){
                           wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADSET 
                           | SND_JACK_UNSUPPORTED);
                        } else {
                          wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADSET);
		      ...
		      bool impedance_det_en)
{
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		ret = snd_jack_set_key(mbhc->button_jack.jack,
           ...
           //function:  headset key modify
           //name:sunyonggnag ; date:2/1/2015
           #if defined (CONFIG_YL_AUDIO_HEADSET)
                ret = snd_jack_set_key(mbhc->button_jack.jack,
                                       SND_JACK_BTN_1,
                                       KEY_VOLUMEUP);
