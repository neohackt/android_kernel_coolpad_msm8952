...
/* add  code as follows, if need to open more log of this file*/
#define DEBUG
#undef pr_debug
...
//atomic_t quat_mi2s_ref;//add by chenshuanglong for USE_QUAT_MI2S
static int msm_q6_enable_m2is_clocks(bool enable);
static int quat_mi2s_clk_ctl_get(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol);
static int quat_mi2s_clk_ctl_put(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol);
#endif
#if defined(CONFIG_SND_SOC_USE_QUIN_MI2S)
static int msm_q6_enable_m2is_clocks(bool enable);
static int quin_mi2s_clk_ctl_get(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol);
static int quin_mi2s_clk_ctl_put(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol);
#endif
...
static struct wcd_mbhc_config mbhc_cfg = {
	...
	.key_code[0] = KEY_MEDIA,
        #if defined (CONFIG_YL_AUDIO_HEADSET)
...
#ifdef CONFIG_EXT_SPK_PA_VDD
//author:sunyonggang
//date:2016-7-2
//func:add extern speak PA

int is_spk_boost_ctl_support(struct platform_device *pdev,
                       struct msm8916_asoc_mach_data *pdata)
{
       const char *spk_boost_ctl = "qcom,msm-spk-boost-ctl";

       pr_err(" axl %s:Enter\n", __func__);

       pdata->spk_boost_ctl_gpio = of_get_named_gpio(pdev->dev.of_node,
                               spk_boost_ctl, 0);
        pr_err("axl in %s,pdata->spk_boost_ctl_gpio=%d\n",__func__,pdata->spk_boost_ctl_gpio);
       if (pdata->spk_boost_ctl_gpio < 0) {
               dev_dbg(&pdev->dev,
                       "%s: missing %s in dt node\n", __func__, spk_boost_ctl);
       } else {
               if (!gpio_is_valid(pdata->spk_boost_ctl_gpio)) {
...
static int enable_boost_ctl(struct snd_soc_codec *codec, int enable)
{
       ...
       if (!gpio_is_valid(pdata->spk_boost_ctl_gpio)) {
               pr_err("%s: Invalid gpio: %d\n", __func__,
                       pdata->spk_boost_ctl_gpio);
               return false;
       }

       pr_err("axl %s: %s boost ctl\n", __func__,
               enable ? "Enable" : "Disable");

       if (enable) {
               //ret = msm_gpioset_activate(CLIENT_WCD_INT, "spk_boost_gpio");
                ret=gpio_request(pdata->spk_boost_ctl_gpio,"spk_boost_gpio");
               if (ret) {
                       pr_err("%s: gpio request fail %s\n",
                                       __func__, "spk_boost_gpio");
                       return ret;
               }
                gpio_direction_output(pdata->spk_boost_ctl_gpio,1);
               gpio_free(pdata->spk_boost_ctl_gpio);
                pr_err("axl gpio_value=%d\n",!!gpio_get_value(pdata->spk_boost_ctl_gpio));
       } else {
               //gpio_set_value_cansleep(pdata->spk_boost_ctl_gpio, enable);
               //ret = msm_gpioset_suspend(CLIENT_WCD_INT, "spk_boost_gpio");
                ret=gpio_request(pdata->spk_boost_ctl_gpio,"spk_boost_gpio");
               if (ret) {
                       pr_err("%s: gpio request fail %s\n",
                                       __func__, "spk_boost_gpio");
                ...
                gpio_direction_output(pdata->spk_boost_ctl_gpio,0);
                gpio_free(pdata->spk_boost_ctl_gpio);
       }
       return 0;
}
...
#if defined(CONFIG_SND_SOC_USE_QUAT_MI2S)
	SOC_SINGLE_BOOL_EXT("QUAT MI2S Clk Ctrl", 0,
			quat_mi2s_clk_ctl_get, quat_mi2s_clk_ctl_put),
#endif
#if defined(CONFIG_SND_SOC_USE_QUIN_MI2S)
        SOC_SINGLE_BOOL_EXT("QUIN MI2S Clk Ctrl", 0,
                        quin_mi2s_clk_ctl_get, quin_mi2s_clk_ctl_put),
#endif
};
...
static int msm_q6_enable_m2is_clocks(bool enable)
{
	...
	printk(KERN_ERR "%s,Set msm_q6_enable_mi2s_clocks.\n",__FUNCTION__);
	...
	if (enable) {
		...
		rc = afe_port_start(AFE_PORT_ID_QUATERNARY_MI2S_RX, &port_config,
			port_config.i2s.sample_rate);
		if (IS_ERR_VALUE(rc)) {
			pr_err("fail to open AFE port %x\n", AFE_PORT_ID_QUATERNARY_MI2S_RX);
			return -EINVAL;
		}
	} else {
		rc = afe_close(AFE_PORT_ID_QUATERNARY_MI2S_RX);
		if (IS_ERR_VALUE(rc)) {
			pr_err("fail to close AFE port %x\n", AFE_PORT_ID_QUATERNARY_MI2S_RX);
			return -EINVAL;
		}
	}
	return rc ;
...
static int quat_mi2s_clk_ctl_get(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = (atomic_read(&quat_mi2s_clk_ref) > 0) ? 1 : 0;
        return 0;
}
                                ...
                                struct snd_ctl_elem_value *ucontrol)
{
	...
	//struct snd_pcm_substream *substrm = NULL;
	int ret = 0,val = 0;
	...
	if (ucontrol->value.integer.value[0]) {
	   ...
	   if (atomic_inc_return(&quat_mi2s_clk_ref) == 1) {
		if ((pdata->ext_pa & QUAT_MI2S_ID) == QUAT_MI2S_ID) {
			...
			ret = msm_q6_enable_m2is_clocks(true);
			if (ret < 0) {
				pr_err("%s: afe_set_lpass_clock failed\n", __FUNCTION__);
				return ret;
			}
	   ...
	   if (atomic_dec_return(&quat_mi2s_clk_ref) == 0) {
        	if (!pdata->codec_type) {
           		ret=afe_close(AFE_PORT_ID_QUATERNARY_MI2S_RX);
		      	if(IS_ERR_VALUE(ret)){
...
static int msm_q6_enable_m2is_clocks(bool enable)
{
	...
	if (enable) {
		...
		port_config.i2s.bit_width = 16;
		port_config.i2s.ws_src = 1;
		rc = afe_port_start(AFE_PORT_ID_QUINARY_MI2S_RX, &port_config,
			port_config.i2s.sample_rate);
		if (IS_ERR_VALUE(rc)) {
			pr_err("fail to open AFE port %x\n", AFE_PORT_ID_QUINARY_MI2S_RX);
			return -EINVAL;
...
static int quin_mi2s_clk_ctl_get(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = (atomic_read(&quin_mi2s_clk_ref) > 0) ? 1 : 0;
        return 0;
}
                                ...
                                struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_ERR"%s:quin_mi2s_clk_ref =%d,.",__func__,atomic_read(&quin_mi2s_clk_ref));
	printk(KERN_ERR"%s:mclk_rsc_ref =%d .",__func__,atomic_read(&pdata->mclk_rsc_ref));
	printk(KERN_ERR"%s:set value =%ld .",__func__,ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
	   if (atomic_inc_return(&quin_mi2s_clk_ref) == 1) {
		if ((pdata->ext_pa & QUIN_MI2S_ID) == QUIN_MI2S_ID) {
			if (pdata->vaddr_gpio_mux_quin_ctl) {
	        ...
	        mi2s_rx_clk.clk_val1 = Q6AFE_LPASS_IBIT_CLK_1_P536_MHZ;
			ret = afe_set_lpass_clock(AFE_PORT_ID_QUINARY_MI2S_RX, &mi2s_rx_clk);
			if (ret < 0) {
				pr_err("%s: afe_set_lpass_clock failed\n", __FUNCTION__);
				return ret;
			}
	  ...
	  if (atomic_dec_return(&quin_mi2s_clk_ref) == 0) {
        	if (!pdata->codec_type) {
           		ret=afe_close(AFE_PORT_ID_QUINARY_MI2S_RX);
		      	if(IS_ERR_VALUE(ret)){
                	printk(KERN_ERR"fail to close AFE port\n");
			  	}
				mi2s_rx_clk.clk_val1 = Q6AFE_LPASS_IBIT_CLK_DISABLE;
...
//func:add extern speak PA
#ifdef CONFIG_EXT_SPK_PA_VDD
        msm8x16_wcd_spk_boost_ctl_cb(enable_boost_ctl,codec);
#endif //CONFIG_EXT_SPK_PA_VDD
...
static struct snd_soc_dai_link msm8952_dai[] = {
	...
	},
	{
		...
		.platform_name = "msm-pcm-routing",
 #if defined(CONFIG_SND_SOC_TFA9890_STUB)
			.codec_name = "tfa9890-stub-codec.1",
			.codec_dai_name = "tfa9890-stub-rx",
 #else
		.codec_dai_name = "snd-soc-dummy-dai",
		.codec_name = "snd-soc-dummy",
 #endif
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
	...
	/* Added by Yulong for boot optimization, fengdongjie*/
        if (!is_8x16_wcd_codec_registered()) {
                dev_err(&pdev->dev, "CODEC 8x16_wcd not registered\n");
                return -EPROBE_DEFER;
        }
        /* Add end */
...
//func:add extern speak PA
#ifdef CONFIG_EXT_SPK_PA_VDD
	 ret = is_spk_boost_ctl_support(pdev,pdata);       
        if (ret < 0)
