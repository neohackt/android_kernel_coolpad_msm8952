...
#if defined(CONFIG_YL_RTC_PRINT)
#include<linux/rtc.h>
#include<linux/time.h>

extern void getnstimeofday_nolock(struct timespec *ts);
#endif
...
#if defined(CONFIG_YL_RTC_PRINT)
static bool printk_rtc = 1;
module_param_named(rtc, printk_rtc, bool, S_IRUGO | S_IWUSR);
#endif
...
static size_t print_time(u64 ts, char *buf)
{
        ...
        unsigned long rem_nsec_yl;
        struct timespec ts_tmp;
        struct rtc_time tm;

        if (!(printk_time || printk_rtc))
                return 0;
        getnstimeofday_nolock(&ts_tmp);
        rtc_time_to_tm(ts_tmp.tv_sec, &tm);

        rem_nsec = do_div(ts, 1000000000);
        rem_nsec_yl = do_div(ts_tmp.tv_nsec, 1000000000);

        if (!buf)
                return snprintf(NULL, 0, "[%5lu.000000] ", (unsigned long)ts);

        if(printk_rtc)
                        return sprintf(buf, "[%02d%02d %02d:%02d:%02d/%5lu.%06lu]@%d ",
                    /*tm.tm_year + 1900,*/ tm.tm_mon + 1, tm.tm_mday+(tm.tm_hour + 8)/24,
                    (tm.tm_hour + 8)%24, tm.tm_min, tm.tm_sec, (unsigned long)ts, rem_nsec / 1000,
                                        smp_processor_id());
        else
